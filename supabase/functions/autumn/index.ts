...
async function proxyToAutumn(
  ...
  const enrichedBody = body && typeof body === 'object'
...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(`[Autumn] ${identity.customerId} → ${route.method} ${route.autumnPath}`);
    ...
    return new Response(
      JSON.stringify({ error: 'Internal billing error' }),
