...
serve(async (req) => {
  ...
  try {
    ...
    const routingResponse = await openai.createChatCompletion({
      model: 'gpt-3.5-turbo',
      messages: [
        { role: 'system', content: 'You are the Bridgebox Intent Router. Read the user prompt and respond with EXACTLY ONE word: finance, operations, or general.' },
        { role: 'user', content: prompt }
      ],
      temperature: 0.0,
    });

    const route = routingResponse.data.choices[0].message?.content?.trim().toLowerCase() || 'general';
    const selectedAgent = AGENT_PERSONAS[route as keyof typeof AGENT_PERSONAS] || AGENT_PERSONAS.general;
    ...
    const executionResponse = await openai.createChatCompletion({
      model: 'gpt-4-turbo',
      messages: [
        { role: 'system', content: selectedAgent + `\n\nContext Data:\n${JSON.stringify(context_data, null, 2)}` },
        { role: 'user', content: prompt }
      ],
      temperature: 0.3,
    });

    const responseContent = executionResponse.data.choices[0].message?.content || '';
