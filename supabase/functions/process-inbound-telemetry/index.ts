  ...
  try {
    ...
    if (type !== "INSERT" || table !== "global_communications" || !record) {
      return new Response(
        JSON.stringify({
          status: "ignored",
          reason: "Not an inbound communication insert",
        }),
        { headers: corsHeaders },
      );
    }
    ...
    if (!bodyText.trim()) {
      return new Response(
        JSON.stringify({ status: "ignored", reason: "Empty payload" }),
        { headers: corsHeaders },
      );
    }
...
Communication: "${bodyText}"`;
    ...
    console.log(
      `Mapped Telemetry for Payload ${record.id}: Urgency [${urgency}], Sentiment [${sentiment}]`,
    );
    ...
    return new Response(
      JSON.stringify({ status: "success", escalated: false }),
      { headers: corsHeaders },
    );
  } catch (err) {
