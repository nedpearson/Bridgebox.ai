...
serve(async (req) => {
  ...
  try {
    ...
    if (!audio_base64) {
      return new Response(JSON.stringify({ error: 'Missing audio base64 payload' }), { status: 400, headers: corsHeaders });
    }

    // Convert Base64 back to Blob/File for OpenAI Whisper
    const binaryString = atob(audio_base64.replace(/^data:audio\/\w+;base64,/, ''));
    ...
    return new Response(JSON.stringify({ status: 'success', text }), { headers: corsHeaders });
