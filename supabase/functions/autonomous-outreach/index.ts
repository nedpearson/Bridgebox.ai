...
serve(async (req) => {
  ...
  try {
    ...
    if (!task_id || !context_data) {
      return new Response(JSON.stringify({ error: 'Missing telemetry bounds' }), { status: 400, headers: corsHeaders });
    }
...
OUTPUT FORMAT (STRICT JSON):
{
  ...
  "body": "The complete email body formatted in plain text."
    ...
    const draft = JSON.parse(executionResponse.data.choices[0].message?.content || '{}');

    return new Response(JSON.stringify({ status: 'success', draft }), { headers: corsHeaders });
