...
serve(async (req) => {
  ...
  try {
    ...
    if (!record_id || !record_type) {
      return new Response(JSON.stringify({ error: 'Missing target mapping IDs' }), { status: 400, headers: corsHeaders });
    }
...
Format your output EXACTLY as this JSON object:
{
  ...
  "proposed_tool_execution": { "tool_name": "create_global_task | update_project_target_date | null", "parameters": {} }
    ...
    return new Response(JSON.stringify({ status: 'success', nba: analysis }), { headers: corsHeaders });
