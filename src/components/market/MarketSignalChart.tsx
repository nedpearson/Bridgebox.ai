...
export function MarketSignalChart({
  signals,
  industry,
}: MarketSignalChartProps) {
  ...
  const byMonth = signals.reduce(
    ...
    {} as Record<
      string,
      { count: number; avgStrength: number; signals: MarketSignal[] }
    >,
  );
  ...
  Object.keys(byMonth).forEach((month) => {
    ...
    monthData.avgStrength =
      monthData.signals.reduce((sum, s) => sum + s.strength_score, 0) /
      monthData.count;
  });
