...
export default function AiUsageWidget({ organizationId }: Props) {
  ...
  const pct =
    budgetTotal > 0
      ? Math.min(100, Math.round((projectedCost / budgetTotal) * 100))
      : 0;
  ...
  const topFeatures: [string, { tokens: number; cost: number }][] =
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 text-xs">
              {fmtTokens(summary?.totalTokens ?? 0)} tokens used ·{" "}
              {fmtCurrency(costSoFar)} spent
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
      ...
      <div className="px-5 pb-4">
        ...
        <div className="flex justify-between text-xs text-slate-600 mt-1">
          <span>$0</span>
          {budgetTotal > 0 && (
            <span>Budget: {fmtCurrency(budgetTotal)}/mo</span>
          )}
        </div>
      </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  },
                  {
                    ...
                    value: fmtTokens(projectedTokens),
                    color: "#38bdf8",
                  },
                  {
                    label: "Est. Cost",
                    value: fmtCurrency(projectedCost),
                    color: "#34d399",
                  },
                ].map((stat) => (
                  <div
              ...
              {projectedCost > 0 && (
                <div className="flex items-start gap-2.5 bg-indigo-500/5 border border-indigo-500/15 rounded-xl px-4 py-3">
                  ...
                  <p className="text-slate-400 text-xs leading-relaxed">
                    Based on current usage pace, your AI cost this month will be
                    approximately{" "}
