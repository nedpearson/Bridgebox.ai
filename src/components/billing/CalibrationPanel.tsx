...
import {
  calibratePricingModel,
  type CalibrationResult,
} from "../../lib/billing/pricingCalibrator";
...
function DriftChip({ ratio }: { ratio: number }) {
  ...
  return (
    <span
      ...
      style={{
        color: positive ? "#fb923c" : "#34d399",
        background: positive ? "rgba(251,146,60,0.1)" : "rgba(52,211,153,0.1)",
      }}
    >
      {positive ? (
...
export default function CalibrationPanel({
  ...
  const confStyle =
    CONFIDENCE_STYLE[record.confidenceLabel] ?? CONFIDENCE_STYLE.low;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          borderColor: `${actionStyle.color}25`,
          background: `${actionStyle.color}08`,
        }}
      >
        <span style={{ color: actionStyle.color }} className="mt-0.5 shrink-0">
          {actionStyle.icon}
        </span>
        <div>
          ...
          {!wasCalibrated && (
            <p className="text-slate-500 text-xs mt-1">
              Pricing engine will auto-calibrate once{" "}
              {Math.max(0, 10 - record.dataSampleSize)} more AI events are
              recorded.
            </p>
          )}
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div className="bg-slate-800/50 rounded-xl p-3">
          ...
          <p className="text-slate-600 text-xs mt-1">
            {fmtTokens(record.projectedTokens30d)} projected
          </p>
        </div>
        ...
        <div className="bg-slate-800/50 rounded-xl p-3">
          ...
          <p className="text-white font-bold text-sm">
            {fmtCurrency(record.estimatedMonthlyTotal)}
          </p>
        ...
        >
          ...
          <p className="font-bold text-sm" style={{ color: "#818cf8" }}>
            {fmtCurrency(record.calibratedMonthlyTotal)}
          </p>
          <p className="text-slate-600 text-xs">/mo</p>
        </div>
      </div>
        ...
        <div className="bg-slate-800/30 rounded-xl p-4">
          <div className="flex items-center justify-between mb-3">
            <p className="text-slate-300 text-sm font-medium flex items-center gap-2">
              ...
              Usage Trend (last {Math.min(prediction.dailyBuckets.length, 14)}{" "}
              days)
            </p>
            <span className="text-slate-500 text-xs">
              {prediction.growthRatePercent > 0 ? "+" : ""}
              {prediction.growthRatePercent}% / 30d
            </span>
          </div>
          ...
          <div className="mt-3 flex items-center justify-between text-xs text-slate-500">
            <span>
              80% CI: {fmtTokens(prediction.lowerBound30d)} –{" "}
              {fmtTokens(prediction.upperBound30d)}
            </span>
            ...
            <div className="bg-slate-800/30 rounded-xl p-3">
              ...
              <div className="h-1.5 bg-slate-700 rounded-full overflow-hidden">
                <motion.div
                  ...
                  style={{
                    background:
                      record.workflowAccuracyScore > 70 ? "#34d399" : "#fb923c",
                  }}
                />
              </div>
              <p className="text-white text-xs font-semibold mt-1">
                {record.workflowAccuracyScore}%
              </p>
            </div>
          )}
          {record.integrationAccuracyScore !== null && (
            <div className="bg-slate-800/30 rounded-xl p-3">
              ...
              <div className="h-1.5 bg-slate-700 rounded-full overflow-hidden">
                <motion.div
                  ...
                  style={{
                    background:
                      record.integrationAccuracyScore > 70
                        ? "#34d399"
                        : "#fb923c",
                  }}
                />
              ...
              <p className="text-white text-xs font-semibold mt-1">
                {record.integrationAccuracyScore}%
              </p>
            </div>
          )}
        </div>
