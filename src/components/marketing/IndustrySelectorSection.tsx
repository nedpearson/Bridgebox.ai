...
const INDUSTRIES = [
  ...
  },
  {
    ...
    metrics: ["Forensic Workflows", "Tax Pipeline AI", "QuickBooks Bridge"],
  },
  {
    id: "bridal",
    icon: Scissors,
  ...
  },
  {
    ...
    metrics: ["GPS Driver Apps", "Load Boards", "Freight Billing"],
  },
  {
    id: "medspa",
    icon: HeartPulse,
    ...
    metrics: ["Patient Portals", "Appointment Logic", "Consumable Inventory"],
  },
  {
    id: "construction",
    icon: Building2,
    ...
    metrics: ["Job Costing", "Field Worker App", "Permit Tracking"],
  },
];
    ...
    <Section background="dark" className="py-24">
      <div className="text-center mb-16">
        ...
        <p className="text-lg text-slate-400 max-w-2xl mx-auto">
          Unlike generic software, Bridgebox builds the precise data schemas
          your specific industry requires instantly.
        </p>
      </div>
      ...
      <div className="grid lg:grid-cols-12 gap-12 max-w-6xl mx-auto">
        ...
        <div className="lg:col-span-5 grid grid-cols-2 gap-3">
          {INDUSTRIES.map((ind) => (
            ...
            >
              <div
                className={`w-12 h-12 rounded-lg ${ind.color} flex items-center justify-center mb-3`}
              >
                <ind.icon className="w-6 h-6" />
              </div>
              <span
                className={`text-sm font-bold ${activeIndustry.id === ind.id ? "text-white" : "text-slate-400"}`}
              >
                {ind.label}
              </span>
            </button>
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-slate-400 mb-8 leading-relaxed">
                    When deployed, Bridgebox instantly compiles a custom
                    database schema, native mobile routes, and automated logical
                    triggers specifically designed for {activeIndustry.label}{" "}
                    dynamics.
                  </p>

                  <div className="space-y-4 mb-8">
                    <div className="text-xs font-bold text-slate-500 uppercase tracking-widest">
                      Included Baseline Logic
                    </div>
                    {activeIndustry.metrics.map((metric, i) => (
                      <div
                        key={i}
                        className="flex items-center gap-3 bg-slate-900/50 p-3 rounded-xl border border-white/5"
                      >
