  ...
  return (
    <div className="bg-[#1e293b] border border-white/10 rounded-xl p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-lg font-semibold text-white mb-1">Client Health Score</h3>
          <p className="text-sm text-slate-400">
            Updated {new Date(score.calculated_at).toLocaleDateString()}
          </p>
        </div>
        <div className={`flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r ${getOverallColor(score.overall_score)} rounded-full text-white font-medium`}>
          {getTrend()}
          {getStatus()}
        </div>
      </div>
