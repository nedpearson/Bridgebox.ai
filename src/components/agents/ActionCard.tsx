...
export function ActionCard({
  ...
  const statusConfig = {
    suggested: { label: 'Suggested', color: 'bg-blue-50 text-blue-700 border-blue-200' },
    pending_review: { label: 'Pending Review', color: 'bg-amber-50 text-amber-700 border-amber-200' },
    approved: { label: 'Approved', color: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
    executed: { label: 'Executed', color: 'bg-gray-50 text-gray-600 border-gray-200' },
  ...
  const showActions = action.status === 'suggested' || action.status === 'pending_review';
          ...
          <div className="space-y-4 pt-4 border-t border-gray-100">
            ...
            {action.reasoning.potential_impact && (
              <div>
                ...
                <p className="text-sm text-gray-600">{action.reasoning.potential_impact}</p>
              </div>
            )}
            ...
            {action.reviewed_by && (
              <div className="pt-4 border-t border-gray-100">
                <h4 className="text-sm font-medium text-gray-900 mb-1">Review</h4>
                <p className="text-xs text-gray-500">
                  Reviewed on {new Date(action.reviewed_at!).toLocaleDateString()}
                </p>
                {action.reviewer_notes && (
                  <p className="text-sm text-gray-600 mt-1">{action.reviewer_notes}</p>
                )}
              </div>
            )}
