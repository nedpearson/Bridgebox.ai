...
import {
  internalRecordingsApi,
  InternalRecording,
} from "../../lib/internalRecordings";
import {
  Play,
  Trash2,
  Calendar,
...
export default function RecordingLibrary() {
  ...
  const [activeRecording, setActiveRecording] =
    useState<InternalRecording | null>(null);
  const [activeTab, setActiveTab] = useState<"standard" | "development">(
    "standard",
  );
  ...
  const handlePlay = async (recording: InternalRecording) => {
    try {
      const url = await internalRecordingsApi.getRecordingUrl(
  ...
  const handleDelete = async (recording: InternalRecording) => {
    if (
      !window.confirm(`Are you sure you want to delete "${recording.title}"?`)
    )
      return;

    try {
      await internalRecordingsApi.deleteRecording(
        recording.id,
        recording.storage_path,
      );
  ...
  if (error) {
    return (
      <div className="text-red-400 py-12 text-center bg-red-500/5 rounded-xl border border-red-500/20">
        {error}
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredRecordings.map((recording) => (
          <div
            key={recording.id}
            className="bg-slate-900 rounded-xl border border-slate-800 overflow-hidden group hover:border-slate-700 transition-colors"
          >
            ...
            <div className="p-4">
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {recording.category && (
                  <span
                    className={`flex items-center gap-1 px-2 py-1 border text-xs font-semibold rounded-md ${getCategoryColor(recording.category)}`}
                  >
                ...
                {recording.tags.map((tag) => (
                  <span
                    key={tag}
                    className="flex items-center gap-1 px-2 py-1 bg-slate-800 text-slate-300 text-xs rounded-md"
                  >
        ...
        {recordings.length === 0 && !loading && (
          <div className="col-span-full py-16 text-center border border-dashed border-slate-800 rounded-xl">
            <h3 className="text-lg font-medium text-slate-300 mb-2">
              No Internal Recordings
            </h3>
            <p className="text-slate-500">
              Capture a new session to see it here.
            </p>
          </div>
        )}
      </div>
