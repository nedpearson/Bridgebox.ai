...
export default function AiIntelligencePane({
  rawContext,
}: AiIntelligencePaneProps) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const response =
          await aiService.generateOnboardingIntelligence(rawContext);
        ...
        {!isTyping ? (
          ...
          >
            ...
            <p className="text-slate-500 max-w-sm">
              Start typing or speaking on the left. The engine will
              automatically interpret and design your system graph here in
              real-time.
            </p>
          </motion.div>
        ) : (
            ...
            {intelligence && (
              <>
                <div className="p-6 bg-slate-900 border border-slate-800 rounded-xl space-y-4">
                  <h3 className="text-emerald-400 font-medium tracking-wide text-sm uppercase">
                    Detected Architectures
                  </h3>
                  {intelligence.detected_workflows?.map(
                    (wf: any, idx: number) => (
                      <div
                        key={idx}
                  ...
                  {intelligence.suggested_features?.map(
                    (feat: any, idx: number) => (
                      <div
                        key={`feat-${idx}`}
                        className="flex items-center justify-between p-3 bg-slate-950 rounded-lg"
                ...
                {intelligence.missing_gaps &&
                  intelligence.missing_gaps.length > 0 && (
                    <div className="p-6 bg-slate-900 border border-slate-800 rounded-xl space-y-4">
                      <h3 className="text-amber-400 font-medium tracking-wide text-sm uppercase">
                        Missing Intelligence Gaps
                      </h3>
                      {intelligence.missing_gaps.map(
                        (gap: any, idx: number) => (
