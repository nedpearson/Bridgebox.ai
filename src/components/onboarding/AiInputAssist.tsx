...
export default function AiInputAssist({ 
  ...
  const handleEnhance = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
          onChange(`${value}. We operate as a distinguished, professional organization executing world-class solutions across this sector.`);
  ...
  return (
    <div className="relative group w-full">
      ...
      <div className={`absolute right-2 flex items-center space-x-1 transition-opacity ${isRecording ? 'opacity-100' : 'opacity-40 group-hover:opacity-100'} ${multiline ? 'top-3' : 'top-1/2 -translate-y-1/2'}`}>
        <button
          onClick={handleMic}
          type="button"
