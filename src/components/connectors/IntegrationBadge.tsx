...
export default function IntegrationBadge({
  ...
  return (
    <div className={`flex items-center gap-2 text-xs ${className}`}>
      <div className={`inline-flex items-center gap-1.5 px-2 py-1 rounded border ${statusColors[syncStatus]}`}>
        <Database className="w-3 h-3" />
        <span className="font-medium capitalize">{providerName.replace('_', ' ')}</span>
      </div>
