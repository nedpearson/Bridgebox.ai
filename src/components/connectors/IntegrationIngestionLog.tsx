  ...
  if (webhooks.length === 0) {
    return (
      <Card glass className="p-8 text-center text-slate-400">
        ...
        <p className="text-sm">Inbound webhooks from Make/Zapier will buffer here securely.</p>
      </Card>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-slate-400">Review AI's translation of unstructured JSON into standard system rows.</p>
        </div>
      ...
      <div className="grid gap-6">
        {webhooks.map((hook) => (
          <Card key={hook.id} glass className="p-4 border-slate-700/50 relative overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row gap-6">
              ...
              <div className="flex-1 bg-slate-950 p-4 rounded-lg border border-slate-800">
                <div className="flex items-center justify-between mb-3">
                   <span className="text-xs font-semibold text-slate-500 uppercase tracking-widest">Inbound Raw Source</span>
              ...
              <div className="flex-1 bg-indigo-950/20 p-4 rounded-lg border border-indigo-500/20">
                ...
                {hook.status === 'failed' ? (
                   <p className="text-red-400 text-xs bg-red-500/10 p-2 rounded">{hook.error_log}</p>
                ) : (
