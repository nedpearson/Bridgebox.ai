...
import { TrendingUp, Shield, HelpCircle, ArrowUpRight, ArrowDownRight } from 'lucide-react';
...
export default function IndustryBenchmarkCard() {
  ...
  const loadBenchmarkData = async () => {
    ...
    try {
      ...
      const bData = await benchmarkingService.getBenchmark(orgIndustry, 'avg_project_duration_days');
      ...
      if (projects && projects.length > 0) {
        const durations = projects.map(p => {
          ...
          return (new Date(p.end_date).getTime() - new Date(p.start_date).getTime()) / 86400000;
        });
  ...
  if (loading) return <div className="animate-pulse bg-slate-800/50 h-32 rounded-xl" />;

  const isOutperformingMedian = tenantVelocity > 0 && tenantVelocity < benchmark?.p50_value;
  const isTop10Percent = tenantVelocity > 0 && tenantVelocity <= benchmark?.p90_value;
      ...
      <div className="relative z-10 flex flex-col h-full">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="font-bold text-white tracking-tight">Ecosystem Baseline</h3>
          </div>
          <button className="text-slate-400 hover:text-white transition-colors" title="Aggregated from anonymized ecosystem data">
