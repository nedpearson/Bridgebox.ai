...
export function VoiceCaptureMini({
  transcript,
  onTranscriptChange,
  placeholder,
}: VoiceCaptureMiniProps) {
  ...
  useEffect(() => {
    accumulated.current = transcript;
  }, []);
  ...
  const startRecording = useCallback(() => {
    ...
    rec.onresult = (e: any) => {
      ...
      if (fin) {
        accumulated.current += fin;
        onTranscriptChange(accumulated.current);
      }
      setInterim(inter);
    };
    rec.onerror = (e: any) => {
    ...
    rec.onend = () => {
      if (isRecording && !isPaused) {
        try {
          ...
          /* auto-restart blocked — Chrome limit or aborted */
        }
      }
    };
    ...
    return (
      <div className="p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl">
        ...
        <p className="text-amber-400/60 text-xs mt-1">
          You can type your response in the text area below instead.
        </p>
      </div>
    );
  }
      ...
      <div className="flex items-center gap-3">
        ...
        {isRecording && !isPaused && (
          <>
            <button
              onClick={pause}
              className="flex items-center gap-2 px-4 py-2 bg-amber-500/15 hover:bg-amber-500/25 text-amber-300 border border-amber-500/25 rounded-xl text-sm font-medium transition-all"
            >
              <Pause className="w-4 h-4" /> Pause
            </button>
            <button
              onClick={stop}
              className="flex items-center gap-2 px-4 py-2 bg-red-500/15 hover:bg-red-500/25 text-red-300 border border-red-500/25 rounded-xl text-sm font-medium transition-all"
            >
        ...
        {isRecording && isPaused && (
          <>
            <button
              onClick={resume}
              className="flex items-center gap-2 px-4 py-2 bg-emerald-500/15 hover:bg-emerald-500/25 text-emerald-300 border border-emerald-500/25 rounded-xl text-sm font-medium transition-all"
            >
