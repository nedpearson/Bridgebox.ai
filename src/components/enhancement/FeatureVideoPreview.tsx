...
import { Play, Pause, Maximize2, Volume2, Settings, MousePointer2, LayoutDashboard, Users, CreditCard, Activity, ArrowRight, X } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const playSimulation = async () => {
      ...
      while (isPlaying && active) {
        ...
        await cursorControls.start({ x: -150, y: -40, transition: { duration: 1, ease: 'easeInOut' } });
        ...
        await cursorControls.start({ x: 100, y: 0, transition: { duration: 1.2, ease: 'easeInOut' } });
        await new Promise(r => setTimeout(r, 400));
        ...
        await modalControls.start({ opacity: 1, scale: 1, y: 0, zIndex: 50, transition: { duration: 0.4, type: 'spring' } });
        
        // 5. Cursor explores Modal Data
        await cursorControls.start({ x: 80, y: 60, transition: { duration: 0.8, ease: 'easeInOut' } });
        await new Promise(r => setTimeout(r, 2000));
        
        // 6. Close Drilldown Modal
        await cursorControls.start({ x: 220, y: -80, transition: { duration: 0.8, ease: 'easeInOut' } });
        await cursorControls.start({ scale: 0.8, transition: { duration: 0.1 } });
        await cursorControls.start({ scale: 1, transition: { duration: 0.1 } });
      ...
      */}
      <div className="relative w-full h-64 md:h-80 overflow-hidden bg-slate-900 border-b border-slate-800">
        ...
        {isPlaying ? (
             ...
             <div className="w-32 bg-slate-900 border-r border-slate-800 p-3 space-y-4">
               ...
               {[LayoutDashboard, Users, CreditCard, Activity].map((Icon, i) => (
                 <div key={i} className={`flex items-center gap-2 p-1.5 rounded ${i === 1 ? 'bg-indigo-500/10 text-indigo-400' : 'text-slate-500'}`}>
                    ...
                    <div className={`h-1.5 rounded w-12 ${i === 1 ? 'bg-indigo-400/50' : 'bg-slate-700'}`} />
                 </div>
               ))}
             </div>
             
             {/* SaaS Main Canvas */}
             <motion.div animate={screenControls} className="flex-1 bg-slate-950 p-4 relative">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                   <div className="h-16 bg-slate-800/50 rounded border border-indigo-500/20 shadow-[0_0_15px_rgba(99,102,241,0.1)] p-3 relative overflow-hidden group">
                      ...
                      <div className="text-[9px] text-slate-400 mb-1">Total Conversions</div>
                      <div className="text-xl font-bold text-white">42,881</div>
                   </div>
                   <div className="h-16 bg-slate-900 border border-slate-800 rounded p-3">
                      <div className="text-[9px] text-slate-400 mb-1">Active Sessions</div>
                ...
                <div className="bg-slate-900 border border-slate-800 rounded p-3 h-24">
                   ...
                   <div className="space-y-2">
                     {[1, 2, 3].map(i => (
                        <div key={i} className="flex border-b border-slate-800 pb-2">
                ...
                >
                   <div className="flex justify-between items-center mb-4">
                      <div className="text-xs font-bold text-white">Conversion Drilldown</div>
                      <X className="w-3 h-3 text-slate-400" />
                   </div>
                   <div className="flex-1 bg-slate-950 rounded border border-slate-800 p-3 flex flex-col gap-2">
                      <div className="text-[10px] text-indigo-400 font-mono">Query Executed: SELECT * FROM conversions</div>
                      <div className="flex items-center gap-2 mt-2">
                         <div className="w-6 h-6 rounded-full bg-emerald-500/20 flex items-center justify-center text-emerald-400 text-[10px] font-bold">1</div>
                         <div className="flex-1 h-3 bg-slate-800 rounded" />
                      </div>
                      <div className="flex items-center gap-2">
                         <div className="w-6 h-6 rounded-full bg-amber-500/20 flex items-center justify-center text-amber-400 text-[10px] font-bold">2</div>
                         <div className="flex-1 h-3 bg-slate-800 rounded" />
                      </div>
                   </div>
                </motion.div>
          ...
          <div className="relative z-10 flex flex-col items-center justify-center w-full h-full cursor-pointer bg-slate-900" onClick={() => setIsPlaying(true)}>
            ...
            <span className="mt-4 text-[11px] font-bold tracking-widest uppercase text-white relative z-20 bg-slate-900/50 px-3 py-1 rounded-full border border-slate-700/50">Play Full Live Video Walkthrough</span>
          </div>
        )}
      </div>
        ...
        <div className="flex items-center gap-4 w-full">
          ...
          <div className="flex items-center gap-2 group/scrub cursor-pointer flex-1">
             <span className="text-[10px] tabular-nums text-slate-500 font-mono">
                {isPlaying ? <motion.span animate={{ opacity: [1, 0.5, 1] }} transition={{ repeat: Infinity, duration: 1 }}>▶ PLAYING</motion.span> : '0:00'}
             </span>
             <div className="flex-1 h-1.5 bg-slate-800 rounded-full relative overflow-hidden group-hover:h-2 transition-all">
                <motion.div 
                  initial={{ width: '0%' }}
                  animate={{ width: isPlaying ? '100%' : '0%' }}
                  transition={{ duration: 7.2, ease: 'linear', repeat: isPlaying ? Infinity : 0 }}
                  className="absolute left-0 top-0 bottom-0 bg-indigo-500 rounded-full" 
                />
             </div>
             <span className="text-[10px] tabular-nums text-slate-500 font-mono uppercase">Live Link</span>
          </div>
