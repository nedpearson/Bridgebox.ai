...
const CONTEXT_MODE_DESCRIPTIONS: Record<VoiceContextMode, string> = {
  describe_current_software:
    "Tell us about the software you use today — what it does, how you use it.",
  ...
  describe_final_vision:
    "Paint the picture of your ideal system — don't hold back.",
  describe_changes: "Explain what you want changed, improved, or removed.",
  describe_pain_points:
    "Tell us what's broken, slow, or frustrating about current tools.",
  free_form:
    "Speak freely — say whatever's on your mind about your software needs.",
};
...
export default function SpeakYourAppModal({
  isOpen,
  onClose,
  onCreated,
}: SpeakYourAppModalProps) {
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-slate-400 text-xs">
                    Voice-to-App capture for {currentOrganization?.name}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="flex items-center gap-2 px-6 pt-4 pb-0">
              {(["mode", "capture", "submit", "done"] as Step[]).map((s, i) => (
                <div key={s} className="flex items-center gap-2">
                  <div
                    className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold border transition-all ${
                      step === s
                        ...
                        : ["mode", "capture", "submit", "done"].indexOf(step) >
                            i
                          ? "bg-emerald-500/20 border-emerald-500 text-emerald-400"
                          : "bg-slate-800 border-slate-700 text-slate-500"
                    }`}
                  >
                    {["mode", "capture", "submit", "done"].indexOf(step) > i ? (
                  ...
                  {i < 3 && (
                    <div
                      className={`h-px w-6 ${["mode", "capture", "submit", "done"].indexOf(step) > i ? "bg-emerald-500/50" : "bg-slate-700"}`}
                    />
                  )}
                </div>
              ))}
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-slate-400 text-sm">
                      Choose a capture mode. Each mode prompts deeper, more
                      useful output.
                    </p>
                  </div>
                  <div className="grid grid-cols-1 gap-2">
                    {ORDERED_MODES.map((mode) => (
                      ...
                      >
                        <div
                          className={`w-3.5 h-3.5 rounded-full border-2 mt-0.5 flex-shrink-0 transition-all ${contextMode === mode ? "border-indigo-400 bg-indigo-400" : "border-slate-600"}`}
                        />
                        <div>
                          <p className="font-medium text-sm">
                            {VOICE_CONTEXT_LABELS[mode]}
                          </p>
                          <p className="text-xs text-slate-400 mt-0.5">
                            {CONTEXT_MODE_DESCRIPTIONS[mode]}
                          </p>
                        </div>
                      </button>
                    ))}
              ...
              {step === "capture" && (
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-slate-400 text-sm">
                      Press the mic and speak naturally. You can pause, resume,
                      and edit afterwards.
                    </p>
                  </div>
                  <VoiceCapturePanel
              ...
              {step === "submit" && (
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-slate-400 text-sm">
                      Edit if needed, then submit for analysis.
                    </p>
                  </div>
                  <textarea
                    value={transcript}
                ...
                <div className="text-center py-6 space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-slate-400 text-sm mt-2">
                      Your voice request has been captured, analyzed, and
                      attached to{" "}
                      <strong className="text-white">
                        {currentOrganization?.name}
                      </strong>
                      . Review it in the Enhancement Queue.
                    </p>
                  </div>
