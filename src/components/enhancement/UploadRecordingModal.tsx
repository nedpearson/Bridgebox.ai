  ...
  const handlePreSubmit = () => {
    ...
    const totalCostRequired = files.reduce((acc, pf) => {
      ...
      const sizeMultiplier = Math.max(
        1,
        Math.ceil(pf.file.size / (200 * 1024 * 1024)),
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const inputMethod = files.some((f) => ALLOWED_VIDEO.includes(f.file.type))
        ? files.some((f) => ALLOWED_IMAGE.includes(f.file.type))
          ? "mixed"
          : "recording"
        : "screenshot";
      ...
      const uploadPromises = files.map(async (pf) => {
        ...
        try {
          ...
          await enhancementRequestsService.incrementMediaCount(
            request.id,
            currentOrganization.id,
          );
      ...
      const { buildEnhancementRecommendations } =
        await import("../../lib/enhancement/analysisEngine");
