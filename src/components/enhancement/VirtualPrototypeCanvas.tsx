  ...
  const renderDashboard = () => (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Active Shipments', value: '1,248', desc: '+12% this week', icon: Users, color: 'text-blue-400', bg: 'bg-blue-500/10' },
          { label: 'Global Revenue', value: '$84,290', desc: 'Upward trend', icon: BarChart3, color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
          { label: 'Current Fleet', value: '42 Active', desc: '3 maintenance', icon: LayoutDashboard, color: 'text-purple-400', bg: 'bg-purple-500/10' },
          { label: 'System Alerts', value: '2 Warnings', desc: 'Check logs', icon: Bell, color: 'text-red-400', bg: 'bg-red-500/10' }
        ].map((kpi, i) => (
          <motion.div 
            key={i}
          ...
          >
            ...
            <div className={`w-8 h-8 rounded-lg ${kpi.bg} mb-3 flex items-center justify-center relative z-10`}>
              <kpi.icon className={`w-4 h-4 ${kpi.color}`} />
            </div>
            <div className="text-xs text-slate-400 font-medium mb-1 relative z-10 uppercase tracking-wider">{kpi.label}</div>
            <div className="text-2xl text-white font-bold relative z-10 font-mono tracking-tight">{kpi.value}</div>
            <div className="text-[10px] text-slate-500 mt-1 relative z-10">{kpi.desc}</div>
          </motion.div>
        ))}
      </div>
      {/* Charts area */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        >
          <div className="text-xs font-bold text-slate-300 uppercase tracking-wider mb-2 border-b border-slate-700 pb-2">Recent Fleet Activity</div>
          {[
             { name: 'Shipment #1042', stat: 'Manifest Replicated', time: '2m' },
             { name: 'Route Optimization', stat: 'Calculation complete', time: '14m' },
            ...
            <div key={i} className="flex items-center gap-3">
               ...
               <div className="flex-1 min-w-0">
                 ...
                 <div className="text-[10px] text-slate-400 truncate">{item.stat}</div>
               </div>
               <div className="text-[10px] text-indigo-300 font-mono flex-shrink-0">{item.time}</div>
            </div>
          ))}
        </motion.div>
      </div>
              ...
              >
                ...
                {idx === 1 && cardIdx === 0 && (
                   <div className="absolute inset-0 bg-indigo-500/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-[1px]">
                      <span className="bg-indigo-500 text-white text-[10px] font-bold px-2 py-0.5 rounded shadow flex items-center gap-1.5">
                         <MousePointerClick className="w-3 h-3" /> View Requirements
                      </span>
                   </div>
                )}
                <div className="flex flex-wrap gap-1 mb-2 relative z-10">
                   <span className="text-[10px] bg-emerald-500/10 text-emerald-400 px-1.5 py-0.5 rounded border border-emerald-500/20 font-bold uppercase">Client Dev</span>
                   {cardIdx % 2 === 0 && <span className="text-[10px] bg-amber-500/10 text-amber-400 px-1.5 py-0.5 rounded border border-amber-500/20 font-bold uppercase">UI Clone</span>}
                </div>
                <div className="text-xs text-white font-bold mb-1 relative z-10 leading-tight">Implement Servicargo Clone Protocol #{idx}{cardIdx}</div>
                <div className="text-[10px] text-slate-400 mb-3 relative z-10 line-clamp-2">Ensure exactly replicated logic from original footage provided by client.</div>
                ...
                <div className="flex items-center justify-between mt-auto relative z-10 border-t border-slate-700/50 pt-2">
                   <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[10px] text-slate-400 font-mono">2/5</span>
                   </div>
                   <div className={`text-[10px] px-2 py-0.5 rounded-full font-bold border transition-colors ${idx === 1 && cardIdx === 0 ? 'bg-indigo-500/20 text-indigo-300 border-indigo-500/30' : 'bg-slate-800 text-slate-400 border-slate-700 group-hover:bg-indigo-500/10 group-hover:text-indigo-300'}`}>
                      Ned Pearson
                   </div>
                </div>
              </motion.div>
    ...
    <div className="flex h-[400px]">
           ...
           {['User Database', 'Delivery Endpoints', 'Payment Gateway'].map((label, i) => (
             ...
             >
               ...
               <div className="flex items-center gap-2 relative z-10 w-full">
                  <Database className={`w-3.5 h-3.5 ${i === 1 ? 'text-indigo-400' : 'text-slate-400'}`} />
                  <span className={`text-[11px] font-medium flex-1 ${i === 1 ? 'text-indigo-100' : 'text-slate-300'}`}>{label}</span>
                  {i === 1 && <span className="text-[9px] bg-emerald-500/20 text-emerald-300 px-1 py-0.5 rounded border border-emerald-500/30">ACTIVE</span>}
               </div>
             </motion.div>
           ))}
       </div>
       ...
       <div className="flex-1 p-6 space-y-6 flex flex-col">
         ...
         <div className="space-y-4 flex-1 overflow-y-auto pr-2">
           {[
             { title: 'GPS Tracking Integration', desc: 'Real-time latitude/longitude mapping system', stat: 'Completed' },
             { title: 'Stripe Invoice Generation', desc: 'Automate PDF exports natively in browser', stat: 'Pending' },
             { title: 'Push Notification Subsystem', desc: 'Firebase websocket relay connected', stat: 'Testing' }
           ].map((item, i) => (
             <motion.div 
               key={i}
             ...
             >
               <div className="w-10 h-10 rounded-full bg-slate-700 border border-slate-600 flex items-center justify-center shrink-0">
                  <CheckSquare className={`w-4 h-4 ${i === 0 ? 'text-emerald-400' : 'text-slate-400'}`} />
               </div>
               <div className="flex-1 min-w-0 flex flex-col justify-center">
                 <div className="flex justify-between items-start mb-1">
                    <span className="text-sm font-bold text-white tracking-tight">{item.title}</span>
                    <span className={`text-[9px] px-1.5 py-0.5 rounded uppercase font-bold tracking-wider border ${i === 0 ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : i === 1 ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' : 'bg-blue-500/10 text-blue-400 border-blue-500/20'}`}>{item.stat}</span>
                 </div>
