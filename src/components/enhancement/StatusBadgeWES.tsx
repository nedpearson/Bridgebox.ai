...
const STATUS_STYLES: Record<EnhancementStatus, string> = {
  ...
  analyzing:
    "bg-yellow-500/15 text-yellow-300 border-yellow-500/30 animate-pulse",
...
export function StatusBadgeWES({ status, size = "sm" }: StatusBadgeWESProps) {
  ...
  return (
    <span
      className={`inline-flex items-center font-semibold rounded-full border ${sizeClass} ${STATUS_STYLES[status]}`}
    >
...
export function RequestTypeBadge({ type, size = "sm" }: RequestTypeBadgeProps) {
  ...
  return (
    <span
      className={`inline-flex items-center font-medium rounded-full border ${sizeClass} ${REQUEST_TYPE_STYLES[type]}`}
    >
