...
export default function MergeWorkspaceModal({
  isOpen,
  onClose,
  onComplete,
  ...
  const handleLoadAssets = useCallback(async () => {
    ...
    try {
      const assets =
        await workspaceTransferService.getAvailableAssets(sourceWorkspaceId);
  ...
  const handleCreateAndPreview = useCallback(async () => {
    ...
    try {
      ...
      // Preview
      const previewData = await workspaceTransferService.previewBatch(
        newBatch.id,
      );
      setBatch(newBatch);
  ...
  const handleApply = useCallback(async () => {
    ...
    setError("");
    try {
      const result = await workspaceTransferService.applyBatch(
        batch.id,
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-white font-bold text-lg">
                  Merge from Workspace
                </h2>
                <p className="text-slate-400 text-xs">
                  Transfer approved assets → {currentOrganization?.name}
                </p>
              </div>
            </div>
            <button
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-slate-400 text-sm">
                    Choose which workspace to pull approved enhancements from.
                    Only non-sensitive assets will be available.
                  </p>
                </div>
                ...
                {otherOrgs.length === 0 ? (
                  <div className="p-6 text-center bg-slate-800/40 border border-slate-700 rounded-xl">
                    ...
                    <p className="text-slate-500 text-xs mt-1">
                      You need access to at least two workspaces to use this
                      feature.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {otherOrgs.map((org) => (
                      ...
                      >
                        <div
                          className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${sourceWorkspaceId === org.id ? "bg-amber-500/20" : "bg-slate-700"}`}
                        >
                          <Building2
                            className={`w-4 h-4 ${sourceWorkspaceId === org.id ? "text-amber-400" : "text-slate-400"}`}
                          />
                        </div>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-slate-400 text-xs mt-0.5">
                        Only approved enhancement recommendations are
                        transferable. No client records, documents, or sensitive
                        data will be listed.
                      </p>
                    </div>
                  </div>
                ...
                >
                  {loadingAssets ? (
                    ...
                    <>
                      Load Available Assets <ChevronRight className="w-4 h-4" />
                    </>
                  )}
                </button>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-slate-400 text-sm mt-1">
                    From <span className="text-white">{sourceOrg?.name}</span> →{" "}
                ...
                {availableAssets.length === 0 ? (
                  <div className="p-6 text-center bg-slate-800/40 border border-slate-700 rounded-xl">
                    ...
                    <p className="text-slate-500 text-xs mt-1">
                      Approved enhancements from that workspace will appear
                      here.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    <p className="text-xs text-slate-500">
                      {selectedAssetIds.size} of {availableAssets.length}{" "}
                      selected
                    </p>
                    {availableAssets.map((asset) => (
                      ...
                      >
                        <div
                          className={`w-4 h-4 rounded border-2 flex-shrink-0 mt-0.5 transition-all ${selectedAssetIds.has(asset.id) ? "bg-indigo-500 border-indigo-500" : "border-slate-600"}`}
                        >
                          {selectedAssetIds.has(asset.id) && (
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className="px-4 py-2.5 border border-slate-700 text-slate-300 hover:text-white rounded-xl text-sm font-medium transition-all"
                  >
                    ← Back
                  </button>
                  <button
                  ...
                  >
                    Preview Merge ({selectedAssetIds.size}){" "}
            ...
            {step === "preview" && preview && (
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {preview.items.map((item, i) => (
                    ...
                    >
                      ...
                      {item.has_conflict && (
                        <span className="text-xs bg-yellow-500/15 text-yellow-300 border border-yellow-500/30 px-2 py-0.5 rounded-full">
                          Conflict
                        </span>
                      )}
                    </div>
                ...
                <div className="p-3 bg-slate-800/40 border border-slate-700/40 rounded-xl text-xs text-slate-400">
                  <p>
                    This action will clone the selected assets into{" "}
                    <strong className="text-white">
                      {currentOrganization?.name}
                    </strong>
                    . Original data in the source workspace is not affected.
                    This action is audited and traceable.
                  </p>
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  >
                    {applying ? (
                      ...
                      <>
                        <GitMerge className="w-4 h-4" /> Apply Merge
                      </>
                    )}
                  </button>
                </div>
              ...
              <div className="text-center py-6 space-y-4">
                ...
                <div>
                  ...
                  <p className="text-slate-400 text-sm mt-2">
                    ...
                    asset{applyResult.applied !== 1 ? "s" : ""} transferred
                    successfully.
                ...
                <button
                  onClick={handleClose}
                  className="px-6 py-2.5 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white rounded-xl text-sm font-medium transition-all"
                >
                  Done
                </button>
              </div>
