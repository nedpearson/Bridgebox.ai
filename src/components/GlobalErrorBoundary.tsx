...
export class GlobalErrorBoundary extends Component<Props, State> {
  ...
  public componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    Logger.error("Unhandled React Rendering Exception Caught", {
      error,
      errorInfo,
    });
  }
        ...
        <div className="min-h-screen bg-slate-950 flex flex-col justify-center items-center p-4 selection:bg-indigo-500/30">
          <div className="w-full max-w-lg bg-slate-900/80 backdrop-blur-xl border border-rose-500/20 rounded-3xl p-8 relative overflow-hidden isolate">
            ...
            <p className="text-slate-400 mb-8 leading-relaxed">
              The Bridgebox OS encountered a critical unhandled rendering
              exception. Our engineers have been securely notified via the
              telemetry layer.
            </p>

            <div className="bg-slate-950 border border-rose-500/10 rounded-xl p-4 mb-8 overflow-x-auto">
              <p className="text-xs font-mono text-rose-400 font-bold mb-2">
                Exception Trace:
              </p>
              <code className="text-[10px] text-slate-500 block w-full whitespace-pre-wrap">
                {this.state.error?.message ||
                  "Unknown fatal error in React Tree"}
              </code>
            </div>
