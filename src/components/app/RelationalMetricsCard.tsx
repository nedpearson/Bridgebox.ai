...
import { Network, CheckCircle2, FileText, MessageSquare, GitMerge } from 'lucide-react';
...
export default function RelationalMetricsCard({ entityType, entityId }: RelationalMetricsCardProps) {
  ...
  const loadMetrics = async () => {
    try {
      setLoading(true);
      const data = await entityLinkService.getEntityLinkCounts(entityType, entityId);
      setCounts(data);
        ...
        {metrics.map((m, i) => {
          ...
          return (
            ...
            >
               ...
               <span className="text-xs text-slate-400 font-medium uppercase tracking-wider mt-1">{m.label}</span>
            </Link>
          );
        })}
      </div>
