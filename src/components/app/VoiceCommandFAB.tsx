...
export default function VoiceCommandFAB() {
  ...
  const processVoiceCommand = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        setSuccessMsg(
          executionSummary || "Processed audio but found no actionable items.",
        );
      } else {
  ...
  return (
    <>
      ...
      >
        <Mic
          className={`w-6 h-6 text-white ${isRecording ? "animate-bounce" : ""}`}
        />
      </button>
            ...
            <Card glass className="p-4 shadow-2xl border-indigo-500/30">
              ...
              <div className="min-h-[60px] max-h-40 overflow-y-auto mb-3">
                {isRecording && !transcript && (
                  <p className="text-sm text-slate-400 animate-pulse italic">
                    Listening closely...
                  </p>
                )}
                {transcript && (
