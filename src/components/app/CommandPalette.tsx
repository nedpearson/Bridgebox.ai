...
import { Search, Loader2, Box, CheckCircle2, Plus, Zap, Link as LinkIcon, FileText, Command, ChevronRight, Sparkles, Building2, Folder, CheckSquare } from 'lucide-react';
...
export default function CommandPalette() {
  ...
  let quickActions = [
    { id: 'create-task', title: 'Create Global Task', subtitle: 'Add a new task to the system', icon: <Plus className="w-5 h-5 text-emerald-400" />, action: () => navigate('/app/tasks?new=true') },
    { id: 'create-project', title: 'Create Project', subtitle: 'Spin up a new client project', icon: <Plus className="w-5 h-5 text-blue-400" />, action: () => navigate('/app/projects?new=true') },
    { id: 'stack-discovery', title: 'Map Tech Stack', subtitle: 'Visually architect external tool integrations using AI.', icon: <Sparkles className="w-5 h-5 text-cyan-400" />, action: () => navigate('/app/stack-discovery') },
    { id: 'create-client', title: 'Add Client Context', subtitle: 'Register a new organization', icon: <Plus className="w-5 h-5 text-purple-400" />, action: () => navigate('/app/clients?new=true') },
    { id: 'log-interaction', title: 'Log Activity', subtitle: 'Record an email, call, or meeting', icon: <Zap className="w-5 h-5 text-amber-400" />, action: () => navigate('/app/communications?new=true') },
  ];
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case 'project': return <Folder className="w-5 h-5 text-emerald-400" />;
      case 'task': return <CheckSquare className="w-5 h-5 text-amber-400" />;
      case 'document': return <FileText className="w-5 h-5 text-rose-400" />;
      case 'onboarding': return <Sparkles className="w-5 h-5 text-cyan-400" />;
          ...
          <div className="relative w-full max-w-2xl bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            {query.length > 5 && results.length === 0 && !loading && (
              <div className="px-4 py-8 text-center text-slate-400">
                ...
                <p className="text-sm">Cannot find exact relational match in your stack.</p>
                <button 
                   onClick={() => { setIsOpen(false); navigate('/app/tasks?new=true'); }}
              ...
              {loading ? (
                ...
                results.length > 0 ? (
                  <div className="p-2 space-y-1">
                    <div className="px-3 pb-2 pt-1 text-xs font-semibold text-slate-500 uppercase tracking-wider">Relational Matches</div>
                    {results.map((result, idx) => (
                      <button
                        ...
                        className={`w-full flex items-center justify-between p-3 rounded-xl transition-colors group text-left ${
                          idx === selectedIndex ? 'bg-indigo-500/10 border border-indigo-500/30' : 'hover:bg-slate-800 border border-transparent'
                        }`}
                      >
                        <div className="flex items-center space-x-4">
                          <div className={`p-2 rounded-lg transition-colors border ${idx === selectedIndex ? 'bg-indigo-500/20 border-indigo-500/30' : 'bg-slate-950 border-slate-800'}`}>
                            {getIcon(result.type)}
                          </div>
                          <div>
                            <h4 className={`font-medium text-sm transition-colors ${idx === selectedIndex ? 'text-indigo-400' : 'text-white'}`}>{result.title}</h4>
                            {result.subtitle && <p className="text-xs text-slate-400">{result.subtitle}</p>}
                          </div>
                        </div>
                        <ChevronRight className={`w-4 h-4 transition-colors ${idx === selectedIndex ? 'text-indigo-400' : 'text-slate-600 group-hover:text-slate-400'}`} />
                      </button>
                    ))}
                  </div>
                  ...
                  {quickActions.map((action, idx) => (
                    ...
                    >
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          <h4 className={`font-medium text-sm transition-colors ${idx === selectedIndex ? 'text-indigo-400' : 'text-white'}`}>{action.title}</h4>
                          {action.subtitle && <p className="text-xs text-slate-400">{action.subtitle}</p>}
                        </div>
                      </div>
                      <span className="text-[10px] text-slate-500 px-2 py-0.5 border border-slate-800 rounded bg-slate-900 shadow-sm opacity-0 group-hover:opacity-100 transition-opacity">Select</span>
                    </button>
                  ))}
                </div>
              )}
            ...
            <div className="px-4 py-3 bg-slate-950/50 border-t border-slate-800 flex items-center justify-between text-xs text-slate-500">
              <div className="flex items-center space-x-4">
                 <span className="flex items-center font-medium"><Command className="w-3 h-3 mr-1"/> Navigate</span>
                 <span className="flex items-center font-medium">↵ Execute</span>
                 <span className="flex items-center font-medium">esc Dismiss</span>
              </div>
              <span className="flex items-center font-semibold text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-cyan-400">
                <Sparkles className="w-3 h-3 mr-1 text-indigo-400"/> AI Operating System Base
              </span>
            </div>
