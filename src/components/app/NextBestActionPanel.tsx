...
import { Lightbulb, ArrowRight, Zap, PlayCircle, AlertTriangle, Plus, Sparkles, Loader2 } from 'lucide-react';
...
export default function NextBestActionPanel({ entityType, entityData }: NextBestActionPanelProps) {
  const navigate = useNavigate();
  const [aiRecommendation, setAiRecommendation] = useState<ActionRecommendation | null>(null);
  ...
  useEffect(() => {
    ...
    async function fetchAiRecommendation() {
      ...
      setLoadingAi(true);
      try {
        const { data, error } = await supabase.functions.invoke('predict-next-action', {
           body: { record_id: entityData.id, record_type: entityType, context_data: entityData }
        });
        ...
        if (!error && data?.nba && active) {
          ...
          setAiRecommendation({
             ...
             action: () => {
                if (mlb.proposed_tool_execution?.tool_name && mlb.proposed_tool_execution?.tool_name !== 'null' && mlb.proposed_tool_execution?.tool_name !== 'none' && (window as any).executeCopilotTool) {
      ...
      case 'organization':
        if (entityData.status === 'lead') {
          recs.push({
            ...
            description: 'This organization is still a lead. Log a call or schedule a discovery meeting.',
            icon: <PlayCircle className="w-5 h-5 text-indigo-400" />,
            action: () => navigate(`/app/communications?new=true&target=${entityData.id}&type=organization`),
            priority: 'high'
          });
        }
        if (!entityData.mrr || entityData.mrr === 0) {
          recs.push({
            ...
            description: 'No active revenue mapped. Create an onboarding project or kick off a new engagement.',
            icon: <Plus className="w-5 h-5 text-emerald-400" />,
            action: () => navigate(`/app/projects?new=true&client=${entityData.id}`),
            priority: 'medium'
          });
        }
        break;
      ...
      case 'project':
        if (entityData.status === 'planning') {
          recs.push({
            ...
            description: 'Project is in planning phase. Generate initial tasks and move to active execution.',
            icon: <PlayCircle className="w-5 h-5 text-blue-400" />,
            action: () => navigate(`/app/tasks?new=true&project=${entityData.id}`),
            priority: 'high'
          });
        }
        if (!entityData.budget || entityData.budget === 0) {
          recs.push({
            ...
            description: 'Project lacks financial tracking. Update configuration to forecast resource run-rate.',
      ...
      case 'task':
        if (entityData.status === 'todo') {
          recs.push({
            ...
            description: 'This task is waiting to be started. Move it to In Progress to signal activity.',
            icon: <PlayCircle className="w-5 h-5 text-indigo-400" />,
            action: () => {}, // Triggers status inline edit mapped externally
            priority: 'high'
          });
        }
        if (new Date(entityData.due_date) < new Date() && entityData.status !== 'done') {
           recs.push({
            id: 'nba_escalate_task',
            title: 'Escalate Overdue Task',
            description: 'Task is past its deadline. Log a communication to unblock dependents.',
            icon: <AlertTriangle className="w-5 h-5 text-red-400" />,
            action: () => navigate(`/app/communications?new=true&target=${entityData.id}&type=task`),
            priority: 'high'
          });
        }
        break;
      ...
      case 'document':
        if (!entityData.is_processed) {
           recs.push({
            ...
            description: 'Document has not been processed. Trigger intelligence extraction for search parsing.',
      ...
      case 'communication':
        if (entityData.direction === 'inbound') {
           recs.push({
            ...
            description: 'This was an inbound interaction. Ensure a follow-up action is scheduled.',
            icon: <Plus className="w-5 h-5 text-emerald-400" />,
  ...
  const finalRecommendations = useMemo(() => {
     if (aiRecommendation) {
        return [aiRecommendation, ...fallbackRecommendations.filter(r => r.priority === 'high').slice(0, 1)];
     }
  ...
  return (
    <Card glass className="p-6">
      <div className="flex items-center justify-between mb-6">
         ...
         {loadingAi && <Loader2 className="w-4 h-4 text-purple-400 animate-spin" />}
      </div>
        ...
        {finalRecommendations.map((rec) => (
          <div 
            ...
            className={`flex items-center p-4 rounded-xl border bg-slate-800/40 hover:bg-slate-700/60 transition-colors cursor-pointer group ${
              rec.priority === 'high' ? 'border-amber-500/20' : 'border-slate-700'
            }`}
          >
            ...
            <div className="flex-1">
              <h4 className="text-sm font-semibold text-white group-hover:text-indigo-400 transition-colors">{rec.title}</h4>
