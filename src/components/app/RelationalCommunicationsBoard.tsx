...
export default function RelationalCommunicationsBoard({ entityType, entityId }: RelationalCommunicationsBoardProps) {
  const { currentOrganization } = useAuth();
  const [comms, setComms] = useState<(GlobalCommunication & { author?: any })[]>([]);
  ...
  const loadComms = async () => {
    try {
      setLoading(true);
      const data = await globalCommunicationsService.getLinkedCommunications(entityType, entityId);
      setComms(data);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-white">Interaction Intelligence</h3>
        <button
      ...
      {comms.length === 0 ? (
        <div className="bg-white/5 border border-white/10 rounded-xl p-12 text-center flex flex-col items-center">
          ...
          <p className="text-slate-400 font-medium mb-1">No interactions logged.</p>
          <p className="text-slate-500 text-sm">Log calls, emails, and meetings to maintain relationship context.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {comms.map(comm => (
            <div key={comm.id} className="bg-slate-800/30 border border-slate-700/50 p-4 rounded-xl hover:border-indigo-500/30 transition-colors">
               <div className="flex items-start justify-between">
                  <div className="flex items-start space-x-4">
                     <div className="p-2 bg-slate-900 rounded-lg">
                       {comm.channel === 'email' && <Mail className="w-5 h-5 text-indigo-400" />}
                       {comm.channel === 'call' && <Phone className="w-5 h-5 text-indigo-400" />}
                       {comm.channel === 'message' && <MessageSquare className="w-5 h-5 text-indigo-400" />}
                     </div>
                     <div>
                       ...
                       <p className="text-sm text-slate-400 line-clamp-2 mt-1">{comm.content}</p>
                       <div className="flex items-center space-x-3 mt-3 text-xs text-slate-500">
                         <span className={`capitalize px-2 py-0.5 rounded border ${
                            comm.direction === 'inbound' ? 'border-green-500/30 text-green-400 bg-green-500/10' :
                  ...
                  <Link to={`/app/communications/${comm.id}`} className="p-2 hover:bg-white/5 rounded-lg text-slate-500 hover:text-white transition-colors">
