...
export default function AppSidebar() {
  ...
  useEffect(() => {
     ...
     const loadBadge = async () => {
        const { count } = await supabase.from('bb_agent_actions_queue')
     ...
     const sub = supabase.channel('agent_queue_changes')
       .on('postgres_changes', { event: '*', schema: 'public', table: 'bb_agent_actions_queue', filter: `organization_id=eq.${currentOrganization.id}` }, () => {
          loadBadge();
       }).subscribe();
  ...
  const topNav: NavItem[] = [
    ...
    { icon: Users, label: 'Customers / Clients', path: '/app/clients', requirePermission: permissions.canManageClients },
    { icon: Rocket, label: 'Operations', path: '/app/delivery', requirePermission: permissions.canAccessAdminPanel },
    { icon: LayoutTemplate, label: 'Templates', path: '/app/marketplace', requirePermission: permissions.canAccessAdminPanel },
