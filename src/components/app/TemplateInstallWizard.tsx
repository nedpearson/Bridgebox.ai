...
export default function TemplateInstallWizard({
  isOpen,
  onClose,
  template,
  onSuccess,
  ...
  const [enableStaffApp, setEnableStaffApp] = useState(
    mobileConfig?.staff_app?.enabled ?? false,
  );
  const [enableCustomerApp, setEnableCustomerApp] = useState(
    mobileConfig?.customer_app?.enabled ?? false,
  );
  const [enableOfflineSync, setEnableOfflineSync] = useState(
    mobileConfig?.offline_sync_enabled ?? false,
  );
  ...
  const handleInstall = async () => {
    ...
    try {
      ...
      setError(
        err.message || "Failed to install template. Please contact support.",
      );
                ...
                <div className="mb-6 space-y-3">
                  ...
                  <label className="flex items-start space-x-3 p-3 rounded-lg border border-slate-700/50 bg-slate-800/30 cursor-pointer hover:bg-slate-800 transition-colors">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-1">
                        Provisions views:{" "}
                        {mobileConfig.staff_app?.views?.join(", ")}
                      </p>
                    </div>
                  </label>

                  {mobileConfig.customer_app &&
                    mobileConfig.customer_app.views?.length > 0 && (
                      <label className="flex items-start space-x-3 p-3 rounded-lg border border-slate-700/50 bg-slate-800/30 cursor-pointer hover:bg-slate-800 transition-colors">
                        <input
                          type="checkbox"
                          checked={enableCustomerApp}
                          onChange={(e) =>
                            setEnableCustomerApp(e.target.checked)
                          }
                          className="mt-1 w-4 h-4 rounded border-slate-600 text-indigo-500 focus:ring-indigo-500 bg-slate-900"
                        />
                        <div>
                          ...
                          <p className="text-xs text-slate-400 mt-1">
                            Client-facing PWA with auth wall
                          </p>
                        </div>
                      </label>
                    )}
                  ...
                  <label className="flex items-start space-x-3 p-3 rounded-lg border border-slate-700/50 bg-slate-800/30 cursor-pointer hover:bg-slate-800 transition-colors">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-1">
                        Caches mutations locally when reception drops
                      </p>
                    </div>
                  </label>
                </div>
          ...
          {step === 2 && (
            <div className="p-12 text-center">
              ...
              <p className="text-indigo-200">
                Bridgebox is provisioning your entities and schemas.
              </p>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="p-12 text-center">
              ...
              <p className="text-slate-300 mb-8">
                The {template.name} blueprint has been successfully mapped to
                your workspace.
              </p>
              <Button
                onClick={onClose}
