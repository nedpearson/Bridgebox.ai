...
import { ShieldAlert, AlertCircle, ArrowRight, CheckCircle } from 'lucide-react';
...
export default function BlockersPanel({ entityType, entityId }: BlockersPanelProps) {
  ...
  const loadBlockers = async () => {
    try {
      ...
      if (links.length > 0) {
        const taskIds = links.map(l => l.source_type === 'task' ? l.source_id : l.target_id);
        ...
        const tasks = await globalTasksService.getLinkedTasks(entityType, entityId);
    ...
    return (
      <Card glass className="p-4 border-emerald-500/20 bg-emerald-500/5">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-white">Topology is Unblocked</h3>
            <p className="text-xs text-emerald-400/80 mt-0.5">0 critical dependencies detected.</p>
          </div>
        </div>
      </Card>
    );
  ...
  return (
    <Card glass className="p-6 border-red-500/20 shadow-[0_0_15px_rgba(239,68,68,0.1)]">
        ...
        {blockers.map((task) => (
          ...
          >
            ...
            <div className="flex-1">
              <h4 className="text-sm font-medium text-white group-hover:text-red-400 transition-colors">{task.title}</h4>
              <p className="text-xs text-red-400/80 mt-1">This topological dependency is overdue. Target resolving this immediately.</p>
            </div>
