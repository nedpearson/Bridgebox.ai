    ...
    const searchTimer = setTimeout(async () => {
      if (query.length >= 2 && currentOrganization?.id) {
        ...
        try {
          ...
          let data = await globalSearchService.search(
            currentOrganization.id,
            query,
          );
          data = data.filter(
            (d) => !(d.type === entityType && d.id === entityId),
          );
          setResults(data);
        } catch (err) {
  ...
  const handleBind = async (target: SearchResult) => {
    ...
    try {
      ...
      if (err.code === "23505") {
        ...
        setError(
          err.message || "Failed to establish relationship matrix constraint.",
        );
      }
    } finally {
      setSubmitting(false);
          ...
          <div className="flex items-center justify-between p-6 border-b border-slate-800 shrink-0">
            <div>
              ...
              <p className="text-sm text-slate-400 mt-1 capitalize">
                Expand topology for {entityType}
              </p>
            </div>
            <button
              onClick={onClose}
            ...
            {query.length < 2 ? (
              <div className="text-center py-10">
                ...
                <p className="text-slate-400">
                  Type to search the universal index...
                </p>
              </div>
            ) : results.length > 0 ? (
