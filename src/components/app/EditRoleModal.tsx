...
export default function EditRoleModal({
  ...
  const [formData, setFormData] = useState({
    ...
    permissions: {} as Record<
      string,
      { view?: boolean; update?: boolean; create?: boolean; delete?: boolean }
    >,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await whiteLabelService.updateCustomRole(role.id, {
        name: roleName,
          ...
          >
            ...
            <div>
              <h3 className="text-sm font-medium text-white mb-3">
                Permissions Matrix (Drilldown)
              </h3>
              <div className="bg-slate-800/50 border border-slate-700 rounded-lg overflow-hidden">
                <table className="w-full text-left">
                  <thead>
                    <tr className="border-b border-slate-700 bg-slate-800">
                      <th className="p-3 text-sm font-medium text-slate-300">
                        Module
                      </th>
                      <th className="p-3 text-sm flex-1 text-center font-medium text-slate-300">
                        View
                      </th>
                      <th className="p-3 text-sm flex-1 text-center font-medium text-slate-300">
                        Edit
