...
export default function OrganizationSwitcher() {
  const { organizations, currentOrganization, setCurrentOrganization, can, profile } = useAuth();
  const [isOpen, setIsOpen] = useState(false);
  const [isCreating, setIsCreating] = useState(false);
  const [newOrgName, setNewOrgName] = useState('');

  const isSuperAdmin = profile?.role === 'super_admin' && profile?.email?.toLowerCase() === 'nedpearson@gmail.com';
                    ...
                    {!isCreating ? (
                      ...
                      <form 
                        onSubmit={async (e) => {
                          ...
                          try {
                            await organizationsService.createOrganization({ name: newOrgName, type: 'internal' });
