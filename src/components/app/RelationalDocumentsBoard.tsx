...
export default function RelationalDocumentsBoard({ entityType, entityId }: RelationalDocumentsBoardProps) {
  ...
  const loadDocs = async () => {
    try {
      setLoading(true);
      const data = await documentService.getLinkedDocuments(entityType, entityId);
      setDocuments(data);
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      {documents.length === 0 ? (
        <div className="bg-white/5 border border-white/10 rounded-xl p-12 text-center flex flex-col items-center">
          ...
          <p className="text-slate-400 font-medium mb-1">No operational files mapped.</p>
          <p className="text-slate-500 text-sm">Any files uploaded here will be pinned strictly to this {entityType} node.</p>
        </div>
      ) : (
        <div className="grid lg:grid-cols-2 gap-4">
          {documents.map(doc => (
            <div key={doc.id} className="bg-slate-800/30 border border-slate-700/50 p-4 rounded-xl hover:border-indigo-500/30 transition-colors flex items-start space-x-4">
               <div className="p-3 bg-slate-800 rounded-lg">
                 <FileText className="w-6 h-6 text-indigo-400" />
               </div>
               <div className="flex-1 min-w-0">
                 <Link to={`/app/documents/${doc.id}`} className="block">
                   <h4 className="font-semibold text-white hover:text-indigo-400 truncate mb-1">{doc.file_name}</h4>
                 </Link>
                 <div className="flex items-center space-x-2 text-xs text-slate-400">
                    <span className="capitalize px-1.5 py-0.5 rounded bg-white/5">{doc.document_type}</span>
                    <span>•</span>
               ...
               <div className="flex flex-col space-y-2">
                 <div className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full border text-center ${
