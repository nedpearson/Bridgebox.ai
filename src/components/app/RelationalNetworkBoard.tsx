...
export default function RelationalNetworkBoard({
  entityType,
  entityId,
}: RelationalNetworkBoardProps) {
  ...
  const loadNetwork = async () => {
    try {
      setLoading(true);
      const data = await entityLinkService.getLinkedEntities(
        entityType,
        entityId,
      );
      setLinks(data);
      ...
      {links.length === 0 ? (
        <div className="bg-white/5 border border-white/10 rounded-xl p-12 text-center flex flex-col items-center">
          ...
          <p className="text-slate-400 font-medium mb-1">
            No relational binds discovered.
          </p>
          <p className="text-slate-500 text-sm">
            This entity exists purely independently in the graph matrix.
          </p>
        </div>
      ) : (
        <div className="grid lg:grid-cols-2 gap-4">
          {links.map((link) => {
            const isTargetNode =
              link.source_id === entityId && link.source_type === entityType;
            const adjacentType = isTargetNode
              ? link.target_type
              : link.source_type;
            ...
            return (
              <div
                key={link.id}
                className="bg-slate-800/30 border border-slate-700/50 p-4 rounded-xl hover:border-indigo-500/30 transition-colors flex items-center justify-between"
              >
