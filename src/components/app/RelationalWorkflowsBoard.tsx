...
export default function RelationalWorkflowsBoard({
  entityType,
  entityId,
}: RelationalWorkflowsBoardProps) {
  const { currentOrganization } = useAuth();
  const [executions, setExecutions] = useState<
    (WorkflowExecution & { workflow?: { name: string; category: string } })[]
  >([]);
  ...
  const loadExecutions = async () => {
    try {
      setLoading(true);
      const data = await workflowService.getExecutionsByEntity(
        entityType,
        entityId,
      );
      setExecutions(data);
      ...
      {executions.length === 0 ? (
        ...
        <div className="space-y-3">
          {executions.map((exec) => (
            <div
              key={exec.id}
              className="bg-slate-800/30 border border-slate-700/50 p-4 rounded-xl flex items-center justify-between hover:border-indigo-500/30 transition-colors"
            >
