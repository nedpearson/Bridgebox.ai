...
import { X, Download, Star, CheckCircle, Clock, Copy, Shield, PackagePlus } from 'lucide-react';
...
export default function TemplateDetailView({ 
  ...
  const { checkoutPremiumTemplate, isProcessing: isCheckingOut } = useMarketplaceCheckout();
  ...
  const handleInstall = async () => {
    ...
    if (template.is_premium) {
      const paymentSuccess = await checkoutPremiumTemplate(template.id, template.price_amount);
    ...
    try {
      ...
      await supabase.rpc('increment_template_install_count', { tmpl_id: template.id });
        ...
        <div className="flex-1 overflow-y-auto p-8 flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-1 space-y-6">
            ...
            {benchmarks && (
              <div className="bg-gradient-to-r from-emerald-500/10 to-teal-500/10 border border-emerald-500/20 rounded-xl p-6">
                <div className="flex items-start space-x-4">
                  ...
                  <div>
                    <h3 className="text-emerald-400 font-bold mb-1">Network Effect Verified</h3>
                    <p className="text-sm text-emerald-200/70">
                      Based on continuous telemetry, companies in {benchmarks.industry} see a 
                      <strong className="text-white"> {benchmarks.efficiencyGain} velocity improvement</strong> using this stack.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
               ...
               <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700/50">
                 <div className="text-xs text-slate-500 mb-1">Included Modules</div>
                 <div className="text-white font-medium">3 Workflows, 2 Agents</div>
               </div>
            </div>
          </div>
          ...
          <div className="w-full md:w-80 space-y-6">
            <div className="bg-slate-800 p-6 rounded-xl border border-slate-700">
               <div className="flex items-baseline justify-between mb-6">
                 {template.is_premium ? (
                   <>
                     <span className="text-3xl font-black text-white">${template.price_amount}</span>
                     <span className="text-sm text-slate-400 uppercase tracking-widest">{template.pricing_model.replace('_', ' ')}</span>
                   </>
                 ) : (
               ...
               >
                 {installSuccess ? (
                   ...
                   <><Clock className="w-5 h-5 animate-spin" /> Authorizing Payment...</>
                 ) : isInstalling ? (
                   <><Clock className="w-5 h-5 animate-spin" /> Provisioning Data Iso-Layer...</>
                 ) : template.is_premium ? (
               ...
               <div className="mt-6 space-y-3 pt-6 border-t border-slate-700">
                 <div className="flex justify-between items-center text-sm">
                   ...
                   <span className="text-white font-medium">{template.install_count.toLocaleString()}</span>
                 </div>
                 <div className="flex justify-between items-center text-sm">
                   <span className="text-slate-400">Community Score</span>
                   <span className="text-amber-400 font-medium flex items-center gap-1">
                     <Star className="w-3.5 h-3.5 fill-current" /> {template.average_rating || 'New'}
                   </span>
                 </div>
               </div>
            </div>
