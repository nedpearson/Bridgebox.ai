...
export default function AgentDetailView({
  ...
  const caps = agent.capabilities_json as {
    triggers: string[];
    actions: string[];
  };
  const recAutonomy =
    agent.recommended_autonomy === "level_1"
      ? "Suggest Only"
      : agent.recommended_autonomy === "level_2"
        ? "Standard Auto"
        : "Fully Autonomous";
          ...
          <div className="grid grid-cols-1 md:grid-cols-2">
            ...
            <div className="p-10 border-b md:border-b-0 md:border-r border-slate-800 relative overflow-hidden">
              ...
              <div className="space-y-4 relative z-10">
                ...
                <div className="bg-slate-950 p-4 border border-slate-800 rounded-xl">
                  <div className="text-xs text-emerald-500/80 font-bold uppercase tracking-wider mb-2">
                    Executed Actions
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {caps?.actions?.map((a: string) => (
                      <span
                        key={a}
                        className="px-2 py-1 bg-emerald-500/10 text-emerald-400 text-xs font-bold rounded-md border border-emerald-500/20"
                      >
                        {a.replace(/_/g, " ")}
                      </span>
                    ))}
                ...
                <ul className="space-y-4 mb-8">
                  <li className="flex items-start text-slate-300">
                    ...
                    <span className="leading-snug">
                      Requires <strong>{recAutonomy}</strong> mode natively,
                      ensuring you remain in total control of external
                      communications.
                    </span>
                  </li>
                  <li className="flex items-start text-slate-300">
                    ...
                    <span className="leading-snug">
                      ...
                      <strong>4.5 Minutes</strong>
                    </span>
                  </li>
                  <li className="flex items-start text-slate-300">
                    <Play className="w-5 h-5 text-blue-400 mr-3 flex-shrink-0 mt-0.5" />
                    <span className="leading-snug">
                      Deploys instantly. No training data or LLM prompt
                      engineering required by your team.
                    </span>
                  </li>
                </ul>
              </div>
              ...
              <div className="pt-6 border-t border-slate-800">
                ...
                {!installed && (
                  <p className="text-center text-xs text-slate-500 mt-4 leading-relaxed">
                    Included with your Premium Platform subscription.
                    <br />
                    Subject to AI token fair-use policies.
                  </p>
                )}
              </div>
            </div>
