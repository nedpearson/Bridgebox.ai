...
export default function WorkflowCaptureWorkspace() {
  ...
  const handleRecordToggle = () => {
    if (!isRecording) {
      ...
      setTranscript(
        "Okay, so when a new client calls in, the receptionist creates a lead in the system. Then the attorney reviews it and either approves or rejects. If approved, we send a retainer agreement via email. Once signed, we automatically generate a case file and schedule the initial deposition.",
      );
    ...
    <div className="flex flex-col lg:flex-row h-full overflow-hidden bg-slate-950">
      ...
      <div className="flex-1 p-6 border-r border-white/5 overflow-y-auto bg-slate-900/20">
        <h3 className="text-white font-medium mb-6">
          Extracted Procedural Operations
        </h3>
        ...
        {isExtracting ? (
          <div className="h-64 flex items-center justify-center flex-col text-slate-500">
            ...
            <p className="text-sm">
              Synthesizing entity handoffs and dependencies...
            </p>
          </div>
          ...
          <div className="p-4 bg-slate-900/50 rounded-lg border border-slate-800 border-dashed text-slate-500 text-sm text-center max-w-md mx-auto mt-12">
            Record or upload a workflow description to visualize the
            step-by-step logic graph.
          </div>
        ) : (
            ...
            <div className="relative pl-6 space-y-6 before:absolute before:inset-y-0 before:left-2 before:w-px before:bg-indigo-500/30">
              <div className="relative bg-slate-900 rounded-xl p-4 border border-slate-800 shadow-sm">
                ...
                <p className="text-xs text-slate-400">
                  Triggered by inbound client call.
                </p>
              </div>
              ...
              <div className="relative bg-slate-900 rounded-xl p-4 border border-slate-800 shadow-sm">
                <span className="absolute -left-[21px] top-4 text-emerald-500 bg-slate-950">
                  <GitCommit className="w-5 h-5" />
                </span>
                <div className="flex justify-between items-start mb-2">
                  <h4 className="text-sm font-semibold text-white">
                    Automated Retainer Generation
                  </h4>
                  <span className="px-2 py-0.5 bg-emerald-500/10 text-emerald-400 text-xs border border-emerald-500/20 rounded flex items-center">
                    <Zap className="w-3 h-3 mr-1" /> System
                  </span>
                </div>
                <p className="text-xs text-slate-400">
                  Generates document and dispatches via DocuSign webhook.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4 animate-fade-in">
            <div className="p-4 bg-indigo-500/10 border border-indigo-500/20 rounded-xl">
              ...
              <p className="text-xs text-slate-300 leading-relaxed mb-3">
                You mentioned sending a retainer agreement. Bridgebox can
                natively integrate with DocuSign to dispatch this immediately
                upon approval.
              </p>
