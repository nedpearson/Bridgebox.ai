...
export default function AiOnboardingWizard() {
  ...
  const mergedContext = `
    ...
    ${properties
      ...
      .map(
        (p) =>
          `- Property: ${p.name || "Unnamed"}, URL: ${p.url || "None"}, Category: ${p.category || "N/A"}, Location: ${p.location || "N/A"}`,
      )
      .join("\n")}
    ...
    try {
      ...
      const { data: sessionData, error } = await supabase
        ...
        .insert({
          ...
          client_id: user.id,
          session_title: isProjectMode
            ? `Project Scaffold: ${projectName || "Untitled"}`
            : "AI Blueprint Architecture Run",
      ...
      await organizationsService.updateOrganization(currentOrganization.id, {
        ...
        metadata: {
          ...
          properties: properties.filter(
            (p) => p.name.trim() || p.url.trim() || p.location.trim(),
          ),
        },
      });
      ...
      setStep(2);
      setTimeout(
        () => navigate(`/app/onboarding/${sessionData.id}/admin`),
        2500,
      );
    } catch (err) {
      ...
      <div className="min-h-screen bg-slate-900 flex items-center justify-center">
        ...
        >
          ...
          <p className="text-slate-400 max-w-md mx-auto">
            Bridgebox AI has successfully extracted your systems telemetry. Your
            dedicated Implementation Engineer in the Command Center is reviewing
            the graph.
          </p>
        </motion.div>
      </div>
        ...
        <div className="mt-8">
          ...
          >
            <h1 className="text-4xl font-bold text-white mb-4">
              {isProjectMode
                ? "Let's blueprint your initial project."
                : "Let's blueprint your business."}
            </h1>
            <p className="text-xl text-slate-400">
              {isProjectMode
                ? "Describe the scope and timeline of the workflow you are implementing, and I will scaffold the exact entity maps instantly."
          ...
          <div className="space-y-6">
            ...
            <div className="mb-8">
              <label className="block text-sm font-medium text-slate-300 mb-2">
                {isProjectMode
                  ? "Describe the project constraints, goals, and necessary sub-tasks."
              ...
              <AiInputAssist
                ...
                placeholder={
                  isProjectMode
                    ? "We need to deploy a new Real Estate CRM integration over the next 4 weeks. Key tasks include..."
                    : "We run a boutique real estate agency. Our biggest pain point is tracking lead conversions and making sure documents aren't lost..."
                }
            ...
            <div>
              ...
              <datalist id="wizard-existing-names">
                {Array.from(
                  new Set(
                    properties.filter((p) => p.name.trim()).map((p) => p.name),
                  ),
                ).map((name) => (
              ...
              <div className="space-y-3 mb-4">
                {properties.map((prop, idx) => (
                  <div
                    key={idx}
                    className="flex flex-col space-y-2 p-3 bg-slate-800/30 border border-slate-700/50 rounded-lg relative"
                  >
                    {properties.length > 1 && (
                      <button
                        type="button"
                    ...
                    <div className="grid grid-cols-2 gap-3 pr-6">
                      <div className="col-span-1">
                        <AiInputAssist
                          list="wizard-existing-names"
                          placeholder="Brand/Company Name"
                          value={prop.name}
                          onChange={(val) => {
                            const newProps = [...properties];
                            newProps[idx].name = val;
                            setProperties(newProps);
                          }}
                          className="bg-slate-900 border border-slate-700 text-sm text-white px-3 py-2 rounded focus:border-indigo-500"
                        />
                      </div>
                      <div className="col-span-1">
                        <AiInputAssist
                      ...
                      <div className="col-span-1">
                        <AiInputAssist
                          list="wizard-existing-categories"
                          placeholder="Category (e.g. Retail)"
                          value={prop.category}
                          onChange={(val) => {
                            const newProps = [...properties];
                            newProps[idx].category = val;
                            setProperties(newProps);
                          }}
                          className="bg-slate-900 border border-slate-700 text-sm text-white px-3 py-2 rounded focus:border-indigo-500"
                        />
                      ...
                      <div className="col-span-1">
                        <AiInputAssist
                          list="wizard-existing-locations"
                          placeholder="Location (e.g. NYC)"
                          value={prop.location}
                          onChange={(val) => {
                            const newProps = [...properties];
                            newProps[idx].location = val;
                            setProperties(newProps);
                          }}
                          className="bg-slate-900 border border-slate-700 text-sm text-white px-3 py-2 rounded focus:border-indigo-500"
                        />
            ...
            {isProjectMode && (
              <div
                className="mt-3 flex items-center space-x-3 bg-slate-800/30 border border-slate-700/50 p-4 rounded-lg cursor-pointer hover:bg-slate-800/50 transition-colors"
                onClick={() => setRequireMobileApp(!requireMobileApp)}
              >
                <div
                  className={`w-5 h-5 rounded border flex items-center justify-center flex-shrink-0 transition-colors ${requireMobileApp ? "bg-indigo-500 border-indigo-500" : "border-slate-600 bg-slate-900"}`}
                >
                  {requireMobileApp && (
                ...
                <div className="select-none">
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    We need this project built for iOS and Android.
                  </p>
                </div>
              </div>
            )}
        ...
        <div className="mt-12 flex items-center justify-between">
          ...
          <button
            ...
            className={`flex items-center px-6 py-3 font-medium rounded-lg transition-colors ${
              isSubmitting || businessInput.length < 10
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          ...
          >
            ...
            <p className="text-slate-400 text-sm mb-6">
              Create an empty organization instantly without AI scaffolding.
            </p>
