...
function addItem<T>(arr: T[], empty: T): T[] {
  return [...arr, empty];
}
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <p className="text-xs font-700 text-slate-400 uppercase tracking-widest mb-3">
      {children}
    </p>
  );
}
    ...
    <div className="min-h-screen bg-slate-950">
      ...
      <div className="max-w-3xl mx-auto px-8 py-12">
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 2 && (
            <Step2CurrentSoftware key="s2" data={data} update={update} />
          )}
          {step === 3 && (
            <Step3KeepImproveRemove key="s3" data={data} update={update} />
          )}
          {step === 4 && (
            <Step4Integrations key="s4" data={data} update={update} />
          )}
          {step === 5 && (
            <Step5TeamDashboard key="s5" data={data} update={update} />
          )}
        ...
        {!creditWarning &&
          ...
          !entitlements.credits.isUnlimited && (
            <div className="mt-4">
              <UpgradeNudge
                trigger="low_credits"
          ...
          <div className="mt-8 flex items-center justify-between">
            ...
            {step < STEPS.length ? (
              ...
              >
                {saving
                  ? "Saving Profile..."
                  : "Generate My Software Blueprint"}
...
const VOICE_PROMPTS = [
  {
    label: "Start here",
    text: "Tell us the names of every software tool your team uses today, even ones you dislike.",
  },
  {
    label: "What works",
    text: "Which features or workflows do you absolutely depend on and never want to lose?",
  },
  {
    label: "What frustrates you",
    text: "What wastes the most time? What's broken, confusing, or just painful every day?",
  },
  {
    label: "Your team",
    text: "Who uses your software? Describe each role — what do they need to see and do every day?",
  },
  {
    label: "Ideal outcome",
    text: "If your perfect system existed today, what would it let you do that you can't do now?",
  },
  {
    label: "Integrations",
    text: "What other tools or systems must your new software connect to? Payments, accounting, email, etc.",
  },
];
  ...
  return (
    <div className="space-y-2">
      <p className="text-xs text-slate-500 uppercase tracking-widest font-semibold mb-3">
        {isRecording
          ? "Guided prompts — rotating automatically"
          : "What to cover"}
      </p>
      <div className="grid grid-cols-2 gap-2">
        {VOICE_PROMPTS.map((p, i) => (
          ...
          >
            <p
              className={`text-xs font-bold mb-1 ${i === activeIdx ? "text-indigo-400" : "text-slate-500"}`}
            >
              {p.label}
            </p>
            <p
              className={`text-xs leading-relaxed ${i === activeIdx ? "text-slate-200" : "text-slate-500"}`}
            >
              {p.text}
            </p>
          </button>
    ...
    >
      <div className="p-4 bg-violet-500/5 border border-violet-500/20 rounded-2xl">
        ...
        <p className="text-slate-400 text-xs leading-relaxed">
          Screen recordings of your current software in action are most
          valuable. Show us your dashboards, workflows, forms, reports —
          anything your team uses daily. Screenshots and audio are also
          accepted.
        </p>
      </div>
      ...
      >
        ...
        <p className="text-slate-300 text-sm font-medium">
          Drop recordings here, or click to browse
        </p>
        <p className="text-slate-600 text-xs mt-1">
          MP4, MOV, WebM, PNG, JPG, MP3 · Max 2GB each
        </p>
      </div>
      ...
      {files.length > 0 && (
        <div className="space-y-2">
          ...
          >
            Continue with {files.length} recording{files.length > 1 ? "s" : ""}{" "}
...
function Step1EntryMode({
  ...
  data: DiscoveryData;
  update: (p: Partial<DiscoveryData>) => void;
  onNext: () => void;
}) {
  const modes = [
    {
  ...
  return (
    <StepCard>
      <p className="text-slate-400 text-sm mb-8 max-w-lg">
        Choose how you'd like to describe your current software and workflows.
        You can mix methods — we'll combine everything into your Workspace
      ...
      <div className="space-y-4">
        {modes.map((m) => (
          ...
          >
            <div
              className={`w-12 h-12 ${m.iconBg} rounded-xl flex items-center justify-center flex-shrink-0 shadow-lg`}
            >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span
                  className={`text-[10px] font-bold px-2 py-0.5 rounded-full border uppercase tracking-wide ${m.badgeColor}`}
                >
                  {m.badge}
                </span>
              </div>
          ...
          >
            <div>
              ...
              <p className="text-slate-500 text-xs">
                Speak freely — the prompts below will guide you through each
                topic automatically.
              </p>
            </div>
  ...
  return (
    <StepCard>
      <p className="text-slate-400 text-sm mb-6 max-w-xl">
        List every tool and platform your team currently uses — even ones you
        hate. We need the full picture to build something genuinely better.
      </p>
      <div className="space-y-3 mb-4">
        {data.currentSoftware.map((sw, i) => (
          <div
            key={i}
            className="grid grid-cols-12 gap-2 items-start p-4 bg-slate-800/40 border border-slate-700/40 rounded-xl"
          >
            <div className="col-span-4">
              <input
                ...
                onChange={(e) =>
                  setSoftware(
                    updateItem(data.currentSoftware, i, {
                      ...sw,
                      name: e.target.value,
                    }),
                  )
            ...
            <div className="col-span-1 flex justify-end pt-1">
              {data.currentSoftware.length > 1 && (
                <button
                  onClick={() =>
                    setSoftware(removeItem(data.currentSoftware, i))
                  }
      ...
      <div className="mt-6 pt-5 border-t border-slate-700/50">
        ...
        <p className="text-slate-400 text-xs mb-3">
          Anything else about how your team works day-to-day?
        </p>
        <textarea
          rows={3}
  ...
  return (
    <StepCard>
      <p className="text-slate-400 text-sm mb-8 max-w-xl">
        This is the most important step. Be specific — even small details here
        shape the entire blueprint.
      </p>
      <div className="space-y-8">
        <div>
          ...
          <p className="text-slate-500 text-xs mb-3">
            Workflows, screen layouts, reports, or processes that work well and
        ...
        <div>
          ...
          <p className="text-slate-500 text-xs mb-3">
            Features you like but that are slow, cumbersome, or only partially
            working.
          </p>
          <TagInput
        ...
        <div>
          ...
          <p className="text-slate-500 text-xs mb-3">
            Tools, screens, or workflows that nobody uses, that create
            confusion, or that duplicate work.
          </p>
          <TagInput
      ...
      <div className="space-y-8">
        <div>
          ...
          <p className="text-slate-400 text-xs mb-4">
            What external systems must your new software connect to? APIs,
            payment processors, ERPs, accounting tools, etc.
          </p>
          <TagInput
        ...
        <div className="pt-4 border-t border-slate-700/50">
          ...
          <p className="text-slate-400 text-xs mb-3">
            What industry or niche are you in? This helps us tailor terminology,
            compliance, and workflow patterns.
          </p>
          <input
            type="text"
...
function Step5TeamDashboard({
  ...
  const togglePref = (pref: string) => {
    const current = data.dashboardPreferences
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
  ...
  const selectedPrefs = data.dashboardPreferences
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
    ...
    <StepCard>
      <div className="space-y-8">
        <div>
          ...
          <p className="text-slate-400 text-xs mb-4">
            List the roles or teams that will use this system. Be specific — a
            salesperson needs different views than an accountant.
          </p>
          <TagInput
        ...
        <div className="pt-4 border-t border-slate-700/50">
          <SectionLabel>
            What should the main dashboard show first?
          </SectionLabel>
          <p className="text-slate-400 text-xs mb-4">
            Select everything that matters. The most important thing should be
            impossible to miss on day one.
          </p>
          <div className="flex flex-wrap gap-2">
            {DASHBOARD_PREFS.map((pref) => {
              ...
              return (
                ...
                >
                  {selected && (
                    <CheckCircle2 className="w-3.5 h-3.5 inline mr-1.5 text-indigo-400" />
                  )}
                  {pref}
                </button>
              );
...
function Step6Outcome({
  ...
  return (
    <StepCard>
      <p className="text-slate-400 text-sm mb-8 max-w-xl">
        The last step. Describe exactly what success looks like for your team —
        in your own words. There are no wrong answers. This becomes the
        foundation of your custom software blueprint.
      </p>
      <div className="space-y-6">
        <div>
          <SectionLabel>Describe your ideal system</SectionLabel>
          <p className="text-slate-400 text-xs mb-3">
            If your perfect software already existed today — what would it do?
            How would your team use it differently? What problem would finally
            be solved?
          </p>
          <textarea
            rows={6}
        ...
        <div className="p-5 bg-gradient-to-br from-indigo-500/5 to-violet-500/5 border border-indigo-500/15 rounded-2xl">
          ...
          <div className="grid grid-cols-2 gap-3 text-xs">
            <PreviewRow
              ...
              value={`${data.currentSoftware.filter((s) => s.name).length} tools captured`}
            />
            <PreviewRow
              label="Keep features"
              value={`${data.keepFeatures.filter(Boolean).length} defined`}
            />
            <PreviewRow
              label="Improve areas"
              value={`${data.improveFeatures.filter(Boolean).length} defined`}
            />
            <PreviewRow
              label="Remove items"
              value={`${data.removeFeatures.filter(Boolean).length} defined`}
            />
            <PreviewRow
              label="Integrations"
              value={`${data.requiredIntegrations.filter(Boolean).length} required`}
            />
            <PreviewRow
              label="User roles"
              value={`${data.userRoles.filter(Boolean).length} defined`}
            />
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      ...
      >
        ...
        <p className="text-slate-400 mb-2">
          {orgName ? `${orgName}'s` : "Your"} software intelligence profile has
          been saved. Bridgebox is generating your custom software blueprint
          now.
        </p>
        <p className="text-slate-600 text-sm">
          Redirecting to your Build Queue…
        </p>
      </motion.div>
    </div>
  );
...
function buildTranscript(data: DiscoveryData): string {
  ...
  if (data.currentSoftware.some((s) => s.name)) {
    ...
    data.currentSoftware
      ...
      .forEach((s) => {
        lines.push(
          `- ${s.name}${s.category ? ` (${s.category})` : ""}${s.notes ? `: ${s.notes}` : ""}`,
        );
      });
    lines.push("");
  }
