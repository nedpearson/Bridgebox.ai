    ...
    <div className="h-full flex flex-col max-w-7xl mx-auto animate-fade-in">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-slate-400">
            Manage the AI-generated structural tasks required to finalize the
            onboarding scope.
          </p>
        </div>
      ...
      <div className="flex-1 overflow-x-auto overflow-y-hidden pb-4">
        {loading ? (
          ...
          <div className="flex h-full items-center justify-center text-slate-500">
            <p>
              No actionable execution constraints detected. AI is waiting for
              directives.
            </p>
          </div>
        ) : (
              ...
              <div className="space-y-3 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                {tasks
                  ...
                  .map((task) => (
                    <div
                      key={task.id}
                      className="bg-slate-950 p-4 rounded-xl border border-slate-800 hover:border-indigo-500/50 transition-colors cursor-pointer group"
                    >
              ...
              <div className="space-y-3 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                {tasks
                  ...
                  .map((task) => (
                    <div
                      key={task.id}
                      className="bg-slate-950 p-4 rounded-xl border border-indigo-500/50 transition-colors cursor-pointer group shadow-[0_0_15px_rgba(99,102,241,0.1)]"
                    >
                      <div className="flex justify-between mb-3">
              ...
              <div className="space-y-3 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                {tasks
                  ...
                  .map((task) => (
                    <div
                      key={task.id}
                      className="bg-slate-950 p-4 rounded-xl border border-emerald-500/20 transition-colors cursor-pointer group"
                    >
                      <div className="flex justify-between mb-3">
                      ...
                      <h4 className="text-white text-sm font-medium mb-1 truncate line-through opacity-70">
                        {task.title}
                      </h4>
                    </div>
                  ))}
