...
export default function IntegrationIngestionLog() {
  ...
  const handleApprove = async (webhook: WebhookPayload) => {
    ...
    const success = await schemaNormalizer.physicallyCommitPayload(
      webhook,
      user.id,
    );
    if (success) {
  ...
  const handleReject = async (id: string) => {
    setProcessingId(id);
    await schemaNormalizer
      .physicallyCommitPayload({ id, status: "rejected" } as any, user!.id)
      .catch(() => {});
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Review and approve AI-normalized payloads captured from external
            software connections (Zapier, Make, APIs).
          </p>
        </div>
      </div>
      ...
      {webhooks.length === 0 ? (
        <div className="rounded-xl border border-dashed border-slate-700 bg-slate-900/50 p-12 text-center text-slate-400">
          ...
          <h3 className="text-lg font-medium text-white mb-1">
            Stack Normalizer Idle
          </h3>
          <p className="text-sm max-w-sm mx-auto">
            No pending webhooks or integration packets detected. External data
            will appear here for schema mapping approval securely.
          </p>
        </div>
      ) : (
        <div className="grid gap-4">
          {webhooks.map((webhook) => (
            <div
              key={webhook.id}
              className="rounded-xl border border-slate-700 bg-slate-800 shadow-sm overflow-hidden"
            >
              <div className="p-4 bg-slate-800/80 border-b border-slate-700/50 flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <div
                    className={`p-2 rounded-lg ${webhook.source === "zapier" ? "bg-orange-500/10 text-orange-400" : "bg-indigo-500/10 text-indigo-400"} border border-slate-700`}
                  >
                    <Network className="w-4 h-4" />
                  </div>
                  <div>
                    <h4 className="text-sm font-semibold text-white capitalize">
                      {webhook.source} Webhook Hooked
                    </h4>
                ...
                <div className="flex items-center space-x-2">
                  <span
                    className={`inline-flex items-center px-2 py-1 rounded text-xs font-semibold uppercase tracking-wider ${
                ...
                <div className="p-4 bg-indigo-950/10 relative overflow-hidden">
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <h5 className="text-xs font-semibold text-indigo-400 uppercase tracking-wider flex items-center">
                      <Sparkles className="w-3 h-3 mr-1" /> AI Target Mapping:{" "}
                      <span className="text-white ml-2 capitalize bg-indigo-500/20 px-2 py-0.5 rounded">
                        {webhook.mapped_entity_type || "Unknown"}
                      </span>
                    </h5>
                  </div>
