...
const TIER_META: Record<
  string,
  { label: string; gradient: string; accent: string; bg: string }
> = {
  low: {
    label: "Starter",
...
function buildLineItems(
  ...
  return [
    ...
    },
    {
      ...
      value:
        Math.round(breakdown.workflowCost * multipliers.workflow * 100) / 100,
    ...
    },
    {
      ...
      value:
        Math.round(breakdown.storageCost * multipliers.storage * 100) / 100,
    ...
    try {
      ...
      if (pricingModelId) {
        ...
        if (pricing?.ai_inputs_snapshot) {
          setBreakdown(
            calculatePricing(pricing.ai_inputs_snapshot as PricingInputs),
          );
        }
      }
    } catch (err) {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-6">
        ...
        <p className="text-slate-400 text-sm max-w-sm">
          Your implementation engineer will review your setup and generate a
          custom price shortly.
        </p>
        <button
  ...
  return (
    <div className="min-h-screen bg-slate-950 px-4 py-12">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        >
          <span
            className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold text-white bg-gradient-to-r ${tierMeta.gradient}`}
          >
          ...
          <p className="text-slate-400 text-sm max-w-md mx-auto">
            Built specifically around your workflows, integrations, and AI
            usage. No hidden fees. Adjust anything below.
          </p>
        </motion.div>
        ...
        <motion.div
          ...
          style={{
            borderColor: `${tierMeta.accent}30`,
            background: tierMeta.bg,
          }}
        >
          {/* subtle glow line */}
          <div
            className={`absolute top-0 left-0 right-0 h-px bg-gradient-to-r ${tierMeta.gradient} opacity-60`}
          />
        ...
        >
          <div className="px-5 py-4 border-b border-slate-800">
            ...
            <p className="text-slate-500 text-xs mt-0.5">
              Click any row to learn more. Adjustable rows have a slider icon.
            </p>
          </div>
          ...
          <div className="divide-y divide-slate-800/70">
            {lineItems.map((item, i) => (
              <React.Fragment key={item.id}>
                <button
                  onClick={() =>
                    setActiveInfo(activeInfo === item.id ? null : item.id)
                  }
        ...
        >
          <div>
            ...
            <p className="text-slate-500 text-xs mt-0.5">
              Drag to see how your choices affect the monthly cost in real time.
            </p>
          </div>
          ...
          {/* Reset */}
          {(multipliers.ai !== 1 ||
            multipliers.workflow !== 1 ||
            multipliers.storage !== 1) && (
            <button
          ...
          >
            ...
            >
              <span className="text-emerald-400 font-semibold text-sm flex items-center gap-2">
                ...
                {breakdown.optimizationOpportunities.length} Way
                {breakdown.optimizationOpportunities.length !== 1 ? "s" : ""} to
                Reduce Your Cost
              </span>
              {showOpts ? (
        ...
        >
          {[
            ...
            <div
              key={p.label}
              className="bg-slate-900/60 border border-slate-800 rounded-xl py-3 px-4 text-center"
            >
        ...
        <p className="text-center text-slate-600 text-xs pb-4">
          Your pricing will be reviewed by your implementation engineer before
          being finalised.
        </p>
      </div>
    </div>
...
const INFO_COPY: Record<string, string> = {
  ...
  workflow:
    "Each workflow automation has a monthly execution cost based on how often it runs and how many steps it contains. You can pause or scale individual workflows at any time.",
  integrations:
    "Connects your existing tools (CRM, ERP, email, etc.) to Bridgebox. Cost scales with the number of systems and how frequently data is synced.",
  storage:
    "Storage for uploaded documents, generated files, and processed data. Document processing incurs an additional cost per page for AI extraction.",
  features:
    "Custom-built modules specific to your business — unique views, automations, or data models not covered by the standard platform.",
  support:
    "AI-powered support and debugging agents that resolve issues, answer questions, and surface insights proactively.",
};
