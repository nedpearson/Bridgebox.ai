...
export default function RevenueDashboard() {
  ...
  const [metrics, setMetrics] = useState({
    ...
    industryBreakdown: [] as { name: string; revenue: number; margin: number }[],
    totalCogs: 0
  });
  ...
  const fetchAggregatedRevenueData = async () => {
    try {
      ...
      orgs?.forEach(org => {
         ...
         if (org.billing_plan === 'Enterprise') { mockUsers = 100; mockTech = ['stripe', 'quickbooks', 'slack']; aiUsage = 'unlimited'; tiers.Enterprise++; }
         else if (org.billing_plan === 'Professional') { mockUsers = 25; mockTech = ['stripe', 'quickbooks']; aiUsage = 'high'; tiers.Professional++; }
         else if (org.billing_plan === 'Growth') { mockUsers = 10; mockTech = ['stripe']; aiUsage = 'standard'; tiers.Growth++; }
         else { tiers.Starter++; }
         ...
         const pricing = calculatePricing({
           ...
           aiUsage: aiUsage,
           mobile: org.billing_plan === 'Enterprise' || org.billing_plan === 'Professional',
           users: mockUsers,
           locations: 1
         });
      ...
      const industryArr = Object.keys(industries).map(ind => {
      ...
      const marginPerc = totalMrr > 0 ? Math.round((grossProfit / totalMrr) * 100) : 0;
  ...
  if (loading) {
    return (
      <>
        <AppHeader title="Revenue & Margin Analytics" subtitle="Financial Performance Overview" />
  ...
  return (
    <>
      <AppHeader title="Revenue & Margin Analytics" subtitle="Global Platform Financial Performance" />
      ...
      <div className="p-8 max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <Card glass className="p-6 relative overflow-hidden">
            ...
            <h3 className="text-slate-400 font-medium text-sm">Annual Run Rate (ARR)</h3>
            <div className="text-3xl font-black text-white mt-1">${metrics.arr.toLocaleString()}</div>
          </Card>

          <Card glass className="p-6 relative overflow-hidden border-amber-500/20 shadow-[0_0_20px_rgba(245,158,11,0.05)]">
            ...
            <h3 className="text-slate-400 font-medium text-sm">Target Gross Margin</h3>
            <div className="flex items-end space-x-2 mt-1">
              <div className="text-3xl font-black text-white">{metrics.overallMargin}%</div>
              <div className="text-sm font-medium text-slate-500 mb-1">COGS: ${metrics.totalCogs.toLocaleString()}</div>
            </div>
          </Card>
          ...
          <Card glass className="p-6 relative overflow-hidden">
            ...
            <h3 className="text-slate-400 font-medium text-sm">Average Revenue Per User</h3>
            <div className="text-3xl font-black text-white mt-1">${metrics.arpu.toLocaleString()}</div>
          </Card>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
           ...
           <Card glass className="p-6">
             <h3 className="text-lg font-bold text-white mb-6 flex items-center">
                <Building2 className="w-5 h-5 mr-2 text-indigo-400" /> License Distribution
             </h3>
             <div className="space-y-4">
               {Object.entries(metrics.tierBreakdown).map(([tier, count]) => {
                 const percentage = metrics.totalTenants > 0 ? (count / metrics.totalTenants) * 100 : 0;
                 return (
                 <div key={tier}>
                   <div className="flex justify-between text-sm mb-2">
                     <span className="font-semibold text-slate-300">{tier}</span>
                     <span className="text-slate-400">{count} Tenants ({percentage.toFixed(1)}%)</span>
                   </div>
           ...
           <Card glass className="p-6">
             ...
             <div className="space-y-4">
               {metrics.industryBreakdown.slice(0, 5).map((ind, idx) => (
                 <div key={idx} className="flex justify-between items-center p-3 rounded-xl bg-slate-900/50 border border-slate-800 hover:bg-slate-800 transition-colors">
                   <div className="flex flex-col">
                     <span className="text-white font-bold">{ind.name}</span>
                     <span className="text-xs font-semibold text-slate-500 uppercase tracking-widest mt-1">Margin: {ind.margin}%</span>
                   </div>
               ...
               {metrics.industryBreakdown.length === 0 && (
                 <div className="text-slate-500 text-sm text-center py-4">No active industry data</div>
               )}
             </div>
           </Card>
