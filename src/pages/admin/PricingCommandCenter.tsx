...
import {
  DollarSign, Sliders, CheckCircle2, Clock, Building2, TrendingUp,
  TrendingDown, AlertTriangle, Sparkles, Edit3, Save, X, BarChart3,
  ChevronDown, ChevronUp, RefreshCw, Eye
} from 'lucide-react';
import { supabase } from '../../lib/supabase';
import { calculatePricing, fmtCurrency, fmtTokens } from '../../lib/billing/pricingEngine';
...
const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string }> = {
  ...
  pending_review: { label: 'Pending',      color: 'text-amber-400', bg: 'bg-amber-400/10' },
...
export default function PricingCommandCenter() {
  ...
  const [expandedTab, setExpandedTab] = useState<'detail' | 'calibration'>('detail');
  ...
  // Summary stats
  const totalPipeline = models.reduce((s, m) => s + (m.estimated_total_monthly_cost || 0), 0);
  const approvedRevenue = models.filter(m => ['approved', 'active'].includes(m.status)).reduce((s, m) => s + (m.approved_total_monthly_cost || m.estimated_total_monthly_cost || 0), 0);
  const pendingModels = models.filter(m => m.status === 'pending_review').length;
  ...
  const runBulkCalibration = async () => {
    ...
    try {
      ...
      setBulkResult(`Calibrated ${result.processed} model(s). ${result.failed} failed.`);
      await loadModels();
    } catch (err: any) {
  ...
  const startEdit = (model: PricingModel) => {
    ...
    setOverrideValues({
      ...
      approved_total_monthly_cost: model.approved_total_monthly_cost ?? model.estimated_total_monthly_cost,
  ...
  const filteredModels = filterStatus === 'all' ? models : models.filter(m => m.status === filterStatus);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        {bulkResult && (
          <p className="text-sm text-slate-400 bg-slate-800 px-4 py-2 rounded-lg border border-slate-700">{bulkResult}</p>
        )}

        {/* Summary Stats */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {[
            { label: 'Total Pipeline', value: fmtCurrency(totalPipeline) + '/mo', icon: <TrendingUp className="w-5 h-5" />, color: 'text-blue-400' },
            { label: 'Approved MRR', value: fmtCurrency(approvedRevenue) + '/mo', icon: <CheckCircle2 className="w-5 h-5" />, color: 'text-emerald-400' },
            { label: 'Pending Review', value: `${pendingModels} models`, icon: <Clock className="w-5 h-5" />, color: 'text-amber-400' },
            { label: 'Total Clients', value: `${models.length} total`, icon: <Building2 className="w-5 h-5" />, color: 'text-violet-400' },
          ].map((stat) => (
        ...
        <div className="flex space-x-2">
          {['all', 'pending_review', 'approved', 'active', 'draft', 'archived'].map((status) => (
            <button
              key={status}
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {filteredModels.map((model) => {
              const statusCfg = STATUS_CONFIG[model.status] || STATUS_CONFIG.draft;
                ...
                >
                  ...
                  <div className="flex items-center justify-between p-5">
                    <div className="flex items-center space-x-4">
                      <div>
                        <p className="text-white font-semibold">{(model as any).organizations?.name || 'Unknown Client'}</p>
                        <p className="text-slate-500 text-xs mt-0.5 capitalize">
                          {(model as any).organizations?.industry || 'Industry unknown'} • {model.tier} tier
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-6">
                      <div className="text-right hidden md:block">
                        ...
                        <p className="text-slate-300 text-sm font-medium">{fmtCurrency(rawAiCost)}</p>
                      </div>
                      <div className="text-right hidden md:block">
                        <p className="text-slate-500 text-xs">AI Profit</p>
                        <p className="text-emerald-400 text-sm font-medium">+{fmtCurrency(aiProfit)}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-slate-500 text-xs">Est. Monthly</p>
                        <p className="text-white text-lg font-bold">{fmtCurrency(model.approved_total_monthly_cost ?? model.estimated_total_monthly_cost)}</p>
                      </div>
                      <span className={`px-3 py-1 rounded-full text-xs font-medium ${statusCfg.color} ${statusCfg.bg}`}>
                        {statusCfg.label}
                      </span>
                      <div className="flex items-center space-x-2">
                        ...
                        <button onClick={() => startEdit(model)} className="p-1.5 text-slate-400 hover:text-white transition-colors">
                          <Edit3 className="w-4 h-4" />
                        </button>
                        <button onClick={() => setExpandedId(isExpanded ? null : model.id)} className="p-1.5 text-slate-400 hover:text-white transition-colors">
                    ...
                    {isEditing && (
                      ...
                      >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                          {[
                            { label: 'Base Platform Fee ($)', key: 'base_platform_fee' as const },
                            { label: 'AI Margin Multiplier (x)', key: 'ai_margin_multiplier' as const },
                            { label: 'Approved Total Monthly ($)', key: 'approved_total_monthly_cost' as const },
                          ].map((field) => (
                            <div key={field.key}>
                              <label className="block text-slate-400 text-xs font-medium mb-1.5">{field.label}</label>
                              <input
                                type="number"
                                value={String(overrideValues[field.key] ?? '')}
                                onChange={(e) => setOverrideValues(prev => ({ ...prev, [field.key]: parseFloat(e.target.value) }))}
                        ...
                        <div className="mb-4">
                          <label className="block text-slate-400 text-xs font-medium mb-1.5">Admin Notes</label>
                          <textarea
                            rows={2}
                            value={overrideValues.admin_notes || ''}
                            onChange={(e) => setOverrideValues(prev => ({ ...prev, admin_notes: e.target.value }))}
                        ...
                        <div className="flex justify-end space-x-2">
                          <button onClick={() => setEditingId(null)} className="flex items-center px-4 py-2 text-slate-400 hover:text-white transition-colors text-sm">
                          ...
                          >
                            {savingId === model.id
                              ...
                              : <><Save className="w-4 h-4 mr-1.5" />Save Override</>
                            }
                          </button>
                        </div>
                      </motion.div>
                      ...
                      >
                        ...
                        <div className="flex border-b border-slate-800">
                          {(['detail', 'calibration'] as const).map(tab => (
                            ...
                            >
                              {tab === 'calibration' ? '🔬 Calibration & Prediction' : '📋 Detail'}
                            </button>
                          ))}
                        </div>
                          ...
                          {expandedTab === 'detail' ? (
                            <>
                              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                {[
                                  { label: 'Tokens/Month', value: fmtTokens(model.estimated_tokens_per_month) },
                                  { label: 'AI Margin', value: `${getMarginPercent(model)}%` },
                                  { label: 'Workflows', value: model.workflow_count },
                                  { label: 'Integrations', value: model.integration_count },
                                  { label: 'Storage (GB)', value: `${model.estimated_storage_gb} GB` },
                                  { label: 'Custom Features', value: model.custom_feature_count },
                                  { label: 'Support Tier', value: model.support_agent_usage },
                                  { label: 'Override Applied', value: model.admin_override_applied ? 'Yes' : 'No' },
                                ].map((item) => (
                              ...
                              {model.admin_notes && (
                                <div className="mt-4 p-3 bg-amber-400/5 border border-amber-400/20 rounded-lg">
                                  <p className="text-amber-400 text-xs font-medium mb-1">Admin Notes</p>
                                  <p className="text-slate-300 text-sm">{model.admin_notes}</p>
                                </div>
                              )}
                            </>
                          ) : (
                            <CalibrationPanel
                              ...
                              organizationName={(model as any).organizations?.name}
                            />
                          )}
                        </div>
                      </motion.div>
