...
const ALL_FEATURE_KEYS = [
  ...
  {
    key: "screen_recording_analysis",
    label: "Screen Recording Analysis",
    tier: "Growth",
  },
  {
    key: "workspace_learning_ai",
    label: "Workspace Learning AI",
    tier: "Growth",
  },
  {
    key: "reusable_feature_packs",
    label: "Reusable Feature Packs",
    tier: "Growth",
  },
  {
  ...
  {
    key: "premium_admin_controls",
    label: "Premium Admin Controls",
    tier: "Pro",
  },
  {
    key: "predictive_recommendations",
    label: "Predictive AI Recommendations",
    tier: "Pro",
  },
  { key: "implementation_queue", label: "Implementation Queue", tier: "Pro" },
  {
    key: "white_glove_onboarding",
...
export default function MonetizationHub() {
  const [activeTab, setActiveTab] = useState<
    "Billing" | "Credits" | "Plans" | "Entitlements"
  >("Billing");
  const totalMRR = mockTenants
    .filter((t) => t.mrr > 0)
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400">
            Control billing, AI credits, plan limits, and feature entitlement
            overrides.
          </p>
        </div>
        <div className="flex gap-2">
          ...
          {activeTab === "Credits" && (
            <Button className="bg-emerald-600 hover:bg-emerald-500 text-white border-transparent">
              <Zap className="w-4 h-4 mr-2" />
              Bulk Credit Grant
            </Button>
          )}
          {activeTab === "Plans" && (
            <Button className="bg-[#10B981] hover:bg-[#059669] text-white border-transparent">
      ...
      <div className="flex space-x-2 border-b border-white/10 mt-6">
        {(["Billing", "Credits", "Plans", "Entitlements"] as const).map(
          (tab) => (
            <button
              key={tab}
        ...
        {[
          {
            ...
            sub: `Across ${mockTenants.length} orgs`,
            color: "#10b981",
          },
          {
            label: "ARR",
          ...
          },
          {
            ...
            sub: "No churn this month",
            color: "#f59e0b",
          },
          {
            label: "Avg Plan MRR",
            value: `$${Math.round(totalMRR / Math.max(1, mockTenants.filter((t) => t.mrr > 0).length)).toLocaleString()}`,
            sub: "Per paying org",
            color: "#38bdf8",
          },
        ].map((stat) => (
          <div
            key={stat.label}
            className="bg-slate-900/60 rounded-2xl p-5"
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="bg-slate-900/50 border-b border-white/5">
              {["Organization", "Plan", "Credits", "Status", "MRR", ""].map(
                (h) => (
                  <th
                    key={h}
            ...
            {mockTenants.map((tenant) => (
              ...
              >
                ...
                <td className="px-6 py-4 text-sm text-indigo-400 font-bold">
                  {tenant.credits === -1 ? "∞" : `${tenant.credits} cr`}
                </td>
                <td className="px-6 py-4 text-sm">
                  <span className="text-emerald-400 px-2 py-1 bg-emerald-500/10 rounded font-bold text-xs uppercase">
                    Active
                  </span>
                </td>
...
function CreditsView() {
  ...
  const handleAdjust = async (direction: number) => {
    ...
    setSuccess("");
    try {
      await creditsService.adminAdjustBalance(
        orgId.trim(),
        qty,
        reason || (qty > 0 ? "Admin grant" : "Admin deduction"),
      );
      setSuccess(
        `${qty > 0 ? "Added" : "Removed"} ${Math.abs(qty)} credits to org ${orgId.slice(0, 8)}…`,
      );
    } catch (e: any) {
    ...
    <div className="grid lg:grid-cols-2 gap-6">
      <Card className="bg-slate-900/60 p-6 border border-indigo-500/15">
        ...
        <p className="text-slate-500 text-xs mb-5">
          Grant or deduct AI credits from any organization wallet.
        </p>
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Sales concession, onboarding gift..."
              className="w-full bg-slate-950 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-indigo-500/50 placeholder:text-slate-600"
            />
          </div>
      ...
      <Card className="bg-slate-900/60 p-6 border border-white/5">
        ...
        <p className="text-slate-500 text-xs mb-5">
          Standard costs per AI action (defined in{" "}
          <code className="text-indigo-400">types/billing.ts</code>).
        </p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {PLANS.map((p) => (
          <div key={p.id} onClick={() => setSelectedPlan(p.id)}>
            <Card
              className={`p-5 cursor-pointer transition-all duration-200 border-2 ${selectedPlan === p.id ? "bg-indigo-500/10 border-indigo-500 shadow-[0_0_30px_rgba(99,102,241,0.15)]" : "bg-slate-900/50 border-white/5 hover:border-white/20"}`}
            >
              <div className="flex justify-between items-start mb-2">
                <h3
                  className={`font-bold ${selectedPlan === p.id ? "text-indigo-400" : "text-white"}`}
                >
                  {p.name}
                </h3>
                {p.badge && (
                  <span className="text-xs text-amber-400">{p.badge}</span>
                )}
              </div>
              <div className="text-xl font-black text-white">
                {p.pricing.monthly
                  ? `$${p.pricing.monthly.toLocaleString()}`
                  : "Custom"}
                {p.pricing.monthly && (
              ...
              <div className="text-xs text-indigo-400 mt-1">
                {p.monthlyCredits === -1 ? "∞" : p.monthlyCredits} cr/mo
              </div>
            </Card>
          </div>
        ))}
          ...
          {[
            {
              ...
              value:
                plan.monthlyCredits === -1 ? "Unlimited" : plan.monthlyCredits,
              icon: Zap,
            },
            {
            ...
            },
            {
              ...
              value:
                plan.maxUsersPerWorkspace === -1
                  ? "Unlimited"
                  : plan.maxUsersPerWorkspace,
              icon: Users,
            },
            {
              label: "Recordings/Month",
              value:
                plan.maxRecordingsPerMonth === -1
                  ? "Unlimited"
                  : plan.maxRecordingsPerMonth,
              icon: Sparkles,
            },
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {plan.entitlements.map((key) => (
              <span
                key={key}
                className="px-2.5 py-1 text-xs font-medium bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-lg"
              >
                {key.replace(/_/g, " ")}
              </span>
            ))}
      ...
      <Card className="p-0 bg-slate-900/30 border border-white/5 overflow-hidden">
        <div className="p-4 border-b border-white/5 bg-slate-900/80">
          ...
          <p className="text-slate-500 text-xs mt-0.5">
            Force-grant or revoke features independent of plan tier.
            Org-specific.
          </p>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  >
                    <span
                      className={`absolute top-0.5 bottom-0.5 w-4 bg-white rounded-full transition-all ${ov === true ? "right-0.5 left-auto" : "left-0.5"}`}
                    />
                  </button>
                  <div>
                ...
                <div className="flex items-center gap-3">
                  <span
                    ...
                    style={{
                      background: `${tierColor[feat.tier]}15`,
                      color: tierColor[feat.tier],
                      border: `1px solid ${tierColor[feat.tier]}30`,
                    }}
                  >
                    {feat.tier}+
                  </span>
                  {ov !== undefined && (
                    <span
                      className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded border ${ov ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" : "bg-red-500/10 text-red-400 border-red-500/20"}`}
                    >
