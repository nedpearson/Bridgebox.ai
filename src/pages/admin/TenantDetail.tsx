        ...
        <div className="flex space-x-8 overflow-x-auto scrollbar-hide">
          {TABS.map((tab) => {
            ...
            return (
              ...
              >
                <Icon
                  className={`w-4 h-4 mr-2 ${activeTab === tab.id ? "text-indigo-500" : "text-slate-600"}`}
                />
                {tab.name}
                {activeTab === tab.id && (
                  <motion.div
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {activeTab !== "Overview" && (
              <Card
                glass
                className="p-12 flex flex-col items-center justify-center text-center bg-slate-900/40 border-dashed border-white/10 h-[300px]"
              >
                ...
                <p className="text-slate-400 font-medium">
                  Drilldown module for{" "}
                  <span className="text-white">{activeTab}</span> initialized.
                </p>
              </Card>
            )}
          </motion.div>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card glass className="bg-slate-900/60 border border-white/5 p-6">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-bold text-white flex items-center">
              <Activity className="w-5 h-5 mr-2 text-indigo-500" /> Execution
              Telemetry
            </h3>
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            ...
            <div className="p-4 bg-rose-500/5 rounded-xl border border-rose-500/20">
              <div className="text-xs text-rose-500 uppercase tracking-widest mb-1">
                Rate Limits Hit
              </div>
      ...
      <div className="lg:col-span-1 space-y-6">
        <Card
          glass
          className="bg-gradient-to-br from-indigo-500/10 to-transparent border border-indigo-500/20 p-6 flex flex-col h-full"
        >
          ...
          <div className="flex items-end space-x-2 mb-1">
            ...
            <span className="text-slate-400 text-sm mb-2 font-bold uppercase">
              / mo
            </span>
          </div>
          <p className="text-sm text-indigo-500 font-bold mb-6">
            Enterprise OS Plan
          </p>
