    ...
    try {
      ...
      const [
        ...
        supabase
          ...
          .eq("event_type", "blocked_by_policy"),
      ]);
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-slate-400 mb-1">
                  Ecosystem Value (Data Moat)
                </p>
                <h3 className="text-3xl font-black text-white">
                  {metrics?.benchmarksGenerated?.toLocaleString()}
                </h3>
          ...
          <Card className="p-6 bg-slate-900 border-slate-800">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="text-3xl font-black text-white">
                  ${metrics?.ecosystemRevenue?.toLocaleString()}
                </h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              {loading ? (
                ...
                <div className="space-y-4">
                  {topTemplates.map((tmpl, idx) => (
                    <div
                      key={tmpl.id}
                      className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-slate-800/50 rounded-xl border border-slate-700 hover:border-slate-600 transition-colors"
                    >
                      <div className="flex items-center gap-4 mb-3 sm:mb-0">
                        ...
                        <div>
                          <div className="text-white font-medium">
                            {tmpl.bb_templates?.name}
                          </div>
                          <div className="text-xs text-slate-400">
                            v{tmpl.version} • {tmpl.category.replace("_", " ")}
                          </div>
                        </div>
                      </div>
          ...
          <Card className="bg-slate-900 border-slate-800 shadow-xl overflow-hidden flex flex-col justify-center items-center p-12 text-center">
            ...
            <p className="text-slate-400 max-w-sm">
              The aggregate Moat engine is currently calculating structural
              system delays across the ecosystem. Next reporting cycle will
              isolate missing workflows blocking tenant revenue.
            </p>
          </Card>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <Card className="p-6 bg-slate-900 border-slate-800">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <h3 className="text-3xl font-black text-white">
                      {agentMetrics?.activeEngines?.toLocaleString()}
                    </h3>
              ...
              <Card className="p-6 bg-slate-900 border-slate-800">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <h3 className="text-3xl font-black text-white">
                      {(agentMetrics?.tokenConsumptionDaily / 1000000).toFixed(
                        2,
                      )}
                      M
                    </h3>
              ...
              <Card className="p-6 bg-slate-900 border-slate-800">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-slate-400 mb-1">
                      Global Policy Faults
                    </p>
                    <h3 className="text-3xl font-black text-white">
                      {agentMetrics?.policyFaults?.toLocaleString()}
                    </h3>
                    <div className="text-xs text-red-400 mt-2">
                      Interdicted anomalous behavior
                    </div>
                  </div>
