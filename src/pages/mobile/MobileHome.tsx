  ...
  return (
    <MobileLayout title="Field Ops">
      <div className="p-4 pb-32 space-y-6">
        ...
        {stats.pendingActions > 0 && (
          ...
          >
             <div className="flex items-center space-x-3">
               ...
               <div>
                  <h4 className="text-white font-semibold text-sm">Action Required</h4>
                  <p className="text-rose-200/80 text-xs">You have {stats.pendingActions} pending approvals</p>
               </div>
             </div>
             <div className="bg-rose-500 text-white text-xs font-bold px-3 py-1.5 rounded-full">Review</div>
          </motion.div>
        )}

        {/* KPI Grid */}
        <div>
          <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-3">Field Metrics</h3>
        ...
        <div>
          <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-3">Recent Activity</h3>
      ...
      */}
      <div className="fixed bottom-20 left-4 right-4 z-50 flex justify-center">
        <div className="bg-slate-900/80 backdrop-blur-2xl border border-white/10 p-2 rounded-full shadow-2xl flex items-center space-x-2">
           <DockButton icon={Camera} label="Photo" color="text-sky-400" onClick={() => navigate('/app/mobile/upload?camera=true')} />
           <DockButton icon={Mic} label="Note" color="text-indigo-400" onClick={() => {}} />
           <DockButton icon={MapPin} label="Check-In" color="text-emerald-400" onClick={() => {}} />
...
function ActivityCard({ icon: Icon, title, desc, time, status }: any) {
  return (
    <div className="flex items-center gap-4 p-4 bg-slate-900/40 backdrop-blur-md rounded-2xl border border-white/5 active:bg-white/5 transition-colors">
      <div className={`p-3 rounded-xl flex-shrink-0 ${status === 'offline' ? 'bg-amber-500/10 text-amber-500' : 'bg-slate-800 text-indigo-400'}`}>
        <Icon className="w-5 h-5" />
      </div>
      <div className="flex-1 min-w-0">
        <h4 className="text-white font-medium text-sm mb-0.5 truncate">{title}</h4>
