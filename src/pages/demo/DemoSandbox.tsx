...
export default function DemoSandbox() {
  ...
  const [status, setStatus] = useState<string>('Initializing sandbox environment...');
  ...
  useEffect(() => {
    async function provisionDemo() {
      ...
      try {
        ...
        const safeIndustry = (industry || 'general').toLowerCase().replace(/[^a-z0-z]/g, '');
        ...
        Logger.info('[Analytics] Instant Demo Sandbox Provisioned', { industry: safeIndustry });
  ...
  useEffect(() => {
     async function configureAndRoute() {
        if (profile && currentOrganization && currentOrganization.name !== `[DEMO] ${industry} Workspace`) {
           try {
              ...
              await DemoDataSeeder.seedWorkspace(currentOrganization.id, industry || 'general');
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4">
      ...
      >
        ...
        <h2 className="text-2xl font-bold text-white mb-2">Bridgebox Sandbox</h2>
        ...
        <div className="mt-8 pt-6 border-t border-slate-800">
           <p className="text-xs text-slate-500">
             Building a zero-latency, secure tenant mathematically isolated from production networks.
           </p>
        </div>
      </motion.div>
    </div>
