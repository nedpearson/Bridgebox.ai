        ...
        {STEPS.map((step) => {
          ...
          return (
            <div key={step.id} className="flex flex-col items-center gap-2">
              ...
              <span
                className={`text-[10px] font-bold uppercase tracking-wider ${isActive ? "text-indigo-300" : isPast ? "text-slate-400" : "text-slate-600"}`}
              >
                {step.title}
              </span>
            </div>
              ...
              {[
                ...
                <button
                  ...
                  className={`p-6 rounded-2xl border text-center transition-all ${
                    orgData.industry === ind
                      ? "bg-indigo-500/20 border-indigo-500 shadow-[0_0_20px_rgba(99,102,241,0.2)]"
                      : "bg-slate-900/40 border-slate-700/50 hover:bg-slate-800/80"
                  }`}
                >
                  <span
                    className={`font-medium ${orgData.industry === ind ? "text-indigo-300" : "text-slate-300"}`}
                  >
                    {ind}
                  </span>
                </button>
              ...
              {[
                {
                  id: "b2b_saas",
                  label: "B2B Services",
                  desc: "Retainers, milestones, and ongoing client operations",
                },
                {
                  id: "b2c_retail",
                  label: "B2C Retail/E-comm",
                  desc: "High-volume transactions, inventory, and POS",
                },
                {
                  id: "marketplace",
                  label: "Marketplace",
                  desc: "Matching buyers with sellers, commission tracking",
                },
              ].map((model) => (
                <button
                  key={model.id}
                  ...
                  className={`p-6 rounded-2xl border text-left transition-all ${
                    orgData.model === model.id
                      ? "bg-indigo-500/20 border-indigo-500"
                      : "bg-slate-900/40 border-slate-700/50 hover:bg-slate-800/80"
                  }`}
                >
                  <div
                    className={`font-bold text-lg mb-2 ${orgData.model === model.id ? "text-indigo-300" : "text-white"}`}
                  >
                    {model.label}
                  </div>
      ...
      case 5: {
        ...
        return (
          <div className="space-y-6">
            <h2 className="text-3xl font-bold text-white text-center mb-8">
              {currentStep === 3
                ? "Select Core Operational Modules"
                : currentStep === 4
                  ? "Which tools do you already use?"
                  : "Enable Copilot Intelligence Features"}
            </h2>
            <div className="grid md:grid-cols-2 gap-4">
              {options.map((opt) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`font-bold ${isSelected ? "text-indigo-300" : "text-slate-200"}`}
                    >
                      {opt}
                    </span>
                    {isSelected && (
        ...
        return (
          <div className="space-y-6 max-w-2xl mx-auto text-center">
            ...
            <p className="text-slate-400 mb-8">
              Do you have staff that need a native mobile workspace for camera
              uploads, GPS check-ins, or offline syncs?
            </p>
            <div className="flex justify-center gap-4">
              <button
                onClick={() => setOrgData({ ...orgData, mobile_fleet: true })}
                className={`px-8 py-4 rounded-xl border font-bold text-lg transition-all ${orgData.mobile_fleet ? "bg-sky-500 border-sky-400 text-white shadow-[0_0_20px_rgba(14,165,233,0.4)]" : "bg-slate-900/40 border-slate-700/50 text-slate-300"}`}
              >
                Yes, Enable Mobile
              </button>
              <button
                onClick={() => setOrgData({ ...orgData, mobile_fleet: false })}
                className={`px-8 py-4 rounded-xl border font-bold text-lg transition-all ${!orgData.mobile_fleet ? "bg-slate-700 border-slate-500 text-white" : "bg-slate-900/40 border-slate-700/50 text-slate-300"}`}
              >
                Not right now
              </button>
            </div>
      ...
      case 7:
        return (
          <div className="space-y-6 max-w-md mx-auto">
            ...
            <div>
              <label className="block text-slate-400 font-medium mt-6 mb-2">
                Team Size
              </label>
              <select
                value={orgData.team_size}
                onChange={(e) =>
                  setOrgData({ ...orgData, team_size: e.target.value })
                }
        ...
        return (
          <div className="space-y-6 max-w-3xl mx-auto">
            <div className="bg-slate-900/80 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl overflow-hidden relative">
              ...
              <p className="text-slate-400 mb-8 border-b border-white/5 pb-6">
                We have structured a completely bespoke system based on your
                unique requirements.
              </p>
              ...
              <div className="grid md:grid-cols-2 gap-8 relative z-10">
                <div className="space-y-4">
                  ...
                  <ul className="space-y-3">
                    <li className="flex gap-2 items-center text-slate-300">
                      ...
                      {orgData.industry} ({orgData.model}) Base Template
                    </li>
                    {orgData.modules.map((m) => (
                      ...
                      >
                        <CheckCircle2 className="w-4 h-4 text-indigo-400" /> {m}{" "}
                        Module
                      </li>
                    ))}
                    {orgData.ai_features.length > 0 && (
                      <li className="flex gap-2 items-center text-indigo-300 font-medium">
                        <Sparkles className="w-4 h-4" /> Cognitive AI Enabled
                      </li>
                    )}
                    {orgData.mobile_fleet && (
                      <li className="flex gap-2 items-center text-sky-300 font-medium">
                        <Smartphone className="w-4 h-4" /> Native Mobile
                        Dispatch Configured
                      </li>
                    )}
                  </ul>
                ...
                <div className="bg-slate-950/50 rounded-2xl p-6 border border-white/5">
                  <div className="text-sm font-bold text-slate-500 uppercase mb-4">
                  ...
                  <p className="text-xs text-slate-400 mb-6">
                    Billed annually. Adjustments can be made later.
                  </p>
                  <Button
                    onClick={handleNext}
                    className="w-full bg-white text-indigo-900 hover:bg-slate-200"
                  >
        ...
        return (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-xl text-slate-400 max-w-xl mb-12">
              Bridgebox is currently writing database permissions, binding{" "}
              {orgData.integrations.length} webhook listeners, and generating
              mobile UI routes dynamically.
            </p>
            ...
            {!loading ? (
              <Button
                ...
                size="lg"
                className="px-12 py-4 text-lg animate-bounce"
              >
                Launch My System
              </Button>
            ) : (
        ...
        {currentStep < 8 && (
          <div className="w-full max-w-3xl mx-auto py-8 flex justify-between items-center border-t border-slate-800/50 mt-12">
            ...
            <Button
              ...
              variant="primary"
              size="lg"
              className="px-8 shadow-[0_0_20px_rgba(59,130,246,0.3)]"
            >
