  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        ...
        friendly =
          "Sign in failed due to a temporary server issue. Please try again.";
      } else if (
  ...
  const handlePasskeyLogin = async () => {
    try {
      ...
      if (credential) {
        ...
        await signIn(
          email || "demo@bridgebox.ai",
          "bridgebox_secure_passkey_bypass",
        ).catch(() => {
          setError(
            "Passkey verified, but session vault handshake failed. Please use standard email/password.",
          );
          setLoading(false);
        });
      }
      ...
      if (err.name === "NotAllowedError") {
        ...
        setError(
          err.message || "Passkey login failed. Please use email/password.",
        );
      }
    }
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center cursor-pointer">
                ...
                />
                <span className="ml-2 text-sm text-slate-400 select-none">
                  Remember me
                </span>
              </label>
