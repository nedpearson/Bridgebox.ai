...
export default function SupportDesk() {
  ...
  const getSeverityColor = (severity?: string) => {
    switch (severity) {
      ...
      default:
        return "bg-slate-800 text-slate-500 border-slate-700"; // Pending AI
    }
  };
        ...
        {[
          ...
          return (
            <button
              key={tab.id}
              onClick={() =>
                setActiveTab(tab.id as "all" | "action_required" | "resolved")
              }
      ...
      {loading ? (
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-16 text-center">
          ...
          <p className="text-slate-500">
            No support vectors match the active pipeline filter.
          </p>
        </div>
      ) : (
            ...
            <tbody className="divide-y divide-slate-800">
              {filteredTickets.map((ticket) => (
                <tr
                  ...
                  onClick={() =>
                    navigate(
                      `/app/internal/recording-center/support/${ticket.id}`,
                    )
                  }
                ...
                >
                  ...
                  <td className="px-6 py-4 align-top">
                    {ticket.ai_processed_at ? (
                      <span
                        className={`px-2.5 py-1 rounded text-xs font-bold border capitalize ${getSeverityColor(ticket.ai_severity)}`}
                      >
                        {ticket.ai_severity}
                      </span>
                    ) : (
