...
import { Plus, LayoutTemplate, Briefcase, Settings, Save, Archive, Trash2, ArrowRight } from 'lucide-react';
    ...
    <div className="flex-1 max-w-7xl mx-auto w-full p-4 md:p-8 space-y-8">
       <div className="flex justify-between items-start mb-6">
          <div className="max-w-xl">
             <h1 className="text-3xl font-bold text-white mb-2">Template Studio</h1>
             <p className="text-slate-400">
               Architect, version, and manage the official Bridgebox Industry Packs and Business Model overlays.
             </p>
          </div>
          <Button onClick={() => {}} variant="primary" className="bg-indigo-500 hover:bg-indigo-600 border-none text-white">
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
         {templates.map(template => (
            <Card key={template.id} className="bg-slate-800/40 border-slate-700/50 hover:border-indigo-500/50 transition-colors flex flex-col justify-between">
               <div>
                  <div className="flex items-start justify-between mb-4">
                     ...
                     <span className={`px-2 py-1 rounded text-xs font-medium uppercase tracking-wider ${
                        template.status === 'published' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400'
                     }`}>
                        {template.status}
                     </span>
                  </div>
                  
                  <h3 className="text-lg font-semibold text-white mb-1 truncate">{template.name}</h3>
