    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Super Admin operational scratchpad and architecture decisions.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {notes.map((note) => (
          <div
            key={note.id}
            className="p-6 bg-slate-900 border border-slate-800 rounded-xl group relative hover:border-slate-700 transition-colors flex flex-col"
          >
            <div className="flex items-start justify-between mb-4">
              ...
              {note.is_pinned && (
                <Pin className="w-4 h-4 text-yellow-400 absolute top-6 right-6" />
              )}
            </div>
        ...
        {notes.length === 0 && !loading && !isCreating && (
          <div className="col-span-full py-16 text-center border border-dashed border-slate-800 rounded-xl">
            <h3 className="text-lg font-medium text-slate-300 mb-2">
              Build Journal is Empty
            </h3>
            <p className="text-slate-500">
              Document architectural decisions and operational records here.
            </p>
          </div>
        )}
      </div>
