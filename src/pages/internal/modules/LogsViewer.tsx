...
export default function LogsViewer() {
  ...
  const loadLogs = async () => {
    try {
      setLoading(true);
      const data = await commandCenterApi.listLogs(
        100,
        filterSeverity !== "all" ? { severity: filterSeverity } : undefined,
      );
      setLogs(data);
    } catch (err: any) {
      console.error(err);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Real-time system, API, and background worker event logs.
          </p>
        </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-4 mb-1 border-b border-slate-800/50 pb-2">
                      ...
                      <span className="text-slate-600 text-xs hidden group-hover:block transition-opacity opacity-50">
                        ID: {log.id.split("-")[0]}
                      </span>
                    </div>
                    <div className="text-slate-200 mt-2 whitespace-pre-wrap">
                      {log.message}
                    </div>
