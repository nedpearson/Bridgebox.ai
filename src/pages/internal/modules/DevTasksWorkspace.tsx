      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-slate-400">
            Internal evaluation layer mapping raw Support Telemetry into
      ...
      {loading ? (
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-16 text-center">
          ...
          <p className="text-slate-500">
            Pipeline is nominal. No outstanding task signatures detected.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredTasks.map((task) => (
            <div
              key={task.id}
              onClick={() =>
                navigate(`/app/internal/recording-center/dev-tasks/${task.id}`)
              }
            ...
            >
              ...
              <div className="flex items-start justify-between mb-4">
                <span
                  className={`px-2.5 py-1 rounded text-[10px] font-bold uppercase tracking-wider border ${getSeverityColor(task.severity)}`}
                >
                  {task.severity}
                </span>
