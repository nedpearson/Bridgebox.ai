...
import { BrainCircuit, Play, CheckCircle, Terminal, Copy, AlertOctagon, RefreshCw } from 'lucide-react';
...
export default function AgentControlRoom() {
  const [activeTab, setActiveTab] = useState<'matrix' | 'antigravity'>('matrix');
  ...
  const agents = [
    { name: 'Stack Discovery Agent', desc: 'Maps text to Integration Topologies', func: StackDiscoveryAgent.mapTopology },
    { name: 'Solution Architect', desc: 'Transcripts to DB Schema constraints', func: null },
    { name: 'Workflow Architect', desc: 'Intent to Automation Rules', func: null },
    { name: 'Integration Planner', desc: 'Payloads to Column Mappings', func: null },
    { name: 'Feature Ingestor', desc: 'Gaps to Antigravity Prompts', func: FeatureIngestionAgent.draftCodePrompt },
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">Orchestration observation deck for the 7 autonomous intelligence modules.</p>
        </div>
        <div className="flex bg-slate-900 rounded-lg p-1 border border-slate-800">
          <button onClick={() => setActiveTab('matrix')} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${activeTab === 'matrix' ? 'bg-indigo-500 text-white' : 'text-slate-400 hover:text-white'}`}>Matrix Status</button>
          <button onClick={() => setActiveTab('antigravity')} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${activeTab === 'antigravity' ? 'bg-indigo-500 text-white' : 'text-slate-400 hover:text-white'}`}>Antigravity Logs</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 border border-slate-800 rounded-xl bg-slate-900/50 p-4 space-y-3">
          <h3 className="text-sm font-bold text-slate-300 uppercase tracking-wider mb-4 border-b border-slate-800 pb-2">Active Neural Threads</h3>
          {agents.map(a => (
            <div key={a.name} className="flex items-center justify-between p-3 bg-slate-950 border border-slate-800/80 rounded-lg group">
              <div>
                <p className="text-sm font-semibold text-slate-200">{a.name}</p>
                <p className="text-xs text-slate-500">{a.desc}</p>
              </div>
              {a.func ? <CheckCircle className="w-4 h-4 text-emerald-500" /> : <div className="w-2 h-2 bg-slate-700 rounded-full" />}
            </div>
          ))}
        </div>
        ...
        <div className="lg:col-span-2 border border-slate-800 rounded-xl bg-slate-900/50 flex flex-col h-[600px] overflow-hidden">
          <div className="p-4 border-b border-slate-800 flex items-center gap-2 bg-slate-950">
            ...
            <h3 className="font-semibold text-white">Manual Override Terminal</h3>
          </div>
          <div className="p-4 space-y-4 flex-1 overflow-y-auto">
            
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-400">Dispatch Intent payload strings</label>
              <textarea 
                value={testInput}
            ...
            {output && (
              <div className="mt-6 border border-slate-800 rounded-lg overflow-hidden">
                <div className="bg-slate-950 px-4 py-2 flex items-center justify-between border-b border-slate-800">
                  <span className="text-xs font-mono text-slate-400">Agent Output Sequence</span>
                  {output?.data?.compiled_prompt && (
                    <button onClick={() => copyToClipboard(output.data.compiled_prompt)} className="text-xs flex items-center gap-1 text-emerald-400 hover:text-emerald-300 transition">
                      <Copy className="w-3 h-3" /> {copied ? 'Copied to IDE' : 'Copy Antigravity Prompt'}
                    </button>
                  )}
                </div>
