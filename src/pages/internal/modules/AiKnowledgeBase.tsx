      ...
      return (
        <div className="p-8 bg-red-950 min-h-screen text-white rounded-lg border border-red-500 w-full z-[9999] absolute top-0 left-0">
          <h2 className="text-2xl font-bold mb-4 flex items-center gap-2">
            <LayoutGrid /> AiKnowledgeBase Crashed!
          </h2>
          <p className="font-mono bg-black/50 p-4 rounded text-red-400">
                ...
                <div className="flex gap-2 mt-3 flex-wrap">
                  <span
                    className={`text-[10px] px-2 py-0.5 rounded-full ${
                      node.sourceOfTruth === "static"
                        ? "bg-blue-500/20 text-blue-400"
                        : node.sourceOfTruth === "dynamic_scan"
                  ...
                  {(node.visibility?.roles || []).slice(0, 2).map((r) => (
                    <span
                      key={r}
                      className="text-[10px] bg-slate-500/20 text-slate-300 px-2 py-0.5 rounded-full"
                    >
                      {r?.replace("_", " ") || "All"}
                    </span>
                  ))}
            ...
            {!selectedNode ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center opacity-50">
                ...
                <p className="text-sm text-slate-400 max-w-sm mt-2">
                  Select a capability node from the left to edit its
                  description. This updates exactly what the Copilot understands
                  about this feature.
                </p>
              </div>
            ) : (
                ...
                <div className="flex-1 space-y-6 overflow-y-auto pr-2">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-2">
                      This text is fed directly into the system prompt when the
                      Copilot parses intents related to this node. Be specific.
                    </p>
                  </div>
