...
export default function IntegrationHealth() {
  ...
  const getStatusDisplay = (status: string) => {
    switch (status) {
      case "healthy":
        return (
          <span className="flex items-center gap-2 px-3 py-1.5 bg-emerald-500/10 text-emerald-400 rounded-lg text-sm font-semibold border border-emerald-500/20 w-fit">
            <CheckCircle2 className="w-4 h-4" /> Healthy
          </span>
        );
      case "degraded":
        return (
          <span className="flex items-center gap-2 px-3 py-1.5 bg-amber-500/10 text-amber-400 rounded-lg text-sm font-semibold border border-amber-500/20 w-fit">
            <AlertTriangle className="w-4 h-4" /> Degraded
          </span>
        );
      case "failing":
        return (
          <span className="flex items-center gap-2 px-3 py-1.5 bg-red-500/10 text-red-500 rounded-lg text-sm font-semibold border border-red-500/20 w-fit">
            <XCircle className="w-4 h-4" /> Failing
          </span>
        );
      case "disconnected":
        return (
          <span className="flex items-center gap-2 px-3 py-1.5 bg-slate-500/10 text-slate-400 rounded-lg text-sm font-semibold border border-slate-500/20 w-fit">
            <ShieldAlert className="w-4 h-4" /> Disconnected
          </span>
        );
      default:
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Status of third-party API connections (Stripe, Resend, Supabase,
            LLMs).
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {events.map((ev) => (
          <div
            key={ev.id}
            className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl flex flex-col justify-between hover:border-slate-700 transition-colors"
          >
            <div>
            ...
            <div className="pt-4 border-t border-slate-800/50 mt-4 flex items-center justify-between text-xs text-slate-500 font-mono">
              <span>
                Last Sync:{" "}
                {ev.last_sync_at
                  ? new Date(ev.last_sync_at).toLocaleString()
                  : "Never"}
              </span>
