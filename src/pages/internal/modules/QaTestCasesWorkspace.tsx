...
export default function QaTestCasesWorkspace() {
  ...
  const filtered = tests.filter((t) => {
    if (
      activeTab === "draft_generated" &&
      t.status !== "draft_generated" &&
    ...
    <div className="space-y-8">
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-slate-400">
            Generative CI/CD test verification packs extracted from internal
            bugs and telemetry anomalies.
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center space-x-2 border-b border-slate-800">
        {[
          ...
          {
            id: "approved",
            label: "Active Verification Specs",
            icon: ShieldCheck,
          },
      ...
      {loading ? (
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-16 text-center">
          ...
          <h3 className="text-xl font-semibold text-white mb-2">
            No Verification Matrix Detected
          </h3>
          <p className="text-slate-500">
            Generative pipelines have not queued active staging packs.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((test) => (
            <div
              ...
              onClick={() =>
                navigate(
                  `/app/internal/recording-center/qa-test-cases/${test.id}`,
                )
              }
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <span
                  className={`px-2.5 py-1 rounded text-xs tracking-wider border bg-indigo-500/10 text-indigo-400 border-indigo-500/20`}
                >
