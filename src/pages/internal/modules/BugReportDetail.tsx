  ...
  const handleUpdateStatus = async (statusStr: any) => {
    ...
    try {
      ...
      const updated = await devQaAiApi.updateBugReport(bug.id, {
        status: statusStr,
        approved_for_build:
          statusStr === "approved" || statusStr === "confirmed",
      });
      setBug(updated);
    } catch (err) {
      console.error(err);
    ...
    <div className="space-y-6 max-w-5xl mx-auto pb-20">
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-start space-x-4">
          <button
            onClick={() =>
              navigate("/app/internal/recording-center/bug-reports")
            }
            ...
            <div className="flex items-center space-x-2 mb-1">
              ...
              <span
                className={`px-2.5 py-1 rounded text-xs font-mono uppercase tracking-wider ${
                  bug.status === "confirmed" || bug.status === "approved"
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between items-center py-2 border-b border-slate-800">
                ...
                <span className="text-blue-400 font-mono text-xs cursor-pointer hover:underline">
                  {bug.linked_support_ticket_id?.slice(0, 8) ||
                    bug.source_id.slice(0, 8)}
                </span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-slate-800">
              ...
              <div className="flex justify-between items-center py-2 border-b border-slate-800">
                ...
                <span className="text-red-400 uppercase tracking-wider text-xs font-bold">
                  {bug.severity}
                </span>
              </div>
            </div>
          ...
          {bug.similar_bug_refs && bug.similar_bug_refs.length > 0 && (
            <div className="bg-orange-500/10 border border-orange-500/20 rounded-xl p-5">
              ...
              <p className="text-xs text-orange-300">
                The generative pipeline isolated{" "}
                <strong>
                  {bug.similar_bug_refs.length} identical signatures
                </strong>
                . Potential massive scale regression in {bug.product_area}.
              </p>
            </div>
          )}
        </div>
