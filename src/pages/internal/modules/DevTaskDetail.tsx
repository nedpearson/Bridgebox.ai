...
export default function DevTaskDetail() {
  ...
  const handleReject = async () => {
    if (!task) return;
    if (!window.confirm("Reject this AI-generated task? It will be archived."))
      return;
    try {
      setLoading(true);
      const updated = await devTasksAiApi.updateTask(task.id, {
  ...
  const handleRegenerate = async () => {
    alert(
      "Initiating secure re-extraction of AI Task vectors based on Origin Linkage. This operation typically takes 15 seconds.",
    );
  };
              ...
              <span
                className={`px-2.5 py-1 rounded text-xs font-mono uppercase tracking-wider ${
                  task.status === "approved"
                    ...
                    : task.status === "sent_to_antigravity"
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-slate-300 p-3 bg-slate-800 rounded border border-slate-700">
                  {task.problem_statement || "N/A"}
                </p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-green-500 p-3 bg-green-500/10 rounded border border-green-500/20">
                    {task.expected_behavior || "N/A"}
                  </p>
                </div>
              </div>
              <div>
                <span className="block text-xs uppercase text-slate-500 font-bold tracking-wider mb-1">
                  Root Cause Hypothesis (AI Assessment)
                </span>
                <p className="text-amber-400 p-3 bg-amber-500/10 rounded border border-amber-500/20 italic">
                  {task.root_cause_hypothesis || "N/A"}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <pre className="text-slate-300 font-sans p-3 bg-slate-800 rounded border border-slate-700 whitespace-pre-wrap">
                  {task.proposed_acceptance_criteria || "N/A"}
                </pre>
              </div>
              <div>
                <span className="block text-xs uppercase text-slate-500 font-bold tracking-wider mb-1">
                  Implementation Scope Vectors
                </span>
                <pre className="text-slate-300 font-sans p-3 bg-slate-800 rounded border border-slate-700 whitespace-pre-wrap">
                  {task.proposed_implementation_notes || "N/A"}
                </pre>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between items-center py-2 border-b border-slate-800">
                ...
                <span className="text-blue-400 font-mono text-xs">
                  {task.linked_support_ticket_id || task.source_id.slice(0, 8)}
                </span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-slate-800">
                <span className="text-slate-500">Priority</span>
                <span className="text-orange-400 uppercase tracking-wider text-xs font-bold">
                  {task.priority}
                </span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-slate-800">
          ...
          {task.similar_task_refs && task.similar_task_refs.length > 0 && (
            <div className="bg-orange-500/10 border border-orange-500/20 rounded-xl p-5">
              ...
              <p className="text-xs text-orange-300">
                The generative pipeline isolated {task.similar_task_refs.length}{" "}
                identical signatures querying similar problem clusters within{" "}
                <strong>{task.product_area}</strong>. Proceed cautiously before
                merging.
              </p>
            </div>
          )}
