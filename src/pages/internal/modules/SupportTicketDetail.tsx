...
import { ArrowLeft, Play, Server, Clock, Wifi, LayoutDashboard, BrainCircuit, Activity, BookOpen, User, Shield, VideoOff, MessageSquare, Loader2, PlayCircle, CheckCircle, Code2, Bug, TestTube2 } from 'lucide-react';
...
export default function SupportTicketDetail() {
  ...
  const [sessionState, setSessionState] = useState<RTCPeerConnectionState | 'disconnected'>('disconnected');
  ...
  const handleGenerateBugReport = async () => {
    ...
    try {
      setIsAiProcessing(true);
      const bug = await devQaAiApi.generateBugReportFromSource(ticket, 'support_ticket');
  ...
  const handleGenerateQaTest = async () => {
    ...
    try {
      setIsAiProcessing(true);
      const qa = await devQaAiApi.generateQaTestCaseFromSource(ticket, 'support_ticket');
  ...
  const handleEscalate = async (type: string, flagField: string) => {
    ...
    try {
      ...
      alert(err.message || 'Failed to trigger engineering escalation transfer.');
    }
  };
  ...
  const handlePlayEvidence = async () => {
    ...
    try {
      const url = await supportTicketsApi.getRecordingUrl(ticket.recording_path);
      setViewingVideo(url);
    } catch (err: any) {
    ...
    <div className="space-y-6 pb-20">
          ...
          <div>
            ...
            <div className="text-slate-400 text-sm flex items-center space-x-4 mt-1">
              <span className="flex items-center space-x-1">
                ...
                <span>{(ticket as any).organizations?.name || 'Unknown Organization'}</span>
              </span>
              <span className="flex items-center space-x-1">
                <User className="w-4 h-4 text-slate-500" />
                <span>{(ticket as any).profiles?.full_name || 'Unknown User'}</span>
              </span>
              <span className="flex items-center space-x-1">
      ...
      <div className="grid grid-cols-3 gap-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
             ...
             <div className="p-6">
                <p className="text-slate-300 leading-relaxed whitespace-pre-wrap">
                  {ticket.description || 'No detailed issue description provided by the tenant.'}
                </p>
             </div>
             ...
             {ticket.recording_path && (
                <div className="p-4 bg-slate-900 border-t border-slate-800 flex justify-between items-center">
                   <div className="flex items-center space-x-2 text-slate-400 text-sm">
                      ...
                      <span>Has Attached Screen Upload ({((ticket.recording_size || 0) / 1024 / 1024).toFixed(1)}MB)</span>
                   </div>
                   <button 
             ...
             <div className="bg-indigo-500/10 p-4 border-b border-indigo-500/20 flex items-center justify-between">
               ...
               {ticket.ai_processed_at ? (
                 <span className="text-indigo-500/70 text-xs flex items-center space-x-1">
                   ...
                   <span>Orchestrated at {new Date(ticket.ai_processed_at).toLocaleTimeString()}</span>
                 </span>
               ) : (
                 <button 
                 ...
                 >
                   {isAiProcessing ? <Loader2 className="w-3 h-3 animate-spin" /> : <Activity className="w-3 h-3" />}
             ...
             {ticket.ai_summary ? (
               <div className="p-6 space-y-4">
                  <div className="bg-slate-950/50 p-4 rounded-lg border border-slate-800/50">
                    <h4 className="text-slate-400 text-xs uppercase tracking-wider font-semibold mb-2">Architectural Summary</h4>
                    <p className="text-slate-200">{ticket.ai_summary}</p>
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                     <div className="bg-slate-950/50 p-3 rounded-lg border border-slate-800/50">
                       <h4 className="text-slate-400 text-xs uppercase tracking-wider font-semibold mb-1">Determined Severity</h4>
                       <div className="flex items-center space-x-2">
                          <span className={`w-2 h-2 rounded-full ${ticket.ai_severity === 'high' || ticket.ai_severity === 'critical' ? 'bg-red-500' : 'bg-amber-500'}`} />
                          <span className="text-white capitalize">{ticket.ai_severity}</span>
                       </div>
                     </div>
                     <div className="bg-slate-950/50 p-3 rounded-lg border border-slate-800/50">
                       <h4 className="text-slate-400 text-xs uppercase tracking-wider font-semibold mb-1">Probable Product Area</h4>
                       <span className="text-white">{ticket.ai_product_area}</span>
                     </div>
                  </div>

                  <div className="bg-slate-950/50 p-4 rounded-lg border border-slate-800/50">
                    <h4 className="text-slate-400 text-xs uppercase tracking-wider font-semibold mb-2">Recommended Action Protocol</h4>
                    <p className="text-slate-200">{ticket.ai_recommended_action}</p>
                  </div>
               </div>
             ) : (
               <div className="p-8 text-center">
                 <BrainCircuit className="w-8 h-8 text-slate-700 mx-auto mb-3" />
                 <p className="text-slate-500 text-sm">Issue has not yet been processed by the Intelligence orchestrator.</p>
               </div>
             )}
          </div>
        ...
        <div className="space-y-6">
          ...
          {/* Live WebRTC Block */}
          {ticket.session_code && (
            <div className={`border rounded-xl p-5 ${sessionState === 'connected' ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-slate-900 border-slate-800'}`}>
               <div className="flex items-center space-x-2 mb-4">
                 <Shield className={`w-5 h-5 ${sessionState === 'connected' ? 'text-emerald-400' : 'text-purple-400'}`} />
                 <h3 className="text-white font-semibold">Active Screen Assist Hook</h3>
               </div>
               <p className="text-slate-400 text-xs mb-4">
                 Tenant has actively broadcasted an encrypted peer string mapping to their viewport context.
               </p>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
             ...
             <p className="text-slate-400 text-xs mb-4">
               Transform this support artifact natively into an active internal engineering target payload.
             </p>
             <div className="space-y-2">
                ...
                >
                  <span className="flex items-center space-x-2">
                    {isAiProcessing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Code2 className="w-4 h-4" />}
                ...
                <button 
                  onClick={() => handleEscalate('Product Feature Gap', 'feature_gap_flag')}
                ...
                <button 
                  onClick={() => handleEscalate('Internal Development Focus', 'build_candidate_flag')}
                  className="w-full text-left px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded border border-slate-700 transition-colors text-sm flex items-center justify-between"
                >
               ...
               <div className="bg-slate-950 p-4 flex justify-between items-center border-b border-slate-800">
                  <div className="flex items-center space-x-4">
                     ...
                     <span className={`px-2 py-0.5 rounded text-xs font-semibold border ${
                        sessionState === 'connected' ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' : 
                        'bg-amber-500/20 text-amber-400 border-amber-500/30 animate-pulse'
                     } uppercase tracking-wider`}>
                        {sessionState}
                     </span>
                  </div>
                  <button onClick={handleEndSession} className="text-red-400 hover:text-red-300 hover:bg-red-500/10 px-3 py-1 rounded-md text-sm transition-colors border border-red-500/20 font-medium">
                    Terminate
                  </button>
               </div>
               <div className="aspect-video bg-black flex items-center justify-center relative">
                 ...
                 {sessionState !== 'connected' && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-900/80 backdrop-blur-sm">
                       ...
                       <p className="text-slate-400 font-medium">Establishing secure Handshake...</p>
                    </div>
                 )}
               </div>
             </motion.div>
      ...
      <AnimatePresence>
        {viewingVideo && (
           <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
             <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setViewingVideo(null)} />
             <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-2xl max-w-5xl w-full">
               <div className="bg-slate-950 p-4 flex justify-between items-center border-b border-slate-800">
                  <h3 className="text-white font-medium flex items-center space-x-2">
                    <Play className="w-4 h-4 text-blue-400" />
                    <span>Evidence Player</span>
                  </h3>
                  <button onClick={() => setViewingVideo(null)} className="text-slate-400 hover:text-white px-3 py-1 bg-slate-800 rounded-md text-sm">Close</button>
               </div>
               <div className="aspect-video bg-black flex items-center justify-center">
                 <video src={viewingVideo} controls autoPlay className="w-full h-full object-contain" />
               </div>
             </motion.div>
           </div>
        )}
