...
export default function ClientSupport() {
  ...
  const loadTickets = async () => {
    ...
    try {
      setLoading(true);
      const data = await supportService.getOrganizationTickets(currentOrganization.id);
      setTickets(data);
    } catch (error) {
  ...
  const handleSubmitTicket = async () => {
    if (!currentOrganization || !newTicket.title.trim() || !newTicket.description.trim()) return;
