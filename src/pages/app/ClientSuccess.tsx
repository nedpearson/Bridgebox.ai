...
export default function ClientSuccess() {
  ...
  const [filterRisk, setFilterRisk] = useState<"all" | "at_risk" | "healthy">(
    "all",
  );
  ...
  const stats = [
    ...
    },
    {
      ...
      value: clients.filter((c) => (c.health_score?.overall_score || 0) >= 80)
        .length,
      icon: TrendingUp,
          ...
          {stats.map((stat) => {
            ...
            return (
              <div
                ...
                onClick={
                  isClickable
                    ? () => setFilterRisk(stat.filterVal as any)
                    : undefined
                }
                className={
                  isClickable
                    ? "block cursor-pointer transition-transform hover:-translate-y-1"
                    : ""
                }
              >
                <Card
                  glass={!isActive}
                  className={`h-full ${isActive ? "bg-white/10 border-white/20" : "hover:border-white/20 transition-colors"}`}
                >
            ...
            <div className="space-y-4">
              {filteredClients.map((client) => (
                <Link
                  ...
                  to={`/app/client-success/${client.organization_id}`}
                >
                  <motion.div
                    ...
                    <div className="flex items-center justify-between gap-6">
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap items-center gap-4 text-sm text-slate-400">
                          <span>
                            {client.active_projects} active project
                            {client.active_projects !== 1 ? "s" : ""}
                          </span>
                          <span>•</span>
                          <span>
                            {client.open_support_tickets} open ticket
                            {client.open_support_tickets !== 1 ? "s" : ""}
                          </span>
                          {client.last_interaction && (
                            <>
                              ...
                              <span>
                                Last contact:{" "}
                                {new Date(
                                  client.last_interaction.interaction_date,
                                ).toLocaleDateString()}
                              </span>
                            </>
                          )}
                      ...
                      <div className="flex items-center gap-6">
                        {client.health_score && (
                          <div
                            className={`flex flex-col items-center justify-center w-20 h-20 rounded-xl border ${getHealthBg(client.health_score.overall_score)}`}
                          >
                            <div
                              className={`text-2xl font-bold ${getHealthColor(client.health_score.overall_score)}`}
                            >
