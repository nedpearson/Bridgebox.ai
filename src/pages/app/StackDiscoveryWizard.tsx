...
import { Box, Plus, Network, Cpu, ArrowRight, Loader2, Link2, Sparkles, CheckCircle } from 'lucide-react';
...
- "complexity": strictly "low", "medium", or "high"
      ...
      const response = await provider.complete({
        messages: [
          { role: 'system', content: 'You are an Integration Architect mapping user tech stacks into Bridgebox topologies natively. Output strictly raw JSON array objects.' },
    ...
    <div className="max-w-5xl mx-auto py-12 px-4 sm:px-6">
      <div className="text-center mb-12">
        ...
        <p className="text-slate-400 max-w-2xl mx-auto">
          Paste the list of software your business currently uses. Bridgebox AI will autonomously map your
          data architecture and generate the exact integration points required to bridge the gaps.
        </p>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          {mappedStack.length === 0 && !isDiscovering && (
             <Card glass className="p-16 flex flex-col items-center justify-center text-center border-dashed border-slate-700">
                <Box className="w-16 h-16 text-slate-800 mb-6" />
                <h3 className="text-xl font-bold text-white mb-2">Awaiting Stack Input</h3>
                <p className="text-slate-500 max-w-md">
                  Enter your existing software suite on the left, and the Artificial Intelligence 
                  will visually map out how Bridgebox replaces your manual gaps.
                </p>
             </Card>
          )}
          ...
          {isDiscovering && (
             <div className="h-full min-h-[400px] flex flex-col items-center justify-center space-y-6">
                ...
                <p className="text-indigo-400 animate-pulse font-medium">Scanning Integration Endpoints...</p>
             </div>
          )}
          ...
          {!isDiscovering && mappedStack.length > 0 && (
             <div className="space-y-6">
               <div className="flex items-center justify-between mb-2">
                   <h3 className="text-xl font-bold text-white">Proposed Orchestration Map</h3>
               ...
               <div className="space-y-4">
                 {mappedStack.map((tool, index) => (
                    <motion.div
                       ...
                       key={index}
                    >
                       <Card glass className="p-5 flex items-start gap-4 hover:border-indigo-500/30 transition-colors border-slate-700/50">
                          <div className={`mt-1 flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center ${
                             tool.complexity === 'high' ? 'bg-rose-500/20 text-rose-400' :
                             tool.complexity === 'medium' ? 'bg-amber-500/20 text-amber-400' :
                          ...
                          <div className="flex-1 min-w-0">
                             <div className="flex items-center justify-between mb-1">
                                <h4 className="text-white font-bold text-lg leading-none">{tool.name}</h4>
                             ...
                             <div className="flex items-center gap-3 mt-3 mb-3">
                                <span className={`text-xs font-medium px-2 py-1 rounded bg-slate-900 border border-slate-800 ${
                                   tool.integrationDataFlow === 'bidirectional' ? 'text-indigo-400' :
                                   tool.integrationDataFlow === 'inbound' ? 'text-emerald-400' : 'text-amber-400'
                                }`}>
                             ...
                             <div className="bg-slate-900/50 rounded-lg p-3 text-sm text-slate-300 border border-slate-800">
                                <span className="font-semibold text-white mb-1 block">Bridgebox Orchestration Value:</span>
                                {tool.bridgeboxValue}
                             </div>
                          </div>
                       </Card>
