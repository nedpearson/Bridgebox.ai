...
export default function AiTemplateWizard() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const previousContext = generatedTemplate
        ? JSON.stringify(generatedTemplate)
        : undefined;
      const blueprint = await aiTemplateGenerator.generateFromPrompt(
        prompt,
      ...
      <div className="flex items-center space-x-4 mb-6">
        ...
        <div>
          ...
          <p className="text-slate-400">
            Transform natural language into a strictly compliant Bridgebox
            schema mapping.
          </p>
        </div>
      </div>
              ...
              {isGenerating && (
                <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm flex flex-col items-center justify-center rounded-xl z-10">
                  ...
                  <div className="text-indigo-300 font-medium">
                    Antigravity Intelligence Engine is designing your
                    architecture...
                  </div>
                  <div className="text-slate-500 text-sm mt-2">
                    Mapping entities, chaining logic graphs, synthesizing
                    internal models.
                  </div>
                </div>
              )}
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h2 className="text-xl font-bold text-white">
                      Generative Schema Success
                    </h2>
                    <p className="text-indigo-200 text-sm">
                      Validating configuration payload against master OS
                      constraints... Passed.
                    </p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div className="bg-slate-900 border border-white/5 p-4 rounded-xl text-center">
                  <div className="text-2xl font-bold text-blue-400">
                    {generatedTemplate.configuration_payload.entities.length}
                  </div>
                ...
                <div className="bg-slate-900 border border-white/5 p-4 rounded-xl text-center">
                  <div className="text-2xl font-bold text-purple-400">
                    {generatedTemplate.configuration_payload.workflows.length}
                  </div>
                  <div className="text-xs text-slate-400 uppercase">
                    Workflows
                  </div>
                ...
                <div className="bg-slate-900 border border-white/5 p-4 rounded-xl text-center">
                  <div className="text-2xl font-bold text-emerald-400">
                    {generatedTemplate.configuration_payload.forms.length}
                  </div>
                  <div className="text-xs text-slate-400 uppercase">Forms</div>
                </div>
                <div className="bg-slate-900 border border-white/5 p-4 rounded-xl text-center">
                  <div className="text-2xl font-bold text-amber-400">
                    {generatedTemplate.configuration_payload.ai_agents.length}
                  </div>
