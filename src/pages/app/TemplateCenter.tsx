...
export default function TemplateCenter() {
  ...
  const [filter, setFilter] = useState<
    "all" | "industry_pack" | "business_overlay" | "ai_generated"
  >("all");
          ...
          <div className="relative overflow-hidden bg-gradient-to-br from-indigo-900 via-indigo-950 to-slate-900 border border-indigo-500/30 rounded-2xl p-8">
            ...
            <div className="relative z-10 max-w-2xl">
              ...
              <h2 className="text-3xl font-bold text-white mb-4">
                Describe your perfect operational system. Watch it build itself.
              </h2>
              <p className="text-indigo-200 text-lg mb-8 leading-relaxed">
                Skip the browsing. Tell our Intelligence Engine what industry
                you're in, upload an SOP, or describe your unique business
                model. Bridgebox will custom-architect the entities, workflows,
                forms, and permission roles for you.
              </p>
              <div className="flex flex-wrap items-center gap-4">
                <Button
                  onClick={() => navigate("/app/templates/ai-wizard")}
                  variant="primary"
                  size="lg"
        ...
        {loading ? (
          ...
          <div className="lg:col-span-3 bg-slate-800/50 border border-slate-700/50 p-12 rounded-xl text-center">
            ...
            <p className="text-slate-400">
              Expand your search query or generate one via AI.
            </p>
          </div>
        ) : (
            ...
            >
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-start">
                  <div className="w-12 h-12 rounded-xl bg-slate-900 border border-white/10 flex items-center justify-center text-indigo-400">
                    {template.category === "industry_pack" ? (
                      <Briefcase className="w-6 h-6" />
                    ) : template.category === "business_overlay" ? (
                ...
                <div>
                  <h3 className="text-xl font-bold text-white mb-1 pr-24 line-clamp-1">
                    {template.name}
                  </h3>
                  <div className="flex items-center space-x-2 text-xs font-bold mb-3">
                    <span className="text-indigo-400 capitalize tracking-wide">
                    ...
                    <div className="flex flex-col space-y-2">
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="text-slate-300 font-bold">
                          {template.modules?.length || 0} Included
                        </span>
                      </div>
                      <div className="flex gap-2 flex-wrap">
                        {template.modules
                          ...
                          .map((mod: any, i: number) => (
                            <span
                              key={i}
                              className="text-[10px] bg-slate-800 text-slate-300 px-2 py-1 rounded-md border border-slate-700"
                            >
                              {mod.name}
                            </span>
                          ))}
                        {(template.modules?.length || 0) > 3 && (
                          <span className="text-[10px] bg-slate-800/50 text-slate-500 px-2 py-1 rounded-md border border-transparent">
                      ...
                      <div className="flex items-center justify-between bg-indigo-500/10 border border-indigo-500/20 px-3 py-2 rounded-lg">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-xs text-indigo-300 font-medium">
                            {template.ai_agents.length} Dedicated Agents
                          </span>
                        </div>
                      </div>
                    )}
              ...
              <div className="pt-4 flex flex-col space-y-2 text-xs text-slate-500 mt-auto">
                ...
                {template.target_personas &&
                  template.target_personas.length > 0 && (
                    <div className="flex flex-wrap items-center gap-1.5 pb-2">
                      <span className="font-semibold text-slate-400 tracking-wider text-[10px] uppercase">
                        Best For:
                      </span>
                      {template.target_personas.map(
                        (persona: string, idx: number) => (
                          <span
                            key={idx}
                            className="bg-white/5 px-2 rounded-md"
                          >
                            {persona}
                          </span>
                        ),
                  ...
                  {template.billing_rules?.base_plan_requirement ? (
                    <div className="flex items-center space-x-1 font-bold bg-amber-500/10 text-amber-500 px-2 py-1 rounded-lg border border-amber-500/20">
                      ...
                      <span className="capitalize">
                        {template.billing_rules.base_plan_requirement} Required
                      </span>
                    </div>
                  ) : (
