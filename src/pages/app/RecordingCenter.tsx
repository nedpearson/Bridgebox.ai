...
import { 
  ...
  PlayCircle, Plus, Sparkles, MessageSquare, LayoutTemplate,
...
export default function RecordingCenter() {
  ...
  const handleStartScreen = () => setState('recording_screen');
  const handleStartVoice = () => setState('recording_voice');
  const handleStop = () => setState('reviewing');
  const handleCancel = () => { setState('idle'); setNotes(""); };

  return (
    <div className="flex-1 overflow-y-auto w-full mb-24 lg:mb-0 pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        
        {/* Header Phase 5 / 11 Typography enforcement */}
        ...
        {/* Active Recording UI (Dynamic State) */}
        <AnimatePresence>
              ...
              <Card glass className="p-8 border-indigo-500/30 shadow-2xl space-y-6">
                ...
                <div className="grid md:grid-cols-2 gap-8">
                  <div className="aspect-video bg-slate-900 rounded-xl border border-slate-800 flex items-center justify-center shadow-inner relative overflow-hidden group hover:border-indigo-500/50 transition-colors">
                    <PlayCircle className="w-16 h-16 text-indigo-500/50 group-hover:scale-110 group-hover:text-indigo-400 transition-all cursor-pointer" />
                  </div>
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-col gap-3 pt-2">
                       <Button variant="primary" className="w-full flex justify-center !bg-indigo-600 hover:!bg-indigo-500 shadow-md glow-effect" onClick={() => { setState('idle'); }}>
                         <Terminal className="w-4 h-4 mr-2" /> Mark as "Build Request"
                       </Button>
                       <div className="grid grid-cols-2 gap-3">
                         <Button variant="outline" className="flex justify-center" onClick={() => { setState('idle'); }}>
                           <Save className="w-4 h-4 mr-2" /> Save Draft
                         </Button>
                         <Button variant="outline" className="flex justify-center" onClick={() => { setState('idle'); }}>
                           <UploadCloud className="w-4 h-4 mr-2" /> Send File
                         </Button>
                       </div>
                    </div>
                  </div>
        ...
        {/* Existing Library (Clean Table UX) */}
        <div>
