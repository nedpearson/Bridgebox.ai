...
export default function DocumentCenter() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await documentIntelligenceService.executeIntelligencePipeline(
        newDoc.id,
  ...
  const handleApproveIntelligence = async (doc: any, proposedOrg: any) => {
    try {
      ...
      // 3. Reload
      setDocuments((docs) =>
        docs.map((d) => (d.id === doc.id ? { ...d, status: "completed" } : d)),
      );
    } catch (err) {
  ...
  return (
    <>
      <AppHeader
        title="Document AI Center"
        subtitle="Intelligent ingestion, classification, and routing"
      />
            ...
            {reviewQueue.length === 0 ? (
              <Card className="p-12 text-center bg-slate-800/30 border-dashed">
                ...
                <p className="text-slate-400">
                  All intake documents have been successfully classified and
                  routed.
                </p>
              </Card>
            ) : (
              reviewQueue.map((doc) => {
                ...
                return (
                  <Card
                    key={doc.id}
                    className="p-0 overflow-hidden border-amber-500/30"
                  >
                        ...
                        <div>
                          ...
                          <div className="flex items-center space-x-2 text-xs text-slate-400 mt-1">
                            <span className="uppercase tracking-wider font-semibold text-indigo-400">
                              {ai?.raw_classification?.document_type ||
                                doc.document_type}
                            </span>
                            <span>•</span>
                            <span>
                              ...
                              <span
                                className={
                                  ai?.system_confidence < 85
                                    ? "text-amber-400"
                                    : "text-emerald-400"
                                }
                              >
                                {ai?.system_confidence || 0}%
                              </span>
                            </span>
                          </div>
                        </div>
                    ...
                    <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div>
                        <h5 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-4">
                          Extracted Intelligence
                        </h5>
                        <p className="text-slate-300 text-sm italic mb-4 border-l-2 border-indigo-500 pl-4 py-1 bg-slate-800/30">
                          "
                          {ai?.raw_classification?.ai_summary ||
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="bg-slate-800/50 p-3 rounded-lg border border-slate-700">
                            ...
                            <p className="text-white font-medium">
                              {ai?.raw_classification?.extracted_metadata?.amounts?.join(
                                ", ",
                              ) || "None"}
                            </p>
                          </div>
                          <div className="bg-slate-800/50 p-3 rounded-lg border border-slate-700">
                            ...
                            <p className="text-white font-medium">
                              {ai?.raw_classification?.extracted_metadata
                                ?.invoice_number || "None"}
                            </p>
                          </div>
                        </div>
                            ...
                            <div className="flex items-center space-x-3">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-slate-400">
                                  Matched via literal text inclusion.
                                </p>
                              </div>
                            </div>
                            <Button
                              size="sm"
                              onClick={() =>
                                handleApproveIntelligence(doc, orgMatch)
                              }
                            >
                              Approve Connection
                            </Button>
        ...
        {!loading && activeTab === "cabinet" && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {cabinetDocs.map((doc) => (
              <Card
                key={doc.id}
                className="p-5 flex flex-col items-center text-center hover:border-indigo-500/50 transition-colors"
              >
                <div className="w-16 h-16 bg-slate-800 rounded-2xl flex items-center justify-center mb-4 text-indigo-400">
                  <File className="w-8 h-8" />
                </div>
                <h4
                  className="text-white font-medium text-sm line-clamp-1 mb-1"
                  title={doc.file_name}
                >
                  {doc.file_name}
                </h4>
