...
export default function ImplementationDetail() {
  ...
  const [implementation, setImplementation] =
    useState<ImplementationDetailType | null>(null);
  ...
  const loadImplementation = async () => {
    ...
    try {
      ...
      setError("");
      const data =
        await implementationService.getImplementationByProjectId(projectId);

      if (!data) {
        const impl =
          await implementationService.createImplementation(projectId);
  ...
  const getCategoryProgress = (category: ChecklistCategory): number => {
    ...
    const categoryItems = implementation.checklists.filter(
  ...
  const getOverallProgress = (): number => {
    ...
    const completed = implementation.checklists.filter(
          ...
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
            <div>
              ...
              <p className="text-slate-400">
                {implementation.project?.organization?.name}
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <DeploymentReadinessBadge
                readiness={implementation.deployment_readiness}
              />
      ...
      <div className="p-4 md:p-8 space-y-8">
        <div className="grid lg:grid-cols-3 gap-6">
          <Card glass className="p-6">
            ...
            {implementation.environments.find(
              (e) => e.environment_type === "staging",
            ) ? (
          ...
          <Card glass className="p-6">
            ...
            {implementation.environments.find(
              (e) => e.environment_type === "production",
            ) ? (
              <div className="space-y-3">
                <EnvironmentStatusBadge
                  status={
          ...
          {implementation.go_live_date && (
            <Card glass className="p-6">
              ...
              <p className="text-lg text-white font-bold">
                {new Date(implementation.go_live_date).toLocaleDateString(
                  "en-US",
                  {
                    weekday: "long",
                    year: "numeric",
        ...
        {implementation.risks.filter((r) => r.status !== "resolved").length >
          0 && (
        ...
        <div className="space-y-6">
          ...
          {(Object.keys(CATEGORY_LABELS) as ChecklistCategory[]).map(
            (category) => {
              const categoryItems = implementation.checklists.filter(
                (item) => item.category === category,
              );
