...
const AUTONOMY_LEVELS = [
  {
    ...
    icon: Eye,
    description:
      "Digital assistant only analyzes and reports. Maps contextual data but proposes no operations.",
  ...
  },
  {
    ...
    icon: ShieldCheck,
    description:
      "Safest operational mode. Drafts actions (e.g. emails) for your explicit approval in the Queue.",
    color: "text-blue-400",
  },
  {
    value: "level_2",
    ...
    icon: Cpu,
    description:
      "Routinely executes low-risk safe workflows automatically. Pushes high-risk events to the Queue.",
  ...
  },
  {
    ...
    icon: Zap,
    description:
      "Maximum delegation. Executes all operations natively within fixed policy engine parameters.",
...
export default function AIAssistants() {
  ...
  const loadTelemetry = async () => {
    try {
      ...
      const drafted = logs.filter(
        (l) => l.event_type === "drafted_for_review",
      ).length;
  ...
  const updateAutonomy = async (tenantAgentId: string, level: string) => {
    try {
      ...
      console.error(err);
      alert(
        "Failed to update execution rules. Ensure you have Admin permissions.",
      );
    } finally {
      setUpdatingId(null);
    }
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center">
        ...
        <p className="text-slate-400 mt-4 text-sm font-medium tracking-wide">
          Loading active workforce telemetry...
        </p>
      </div>
    );
  }
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <Card className="p-6 bg-slate-900 border-slate-800">
              ...
              <div className="text-3xl font-black text-white">
                {metrics.hoursSaved}{" "}
                <span className="text-lg text-slate-500 font-normal">hrs</span>
              </div>
            </Card>
            ...
            <Card className="p-6 bg-slate-900 border-slate-800">
              ...
              <div className="text-3xl font-black text-white">
                $
                {metrics.revenueImpact.toLocaleString(undefined, {
            ...
            <Card className="p-6 bg-indigo-500/5 border-indigo-500/20 flex flex-col items-start justify-center">
              ...
              <p className="text-slate-400 text-xs leading-relaxed">
                Actions natively blocked if executing beyond configured
                autonomous policies across your active fleet.
              </p>
            </Card>
          </div>
        )}
        ...
        {tenantAgents.length === 0 ? (
          <Card className="p-16 border-slate-800 bg-slate-900/50 flex flex-col items-center justify-center text-center">
            ...
            <h2 className="text-2xl font-bold text-white mb-3">
              No AI Assistants Deployed
            </h2>
            <p className="text-slate-400 max-w-md mb-8">
              Your organization has no digital workers installed. Visit the
              Ecosystem Marketplace to provision templates specialized for your
              industry.
            </p>
            <button
              onClick={() =>
                navigate("/app/marketplace", {
                  state: { category: "ai_agent" },
                })
              }
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {tenantAgents.map((ta) => {
                ...
                const LevelIcon =
                  AUTONOMY_LEVELS.find((l) => l.value === ta.autonomy_level)
                    ?.icon || Lock;
                const levelColor =
                  AUTONOMY_LEVELS.find((l) => l.value === ta.autonomy_level)
                ...
                return (
                  <Card
                    key={ta.id}
                    className={`bg-slate-900 overflow-hidden relative transition-all ${ta.status === "paused" ? "border-slate-800 opacity-60" : "border-slate-700 hover:border-slate-600 shadow-xl"}`}
                  >
                    {ta.status === "paused" && (
                    ...
                    <div className="p-6">
                      <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center gap-4">
                          <div
                            className={`p-4 rounded-2xl border ${ta.status === "active" ? "bg-indigo-500/10 border-indigo-500/30" : "bg-slate-800 border-slate-700"}`}
                          >
                            <Bot
                              className={`w-8 h-8 ${ta.status === "active" ? "text-indigo-500" : "text-slate-500"}`}
                            />
                          </div>
                          <div>
                            ...
                            <p className="text-sm font-medium text-slate-400 capitalize tracking-wide">
                              {agent.category.replace("_", " ")} Focus
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between mb-4">
                          <div>
                            ...
                            <p className="text-xs text-slate-400">
                              Controls this assistant's allowable independence
                              logic.
                            </p>
                          </div>
                          <div
                            className={`flex items-center px-3 py-1.5 rounded-lg border bg-slate-900 ${levelColor.replace("text-", "border-").replace("400", "500/30")} ${levelColor}`}
                          >
                            <LevelIcon className="w-4 h-4 mr-2" />
                            <span className="text-xs font-bold tracking-wide">
                              {
                                AUTONOMY_LEVELS.find(
                                  (l) => l.value === ta.autonomy_level,
                                )?.label
                              }
                            </span>
                          </div>
                          ...
                          {AUTONOMY_LEVELS.map((level) => {
                            ...
                            return (
                              <button
                                key={level.value}
                                onClick={() =>
                                  updateAutonomy(ta.id, level.value)
                                }
                                disabled={
                                  updatingId === ta.id || ta.status !== "active"
                                }
                              ...
                              >
                                <level.icon
                                  className={`w-4 h-4 mb-2 mx-auto md:mx-0 ${isActive ? level.color : "text-slate-500"}`}
                                />
                                <div
                                  className={`text-xs font-bold leading-tight text-center md:text-left ${isActive ? "text-white" : "text-slate-400"}`}
                                >
                                  {level.label}
                                </div>
                              </button>
