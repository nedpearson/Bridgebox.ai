...
import { getMarketSignals, getSignalScores, getMarketOpportunities } from '../../lib/db/marketSignals';
import { marketSignalService } from '../../lib/market';
import type { MarketSignal, MarketSignalScore, MarketOpportunity, EmergingTrend } from '../../lib/market/types';
...
export default function MarketSignals() {
  ...
  async function loadMarketData() {
    ...
    try {
      const [signalsRes, scoresRes, oppsRes, trendsRes] = await Promise.all([
        ...
        getMarketOpportunities(currentOrganization.id, { status: 'identified' }),
  ...
  const signalsByIndustry = signals.reduce((acc, signal) => {
  ...
  const avgStrength = signals.length > 0
    ? Math.round(signals.reduce((sum, s) => sum + s.strength_score, 0) / signals.length)
    : 0;
    ...
    <div className="space-y-6">
      <div>
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-gray-600">Track external market motion and identify opportunities</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <Card>
          ...
          <p className="text-xs text-gray-500 mt-1">
            {signals.length > 0 ? Math.round((risingCount / signals.length) * 100) : 0}% of total
          </p>
        </Card>
      ...
      <div>
        ...
        {emergingTrends.length > 0 ? (
          ...
          <Card>
            <p className="text-center text-gray-500 py-8">No emerging trends detected yet</p>
          </Card>
        )}
      </div>
      ...
      <div>
        ...
        {opportunities.length > 0 ? (
          ...
          <Card>
            <p className="text-center text-gray-500 py-8">No opportunities identified yet</p>
          </Card>
        )}
      </div>

      <div>
        <h2 className="text-lg font-semibold text-gray-900 mb-4">Industry Activity</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {Object.entries(signalsByIndustry)
              ...
              .map(([industry, industrySignals]) => (
                <Card key={industry}>
                  <MarketSignalChart signals={industrySignals} industry={industry} />
                </Card>
              ))}
          </div>
        ) : (
          <Card>
            <p className="text-center text-gray-500 py-8">No signal data available</p>
          </Card>
        )}
      </div>
      ...
      {topScores.length > 0 && (
        <div>
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Top Scored Opportunities</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {topScores.map((score) => (
              <Card key={score.id}>
                <div className="mb-4">
                  <h3 className="font-semibold text-gray-900 mb-1">{score.industry}</h3>
                  {score.service_type && (
                    <p className="text-sm text-gray-600">{score.service_type}</p>
                  )}
                </div>
                ...
                <div className="pt-4 border-t border-gray-100">
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-medium text-gray-900">{score.momentum_score}%</span>
                  </div>
                  <div className="flex justify-between text-xs mt-1">
                    <span className="text-gray-500">Signals</span>
                    <span className="font-medium text-gray-900">{score.signal_count}</span>
                  </div>
                </div>
              </Card>
            ))}
