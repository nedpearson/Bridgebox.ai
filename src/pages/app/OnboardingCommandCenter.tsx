...
export default function OnboardingCommandCenter() {
  ...
  const [featureScopes, setFeatureScopes] = useState<
    Record<string, "tenant" | "platform">
  >({});
...
${Object.entries(featureScopes)
  .map(
    ([feat, scope]) =>
      `   - [${feat}]: Build as a ${scope === "platform" ? "GLOBAL PLATFORM FEATURE" : "TENANT-ISOLATED FEATURE"}`,
  )
  .join("\n")}
`;
  };
        ...
        <div className="bg-slate-900 rounded-xl border border-slate-800 overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-800">
            <h2 className="text-lg font-bold text-white">
              Active Scaffolding Architectures
            </h2>
          </div>

          {isLoading ? (
            <div className="p-6 text-center text-slate-500 py-24">
              Loading sessions...
            </div>
          ) : sessions.length === 0 ? (
                    ...
                    <div className="mt-6 pt-6 border-t border-slate-800 animate-fade-in space-y-6">
                      ...
                      <div className="bg-slate-900/50 rounded-lg p-5 border border-slate-800">
                        <h4 className="text-white font-medium mb-4 flex items-center">
                          ...
                          Feature Scope Decision Engine
                        </h4>
                        <p className="text-sm text-slate-400 mb-4">
                          Classify detected features to inject proper
                          architectural constraints into the Antigravity
                          compiler.
                        </p>
                      ...
                      <div className="bg-slate-950 rounded-lg p-5 border border-slate-800">
                        <div className="flex items-center justify-between mb-4">
                          <h4 className="text-emerald-400 font-medium text-sm tracking-widest uppercase flex items-center">
                            <Zap className="w-4 h-4 mr-2" /> Antigravity Agent
                            Prompt
                          </h4>
                          <button
                            onClick={() =>
                              copyPrompt(generateAntigravityPrompt(session))
                            }
                          ...
                          >
                            <Copy className="w-3 h-3 mr-1.5" /> Copy Explicit
                            Instructions
                          </button>
                        </div>
                      ...
                      <div className="bg-indigo-950/20 rounded-lg p-5 border border-indigo-500/20">
                        <div className="flex items-center justify-between mb-4">
                          <h4 className="text-indigo-400 font-medium tracking-widest uppercase flex items-center">
                            <Workflow className="w-5 h-5 mr-2" /> Bridgebox
                            Auto-Build Execution Queue
                          </h4>
                          ...
                          >
                            {isExecuting[session.id] ? (
                              <>
                                ...
                                Executing Architectures natively...
                              </>
                            ) : (
                              <>
                                <Play className="w-5 h-5 mr-2" /> Auto-Build
                                Complete Ecosystem
                              </>
                            )}
                          </button>
                      ...
                      <div className="flex items-center justify-end space-x-3 pt-2">
                        ...
                        >
                          <Check className="w-4 h-4 mr-1.5" /> Approve & Lock
                          Architecture
                        </button>
                      </div>
                    </div>
