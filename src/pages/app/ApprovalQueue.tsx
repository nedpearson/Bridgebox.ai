    ...
    return (
      <div className="bg-emerald-500/5 p-4 rounded-xl border border-emerald-500/20">
        ...
        <div className="text-xs text-slate-400">
          Target Entity: {payload.invoiceId || payload.targetId || "Unknown ID"}{" "}
          <br />
        ...
        {loading ? (
          <div className="space-y-4">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-48 bg-slate-900 animate-pulse rounded-xl border border-slate-800"
              ></div>
            ))}
          </div>
        ) : actions.length === 0 ? (
          <Card className="p-16 text-center bg-slate-900 border-slate-800 flex flex-col justify-center items-center">
            ...
            <p className="text-slate-400 max-w-sm">
              Your assistants have no pending operations requiring human
          ...
          <div className="space-y-6">
            <AnimatePresence>
              {actions.map((action) => {
                const isHighConfidence = action.confidence_score >= 0.9;
                ...
                return (
                  ...
                  >
                    <Card
                      className={`bg-slate-900 shadow-xl overflow-hidden group border transition-all ${isHighConfidence ? "border-blue-500/30 hover:border-blue-500/50" : "border-slate-800"}`}
                    >
                      {/* Action Header */}
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <h4 className="text-white font-bold text-sm">
                              {action.bb_tenant_agents?.bb_agents?.name ||
                                "Digital Assistant"}
                            </h4>
                            <p className="text-xs text-slate-400 flex items-center gap-1 mt-0.5 capitalize">
                              {action.bb_tenant_agents?.bb_agents?.category?.replace(
                                "_",
                                " ",
                              ) || "Operation"}
                              <span>•</span>
                      ...
                      <div className="p-6">
                        <div className="mb-6 flex gap-4 items-start">
                          <AlertTriangle
                            className={`w-5 h-5 flex-shrink-0 mt-0.5 ${action.risk_level === "high" ? "text-orange-400" : "text-slate-500"}`}
                          />
                          <div>
                            <h5 className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-1">
                        ...
                        <div className="mb-6 flex gap-4 items-start">
                          ...
                          <div className="flex-1">
                            <h5 className="text-xs font-bold uppercase tracking-widest text-indigo-500 mb-2">
                              Proposed Execution (The What)
                            </h5>
