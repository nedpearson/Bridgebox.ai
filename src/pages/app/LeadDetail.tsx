...
import {
  ConversionBadge,
  ConfidenceIndicator,
} from "../../components/predictions/PredictionBadge";
  ...
  const handleEnrichLead = async () => {
    ...
    try {
      ...
      const {
        ...
        data: enrichedData,
        error,
      } = await aiService.enrichLeadData(domain, bodyText);
  ...
  const loadLead = async () => {
    ...
    try {
      ...
      const [leadData, predictionData, contextInsights] = await Promise.all([
        ...
        predictiveAnalytics
          .predictLeadConversion(id, currentOrganization?.id)
          .catch(() => null),
        ...
        <div className="grid lg:grid-cols-3 gap-6">
                  ...
                  className={`border-none rounded-full px-3 py-1 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer ${
                    (lead.status as string) === "converted"
                      ...
                      : (lead.status as string) === "lost"
                        ...
                        : (lead.status as string) === "proposal_sent" ||
                            (lead.status as string) === "negotiation"
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              >
                  ...
                  <div className="space-y-4">
                    <div>
                      <p className="text-sm text-slate-300 leading-relaxed">
                        {aiSummary.data.overview}
                      </p>
                    </div>

                    {aiSummary.data.keyNeeds &&
                      aiSummary.data.keyNeeds.length > 0 && (
                        <div>
                          <p className="text-xs font-medium text-slate-400 mb-2">
                            Key Needs
                          </p>
                          <div className="space-y-1">
                            {aiSummary.data.keyNeeds.map(
                              (need: string, idx: number) => (
                                <div
                                  key={idx}
                    ...
                    {aiSummary.data.suggestedServices &&
                      aiSummary.data.suggestedServices.length > 0 && (
                        <div>
                          <p className="text-xs font-medium text-slate-400 mb-2">
                            Suggested Services
                          </p>
                          <div className="flex flex-wrap gap-2">
                            {aiSummary.data.suggestedServices.map(
                    ...
                    {aiSummary.data.nextActions &&
                      aiSummary.data.nextActions.length > 0 && (
                        <div className="pt-4 border-t border-slate-700">
                          <p className="text-xs font-medium text-slate-400 mb-2">
                            Recommended Next Actions
                          </p>
                          <div className="space-y-2">
                            {aiSummary.data.nextActions.map(
                ...
                {!aiSummary.data && !aiSummary.isAvailable && (
                  <div className="flex items-start gap-3 text-slate-400">
                    ...
                    <p className="text-sm">
                      AI analysis is not available. Configure an AI provider
                      (Anthropic or OpenAI) to enable intelligent lead
                      summaries.
                    </p>
                  </div>
                )}
            ...
            <Card glass className="p-6">
              ...
              {(lead as any).message &&
                (lead as any).message !== lead.project_description && (
                  <div className="mb-6">
                    <p className="text-slate-500 text-xs mb-2">
                      Additional Message
                    </p>
              ...
              {(lead as any).requested_service && (
                <div className="mb-6">
                  ...
                  <StatusBadge
                    status={(lead as any).requested_service}
                    variant="info"
                  />
                </div>
              )}
