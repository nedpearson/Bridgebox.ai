...
export const BuildOrchestratorAgent = {
  ...
  async extractTasksFromSession(
    ...
    return AgentRegistry.execute<{
      tasks: ExtractedBuildTask[];
      pricingModelId?: string;
    }>("BuildOrchestratorAgent", payload, async () => {
...
- supportAgentUsage: "basic" | "standard" | "advanced" | "enterprise"
        ...
        messages: [
          {
            role: "system",
            content:
              'You are an expert autonomous software integration engine. You must output exclusively in valid JSON object format containing a "tasks" array.',
          },
      ...
      if (!response.content)
        throw new Error(
          "AI failed to return any text content during extraction.",
        );
      ...
      const extractedTasks: ExtractedBuildTask[] = Array.isArray(parsedBlob)
        ? parsedBlob
        : parsedBlob.tasks || [];
      const extractedPricing: ExtractedPricingInputs | null =
        parsedBlob.pricing || null;
          ...
          await supabase
            ...
            .update({
              ai_intelligence: {
                ...
                pricing_inputs: extractedPricing,
              },
            })
            .eq("id", sessionId);
        } catch (pricingErr) {
          console.warn(
            "[BuildOrchestrator] Pricing model generation failed (non-fatal):",
            pricingErr,
          );
        }
      }
