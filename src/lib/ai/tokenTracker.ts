 ...
 */
export async function getOrganizationTokenSummary(organizationId: string, periodDays: number = 30) {
  ...
  const totalCost = data?.reduce((sum, r) => sum + parseFloat(r.cost_usd || '0'), 0) || 0;
 ...
 */
export async function getUserTokenBreakdown(organizationId: string, periodDays: number = 30) {
  const since = new Date(Date.now() - periodDays * 24 * 60 * 60 * 1000).toISOString();
  ...
  const byUser: Record<string, { tokens: number; cost: number; events: number }> = {};
  for (const row of data || []) {
