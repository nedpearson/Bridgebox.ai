  ...
  private generateMockResponse(prompt: string): string {
    if (prompt.includes("summarize") && prompt.includes("lead")) {
      return JSON.stringify({
        overview:
          "This lead represents a mid-sized company looking to modernize their legacy systems with a focus on cloud infrastructure and automation.",
        keyNeeds: [
    ...
    if (prompt.includes("summarize") && prompt.includes("project")) {
      return JSON.stringify({
        ...
        progress:
          "Project is 65% complete with 2 of 4 major milestones achieved. Current sprint is focused on API integration.",
        keyMilestones: [
    ...
    if (prompt.includes("summarize") && prompt.includes("ticket")) {
      return JSON.stringify({
        issue:
          "User authentication system experiencing intermittent timeout errors during peak hours",
        severity: "high",
        category: "technical",
        suggestedResolution:
          "Increase connection pool size and implement request queuing to handle peak load",
    ...
    if (prompt.includes("business insights")) {
      return JSON.stringify({
        summary:
          "Strong growth in custom software and system integration services. Client retention is excellent but lead conversion could be improved.",
        opportunities: [
          {
            title: "Expand Custom Software Services",
            description:
              "High demand and 80% conversion rate indicates market fit",
            impact: "high",
            effort: "medium",
          },
          {
            ...
            description:
              "Growing interest from healthcare sector with premium pricing potential",
            impact: "high",
            effort: "medium",
          },
        ],
        risks: [
          {
            ...
            description:
              "Average response time of 24+ hours may be losing opportunities",
            severity: "medium",
            mitigation:
              "Implement automated lead qualification and faster initial response",
          },
        ],
        recommendations: [
      ...
      return JSON.stringify([
        {
          ...
          priority: "high",
          reasoning:
            "3 leads valued over $100K have not been contacted in 5+ days",
          expectedOutcome:
            "Move leads to qualification stage and schedule discovery calls",
          effort: "quick",
        },
        {
          ...
          priority: "medium",
          reasoning:
            "Project approaching milestone deadline with 15% remaining work",
    ...
    if (prompt.includes("proposal")) {
      return JSON.stringify({
        executiveSummary:
          "This proposal outlines a comprehensive solution to modernize your legacy systems, improve operational efficiency, and enable data-driven decision making through cloud infrastructure and automation.",
        scopeHighlights: [
        ...
        valueProposition:
          "By modernizing your systems, you will reduce operational costs by approximately 30%, improve system reliability, and enable your team to focus on strategic initiatives rather than maintenance.",
        recommendedApproach:
          "Phased implementation over 4 months starting with infrastructure assessment, followed by pilot migration, full rollout, and optimization phase.",
        estimatedTimeline:
          "16-20 weeks from contract signing to full deployment",
      });
    }
    ...
    if (prompt.includes("priority") || prompt.includes("classify")) {
      return JSON.stringify({
        ...
        reasoning:
          "Issue affects multiple users and impacts core functionality",
        confidence: 0.88,
      });
    }
