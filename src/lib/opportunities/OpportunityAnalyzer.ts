...
export class OpportunityAnalyzer {
  async analyzeIndustryOpportunities(
    ...
    try {
      ...
      const [leadsRes, projectsRes, proposalsRes, signalsRes] =
        await Promise.all([
          supabase
            .from("bb_leads")
      ...
      for (const industry of industries) {
        const industryLeads =
          leadsRes.data?.filter((l) => l.industry === industry) || [];
        const industryProjects =
          projectsRes.data?.filter((p) => l.industry === industry) || [];
        const industryProposals =
          proposalsRes.data?.filter((p) => p.industry === industry) || [];
        const industrySignals =
          signalsRes.data?.filter((s) => s.industry === industry) || [];

        const currentRevenue = industryProjects.reduce(
        ...
        const wonProposals = industryProposals.filter(
          (p) => p.status === "accepted",
        ).length;
        ...
        const avgSignalStrength =
          industrySignals.length > 0
            ? industrySignals.reduce((sum, s) => sum + s.strength_score, 0) /
        ...
        industryProjects.forEach(
          (p: any) => p.service_type && services.add(p.service_type),
        );
        ...
        opportunities.push({
          ...
          client_count: new Set(industryProjects.map((p: any) => p.client_id))
            .size,
  ...
  async analyzeServiceOpportunities(
    ...
    try {
      ...
      for (const [service, items] of serviceMap.entries()) {
        ...
        const currentRevenue = projects.reduce(
          (sum, p) => sum + (p.budget || 0),
          0,
        );
        ...
        const scoreInput: ScoreInputData = {
          ...
          internal_capability_score:
            projects.length > 5 ? 85 : projects.length > 2 ? 70 : 55,
        };
    ...
    try {
      ...
      if (input?.min_confidence) {
        filtered = filtered.filter(
          (o) => o.confidence_level >= input.min_confidence!,
        );
      }
  ...
  async getRankedOpportunities(
    ...
    try {
      ...
      if (industryRes.error || serviceRes.error) {
        return {
          opportunities: [],
          error: industryRes.error || serviceRes.error,
        };
      }
