   ...
   async generateFromPrompt(prompt: string, organizationId: string, previousContext?: string) {
      ...
      try {
         ...
         if (previousContext) {
            ...
            messages.push({ role: 'user', content: `Please refine the previous template based on this new feedback: ${prompt}` });
         } else {
         ...
         if (!response.ok) {
            ...
            throw new Error(`OpenAI API Error: ${errorPayload.error?.message || response.statusText}`);
         }
         ...
         return {
            ...
            description: generatedPayload.description || `Mapped from prompt: ${prompt.substring(0, 50)}`,
            ...
            configuration_payload: generatedPayload.configuration_payload || { 
               entities: [], workflows: [], forms: [], ai_agents: [],
               mobile_config: { staff_app: { enabled: true, views: ['Dashboard'], features: [] }, customer_app: { enabled: false, views: [], features: [] }, offline_sync_enabled: true }
            },
            billing_rules: {},
            branding_tokens: {},
            is_overlay: false,
   ...
   fallbackGeneration(prompt: string) {
      return {
         ...
         configuration_payload: {
            ...
            ai_agents: [
               { name: "Dispatch Copilot", role: "Routing Assistant" }
            ],
            mobile_config: {
               staff_app: { enabled: true, views: ["Route Map", "Job Details"], features: ["Camera", "Voice-to-Text"] },
               customer_app: { enabled: true, views: ["Client Portal", "Appointment Booking"], features: ["Messaging", "Payments"] },
