...
export class MetricsAggregator {
  ...
  private async computeCRMMetrics(
    ...
    try {
      ...
      const totalBudget =
        leads?.reduce((sum, l) => sum + (l.estimated_budget || 0), 0) || 0;
      ...
      await dataPipelineService.storeMetric(
        ...
        newProposals,
        {
          ...
          metricData: {
            ...
            conversion_rate:
              newProposals > 0 ? (acceptedProposals / newProposals) * 100 : 0,
  ...
  private async computeProjectMetrics(
    ...
    try {
      ...
      await dataPipelineService.storeMetric(
        ...
        activeProjects,
        {
          ...
          metricData: {
            ...
            health_percentage:
              activeProjects > 0
                ? ((activeProjects - atRiskProjects) / activeProjects) * 100
                : 100,
          },
        },
      );
      ...
      const delayedMilestones =
        milestones?.filter((m) => m.status === "delayed").length || 0;
      ...
      await dataPipelineService.storeMetric(
        ...
        completedMilestones,
        {
          ...
          metricData: {
            ...
            on_time_rate:
              completedMilestones + delayedMilestones > 0
                ? (completedMilestones /
                    (completedMilestones + delayedMilestones)) *
                  100
                : 100,
          },
        },
      ...
      await dataPipelineService.storeMetric(
        ...
        newTickets,
        {
          ...
          metricData: {
            ...
            resolution_rate:
              newTickets > 0 ? (resolvedTickets / newTickets) * 100 : 0,
  ...
  private async computeEngagementMetrics(
    ...
    try {
      ...
      const actionTypes = activityLogs.reduce(
        (acc, log) => {
      ...
      const resourceTypes = activityLogs.reduce(
        (acc, log) => {
    ...
    try {
      ...
      for (const todayMetric of todayMetrics) {
        const similarMetrics = lastWeekMetrics.filter(
          (m) =>
            m.metric_type === todayMetric.metric_type &&
        ...
        const avgValue =
          similarMetrics.reduce((sum, m) => sum + Number(m.metric_value), 0) /
          similarMetrics.length;
        const stdDev = Math.sqrt(
          similarMetrics.reduce(
            (sum, m) => sum + Math.pow(Number(m.metric_value) - avgValue, 2),
            0,
          ) / similarMetrics.length,
        );
