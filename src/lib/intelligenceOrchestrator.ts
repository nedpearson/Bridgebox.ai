...
class IntelligenceOrchestrator {
  async generateFullIntelligence(organizationId?: string): Promise<IntelligenceSnapshot> {
    const [
      salesMetrics,
      operationsMetrics,
      clientMetrics,
    ...
    return {
      ...
      summary: {
        ...
        highPriorityActions: highPriorityInsights.length + actionStats.highPriority,
        opportunitiesIdentified: opportunities.length + opportunityData.highOpportunities,
  ...
  async generateExecutiveBriefing(organizationId?: string): Promise<IntelligenceBriefing[]> {
    ...
    if (intelligence.summary.criticalAlerts > 0) {
      const criticalInsights = intelligence.insights.filter(i => i.priority === 'critical');
      briefings.push({
    ...
    if (intelligence.predictions.clientChurn?.highRiskClients?.length > 0) {
      ...
      briefings.push({
        ...
        keyPoints: churn.highRiskClients.slice(0, 3).map((c: any) =>
    ...
    const opportunities = intelligence.insights.filter(i => i.category === 'opportunity');
  ...
  async getBusinessHealth(organizationId?: string): Promise<{
    ...
    score: number;
    factors: Array<{ name: string; status: 'good' | 'warning' | 'critical'; impact: number }>;
  }> {
    const intelligence = await this.generateFullIntelligence(organizationId);
    const factors: Array<{ name: string; status: 'good' | 'warning' | 'critical'; impact: number }> = [];
    ...
    if (intelligence.summary.opportunitiesIdentified > 0) {
      ...
      totalScore += Math.min(intelligence.summary.opportunitiesIdentified * 2, 10);
    }
    ...
    if (intelligence.metrics.sales?.conversionRate) {
      ...
      if (convRate > 30) {
        ...
        factors.push({ name: 'Sales Performance', status: 'warning', impact: -5 });
        totalScore -= 5;
      }
    }
  ...
  async generateActionRecommendations(organizationId: string, entityType: 'lead' | 'project' | 'ticket', entityId: string) {
    try {
      if (entityType === 'lead') {
        return await actionRecommender.recommendLeadActions(organizationId, entityId);
      } else if (entityType === 'project') {
        return await actionRecommender.recommendProjectActions(organizationId, entityId);
      } else if (entityType === 'ticket') {
        return await actionRecommender.recommendSupportActions(organizationId, entityId);
      }
      return [];
    } catch (error) {
