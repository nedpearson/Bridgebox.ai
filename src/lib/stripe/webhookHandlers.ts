  ...
  try {
    ...
    if (addonType) {
      ...
      if (addon?.creditValue) {
        ...
        Logger.info(
          `Provisioned ${addon.creditValue} credits for add-on ${addonType} to org ${organizationId}`,
        );
      }
...
export async function handleInvoicePaid(event: any) {
  ...
  try {
    ...
    try {
      ...
      Logger.info(
        `Monthly credits renewed for org ${organizationId} on plan ${planTier}`,
      );
    } catch (creditErr: any) {
...
export async function handleInvoicePaymentFailed(event: any) {
  ...
  try {
    ...
    Logger.error(
      `Invoice payment failed for organization ${organizationId}. Notification emitted.`,
    );
    return { success: true };
  } catch (error: any) {
