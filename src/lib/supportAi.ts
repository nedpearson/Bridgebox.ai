...
export const supportAiApi = {
  ...
  async triggerTriage(ticket: SupportTicket): Promise<AITriageResult> {
    try {
      ...
      if (!apiKey) {
         Logger.warn("OpenAI API Key is missing. Falling back to structured mock generation for dev.");
         const isUrgent = ticket.title.toLowerCase().includes('broken') || ticket.category === 'billing_issue';
         return {
