...
class MetricsEngine {
  async calculateConversionRate(
    ...
    const projectedRevenue =
      proposals?.reduce((sum, p) => sum + (p.total_amount || 0), 0) || 0;

    const leadToProposalRate =
      totalLeads > 0 ? (proposalsSent / totalLeads) * 100 : 0;
    const proposalAcceptanceRate =
      proposalsSent > 0 ? (proposalsAccepted / proposalsSent) * 100 : 0;
    const overallConversionRate =
      totalLeads > 0 ? (proposalsAccepted / totalLeads) * 100 : 0;
  ...
  async calculateMRR(filters: MetricsFilters = {}): Promise<RevenueMetrics> {
    ...
    subscriptions?.forEach((sub) => {
      ...
      revenueByService[serviceName] =
        (revenueByService[serviceName] || 0) + (sub.monthly_amount || 0);
    });
    ...
    const projectedProjectRevenue =
      projectRevenue + dailyRunRate * daysRemaining;
  ...
  async calculateProjectVelocity(
    ...
    let projectsQuery = supabase
      .from("bb_projects")
      .select(
        "id, status, health_status, created_at, start_date, end_date, completion_percentage",
      );
    ...
    const healthyProjects =
      projects?.filter(
        (p) => p.health_status === "excellent" || p.health_status === "good",
      ).length || 0;

    const atRiskProjects =
      projects?.filter(
        (p) => p.health_status === "at_risk" || p.health_status === "poor",
      ).length || 0;
  ...
  async calculateClientHealthScore(
    filters: MetricsFilters = {},
    ...
    if (organizationId) {
      healthScoresQuery = healthScoresQuery.eq(
        "organization_id",
        organizationId,
      );
    }
    ...
    const retentionRate =
      totalClients > 0 ? ((totalClients - 0) / totalClients) * 100 : 100;
  ...
  async calculateOnboardingMetrics(
    filters: MetricsFilters = {},
  ): Promise<OnboardingMetrics> {
    const { organizationId, dateRange } = filters;
    const startDate =
      dateRange?.startDate || new Date(Date.now() - 90 * 24 * 60 * 60 * 1000);
    const endDate = dateRange?.endDate || new Date();
  ...
  async calculateTemplateMetrics(
    filters: MetricsFilters = {},
    ...
    let installsQuery = supabase
      ...
      .select("id, template_id, created_at, bb_templates(name)", {
        count: "exact",
      });
