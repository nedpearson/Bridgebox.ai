...
class PredictiveAnalyticsEngine {
  async predictLeadConversion(
    ...
    let query_lead = supabase
      .from("bb_leads")
      .select("*, organization:bb_organizations(*)")
      .eq("id", leadId);
    if (organizationId)
      query_lead = query_lead.eq("organization_id", organizationId);
  ...
  async predictProjectDelay(
    ...
    let query_project = supabase
      .from("bb_projects")
      .select("*")
      .eq("id", projectId);
    if (organizationId)
      query_project = query_project.eq("organization_id", organizationId);
    ...
    let query_openTickets = supabase
      .from("bb_support_tickets")
      .select("id")
      .eq("project_id", projectId)
      .in("status", ["open", "in_progress"]);
    if (organizationId)
      query_openTickets = query_openTickets.eq(
        "organization_id",
        organizationId,
      );
  ...
  async predictProjectDelayBatch(
    ...
    const predictions = await Promise.all(
      projects.map((project) =>
        this.predictProjectDelay(project.id, organizationId),
      ),
    );
  ...
  async predictClientRisk(
    clientId: string,
    organizationId?: string,
  ): Promise<ClientChurnPrediction> {
    let query_client = supabase
      .from("bb_organizations")
      .select("*")
      .eq("id", clientId)
      .eq("type", "client");
    if (organizationId)
      query_client = query_client.eq("organization_id", organizationId);
    ...
    if (openTickets) {
      ...
      if (highPriorityTickets.length > 3) {
        riskScore += 30;
        riskFactors.push(
          `${highPriorityTickets.length} high-priority open tickets`,
        );
    ...
    let query_recentActivity = supabase
    ...
    if (organizationId)
      query_recentActivity = query_recentActivity.eq(
        "organization_id",
        organizationId,
      );
    ...
    let query_activeProjects = supabase
      .from("bb_projects")
      .select("id")
      .eq("organization_id", clientId)
      .eq("status", "in_progress");
    if (organizationId)
      query_activeProjects = query_activeProjects.eq(
        "organization_id",
        organizationId,
      );
  ...
  async predictClientRiskBatch(
    ...
    const predictions = await Promise.all(
      clients.map((client) =>
        this.predictClientRisk(client.id, organizationId),
      ),
    );
  ...
  async forecastRevenue(
    ...
    const currentMRR =
      historicalMRR?.reduce((sum, sub) => sum + (sub.monthly_amount || 0), 0) ||
      0;

    const last90DaysRevenue =
      historicalInvoices
        ?.filter((inv) => {
    ...
    for (let i = 1; i <= months; i++) {
      ...
      const projectedProjectRevenue =
        avgProjectRevenuePerMonth * (1 + i * 0.02);
  ...
  async generateExecutivePredictions(organizationId?: string) {
    const [leadPredictions, projectRisks, clientRisks, revenueForecast] =
      await Promise.all([
    ...
    const highValueLeads = leadPredictions
      .filter(
        (p) =>
          p.conversionLikelihood === "very_high" ||
    ...
    return {
      ...
      summary: {
        ...
        potentialRevenue: highValueLeads.reduce(
