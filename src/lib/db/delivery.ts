...
export type DeliveryPhase = 'discovery' | 'planning' | 'design' | 'build' | 'integration' | 'qa' | 'deployment' | 'support';
export type HealthStatus = 'green' | 'yellow' | 'red';
export type RiskLevel = 'none' | 'low' | 'medium' | 'high' | 'critical';
export type MilestoneStatus = 'not_started' | 'in_progress' | 'at_risk' | 'completed' | 'blocked';
export type DeliverableType = 'documentation' | 'design' | 'code' | 'dashboard' | 'mobile_app' | 'integration' | 'deployment' | 'training';
export type DeliverableStatus = 'pending' | 'in_progress' | 'review' | 'approved' | 'delivered';
export type NoteType = 'update' | 'risk' | 'blocker' | 'decision' | 'client_feedback';
  ...
  async getDeliveryStats() {
    ...
    const stats = {
      ...
      by_phase: {
        ...
        integration: data.filter((d) => d.delivery_phase === 'integration').length,
        qa: data.filter((d) => d.delivery_phase === 'qa').length,
        deployment: data.filter((d) => d.delivery_phase === 'deployment').length,
      ...
      high_risk: data.filter((d) => ['high', 'critical'].includes(d.risk_level)).length,
    };
