...
export const templateInstallEngine = {
  ...
  async resolveDependencies(templateId: string): Promise<BridgeboxTemplate[]> {
    ...
    for (const dep of deps) {
      ...
      const depData = await templateService.getTemplateById(
  ...
  async unpack(
    ...
    if (activeMemLocks.has(lockKey)) {
      Logger.warn(
        `[Install Reject] Template ${template.name} is already unpacking.`,
      );
      throw new Error(
        `Active unpacking transaction exists for ${template.name}.`,
      );
    }
    ...
    try {
      ...
      if (payload.entities && Array.isArray(payload.entities)) {
        for (const entity of payload.entities) {
          ...
          if (existingEntity) {
            Logger.info(
              `[Schema Merge] Entity ${entity.name} exists. Resolving conflicts leveraging ${template.merge_strategy}...`,
            );
          ...
          if (!error && data) {
            ...
            generatedAssetManifest.push({
              table: "bb_custom_entities",
              id: data.id,
              name: entity.name,
            });
          } else if (error) {
            throw new Error(
              `Failed to unroll entity ${entity.name}: ${error.message}`,
            );
          }
        }
      }
      ...
      if (payload.workflows && Array.isArray(payload.workflows)) {
        for (const workflow of payload.workflows) {
          ...
          if (existing) {
            if (template.merge_strategy === "skip_existing") {
              Logger.info(
                `Skipping workflow creation, ${workflow.name} exists.`,
              );
              continue;
          ...
          const { data, error } = await supabase
            ...
            .insert({
              ...
              name: workflow.name,
              description:
                workflow.description || `Installed via ${template.name}`,
          ...
          if (!error && data) {
            ...
            throw new Error(
              `Failed to unroll workflow ${workflow.name}: ${error.message}`,
            );
          }
        }
      }
      ...
      Logger.error(
        "Template Unpack Failure. Initiating automatic rollback.",
        e,
      );
      ...
      for (let i = generatedAssetManifest.length - 1; i >= 0; i--) {
        ...
        try {
          ...
          Logger.error(
            `CRITICAL ROLLBACK FAILURE on ${asset.table} ${asset.id}:`,
            rollbackError,
          );
        }
      }
