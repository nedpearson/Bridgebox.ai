...
export const analyticsService = {
  ...
  async getDeliveryAnalytics(
    ...
    try {
      const [
        ...
        organizationId
          ? supabase
              ...
              .select("status")
              .eq("organization_id", organizationId)
          : supabase.from("bb_projects").select("status"),
        organizationId
          ? supabase
              .from("bb_projects")
              .select("service_type_category")
              .eq("organization_id", organizationId)
          : supabase.from("bb_projects").select("service_type_category"),
      ...
      const projectsWithDates = projects.filter(
        (p) => p.start_date && p.actual_launch_date,
      );
      ...
      const [
        ...
        organizationId
          ? supabase
              ...
              .select("*")
              .eq("organization_id", organizationId)
          : supabase.from("bb_subscriptions").select("*"),
        organizationId
          ? supabase
      ...
      const [
        ...
        organizationId
          ? supabase
              ...
              .select("priority")
              .eq("organization_id", organizationId)
          : supabase.from("bb_support_tickets").select("priority"),
        organizationId
          ? supabase
              .from("bb_support_tickets")
              .select("category")
              .eq("organization_id", organizationId)
          : supabase.from("bb_support_tickets").select("category"),
        organizationId
          ? supabase
              .from("bb_support_tickets")
              .select("status")
              .eq("organization_id", organizationId)
          : supabase.from("bb_support_tickets").select("status"),
      ]);
  ...
  async getClientHealthAnalytics(
    organizationId?: string,
  ): Promise<ClientHealthAnalytics> {
    try {
      const [organizationsResult, clientOrgsResult, projectsResult] =
        await Promise.all([
          ...
          organizationId
            ? supabase
                .from("bb_projects")
                .select("organization_id, contract_value")
                .eq("organization_id", organizationId)
            : supabase
                .from("bb_projects")
                .select("organization_id, contract_value"),
      ...
      const onboardingCompleted = clients.filter(
        (c) => c.onboarding_completed,
      ).length;
      const onboardingInProgress = clients.filter(
        (c) => !c.onboarding_completed && c.onboarding_status === "in_progress",
      ).length;
      ...
      const clientsByType = [
        {
          type: "Platform Subscriber",
          count: clients.filter(
            (c) => !c.is_enterprise_client && c.subscription_status,
          ).length,
        },
        {
          type: "Enterprise Client",
          count: clients.filter((c) => c.is_enterprise_client).length,
        },
        {
          type: "Free/Trial",
          count: clients.filter(
      ...
      const enterpriseClients = clients.filter(
        (c) => c.is_enterprise_client,
      ).length;
      ...
      projects.forEach((p) => {
        ...
        clientValues.set(
          p.organization_id,
          current + (Number(p.contract_value) || 0),
        );
      });

      const averageClientValue =
        clientValues.size > 0
          ? Array.from(clientValues.values()).reduce((sum, v) => sum + v, 0) /
            clientValues.size
          : 0;
  ...
  async getConversionMetrics(organizationId?: string) {
    const [leads, proposals, projects, orgs] = await Promise.all([
      ...
      organizationId
        ? supabase
            .from("bb_proposals")
            .select("id, status, converted_to_project")
            .eq("organization_id", organizationId)
        : supabase
            .from("bb_proposals")
            .select("id, status, converted_to_project"),
      organizationId
        ? supabase
            ...
            .select("id, source")
            .eq("organization_id", organizationId)
        : supabase.from("bb_projects").select("id, source"),
    ...
    return {
      ...
      convertedLeads:
        leads.data?.filter((l) => l.converted_to_client).length || 0,
      totalProposals: proposals.data?.length || 0,
      approvedProposals:
        proposals.data?.filter((p) => p.status === "approved").length || 0,
      convertedProposals:
        proposals.data?.filter((p) => p.converted_to_project).length || 0,
      totalProjects: projects.data?.length || 0,
      pendingOnboarding:
        orgs.data?.filter((o) => o.onboarding_status === "in_progress")
          .length || 0,
    };
  },
