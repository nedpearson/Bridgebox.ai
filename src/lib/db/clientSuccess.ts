...
class ClientSuccessService {
  ...
  async calculateAndSaveHealthScore(
    organizationId: string,
  ): Promise<ClientHealthScore> {
    const [onboardingScore, projectScore, supportScore, engagementScore] =
      await Promise.all([
  ...
  private async calculateOnboardingScore(
    organizationId: string,
  ): Promise<number> {
  ...
  private async calculateProjectScore(organizationId: string): Promise<number> {
    ...
    return Math.round(
      avgProgress * 0.6 + (onTimeProjects / projects.length) * 40,
    );
  }
  ...
  async createInteraction(
    interaction: Omit<ClientInteraction, "id" | "created_at" | "updated_at">,
  ): Promise<ClientInteraction> {
    const { data, error } = await supabase
      .from("bb_client_interactions")
      .insert(interaction)
  ...
  async getSuccessOpportunities(
    organizationId: string,
  ): Promise<SuccessOpportunity[]> {
    const { data, error } = await supabase
  ...
  async createOpportunity(
    opportunity: Omit<SuccessOpportunity, "id" | "created_at" | "updated_at">,
  ): Promise<SuccessOpportunity> {
    const { data, error } = await supabase
      .from("bb_success_opportunities")
      .insert(opportunity)
  ...
  async createRiskFlag(
    risk: Omit<RiskFlag, "id" | "created_at" | "updated_at">,
  ): Promise<RiskFlag> {
    const { data, error } = await supabase
      .from("bb_risk_flags")
      .insert(risk)
  ...
  async getClientSuccessOverview(
    organizationId: string,
  ): Promise<ClientSuccessOverview> {
    const [
      org,
      healthScore,
      accountOwner,
      ...
      supabase
        .from("bb_organizations")
        .select("name, billing_plan, subscription_status")
        .eq("id", organizationId)
        .maybeSingle(),
      ...
      supabase
        ...
        .select("id, status")
        .eq("organization_id", organizationId)
        .in("status", ["in_progress", "planning", "testing"]),
      supabase
        .from("bb_support_tickets")
        .select("id")
        .eq("organization_id", organizationId)
