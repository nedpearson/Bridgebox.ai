...
export const enhancementRequestsService = {
  ...
  async updateStatus(id: string, status: EnhancementStatus, workspaceId: string): Promise<void> {
    const { data: userResult } = await supabase.auth.getUser();
    const { error } = await supabase
      .from('bb_enhancement_requests')
      .update({ status, updated_by: userResult.user?.id, updated_at: new Date().toISOString() })
      .eq('id', id)
      .eq('workspace_id', workspaceId);
  ...
  async reject(id: string, workspaceId: string, reason?: string): Promise<void> {
    const { data: userResult } = await supabase.auth.getUser();
    const { error } = await supabase
      .from('bb_enhancement_requests')
      .update({
