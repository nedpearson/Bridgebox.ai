...
export const usageEventsService = {
  ...
  async getMonthlyBreakdown(organizationId: string): Promise<Record<UsageMetricType, number>> {
    const now = new Date();
  ...
  async getDailyTimeline(
    organizationId: string,
    days = 30
  ): Promise<Array<{ date: string; total: number; byType: Partial<Record<UsageMetricType, number>> }>> {
    const from = new Date();
    ...
    const byDay: Record<string, { total: number; byType: Partial<Record<UsageMetricType, number>> }> = {};
    ...
    for (const row of data ?? []) {
      ...
      byDay[date].byType[t] = (byDay[date].byType[t] ?? 0) + (row.quantity ?? 1);
    }
  ...
  async getRecentEvents(organizationId: string, limit = 50): Promise<UsageEvent[]> {
    const { data, error } = await supabase
      .from('bb_usage_events')
      .select('*')
      .eq('organization_id', organizationId)
