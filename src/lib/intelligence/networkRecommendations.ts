    ...
    try {
      ...
      return (
        recommendations?.map((rec) => ({
          id: rec.id,
          title: Array.isArray(rec.bb_templates)
            ? rec.bb_templates[0]?.name
            : (rec.bb_templates as any)?.name,
          description: Array.isArray(rec.bb_templates)
            ? rec.bb_templates[0]?.description
            : (rec.bb_templates as any)?.description,
