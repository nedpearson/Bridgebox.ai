...
class TrendDetectionEngine {
  ...
  async detectTrendingServices(
    ...
    let query_currentLeads = supabase
      .from("bb_leads")
      .select("lead_type, estimated_budget")
      .gte("created_at", currentPeriodStart.toISOString());
    if (organizationId)
      query_currentLeads = query_currentLeads.eq(
        "organization_id",
        organizationId,
      );
    const { data: currentLeads } = await query_currentLeads;

    let query_previousLeads = supabase
      .from("bb_leads")
      .select("lead_type, estimated_budget")
      .gte("created_at", previousPeriodStart.toISOString())
      .lt("created_at", previousPeriodEnd.toISOString());
    if (organizationId)
      query_previousLeads = query_previousLeads.eq(
        "organization_id",
        organizationId,
      );
    ...
    currentLeads?.forEach((lead) => {
      const existing = currentCounts.get(lead.lead_type) || {
        count: 0,
        revenue: 0,
      };
    ...
    previousLeads?.forEach((lead) => {
      const existing = previousCounts.get(lead.lead_type) || {
        count: 0,
        revenue: 0,
      };
    ...
    allServiceTypes.forEach((serviceType) => {
      const current = currentCounts.get(serviceType) || {
        count: 0,
        revenue: 0,
      };
      const previous = previousCounts.get(serviceType) || {
        count: 0,
        revenue: 0,
      };
  ...
  async detectIndustryGrowth(
    ...
    const currentCounts = new Map<
      string,
      { count: number; revenue: number; converted: number }
    >();
    const previousCounts = new Map<
      string,
      { count: number; revenue: number; converted: number }
    ...
    currentLeads?.forEach((lead) => {
      ...
      const existing = currentCounts.get(lead.industry) || {
        count: 0,
        revenue: 0,
        converted: 0,
      };
    ...
    previousLeads?.forEach((lead) => {
      if (!lead.industry) return;
      const existing = previousCounts.get(lead.industry) || {
        count: 0,
        revenue: 0,
        converted: 0,
      };
    ...
    allIndustries.forEach((industry) => {
      const current = currentCounts.get(industry) || {
        count: 0,
        revenue: 0,
        converted: 0,
      };
      const previous = previousCounts.get(industry) || {
        count: 0,
        revenue: 0,
        converted: 0,
      };
      ...
      const conversionRate =
        current.count > 0 ? (current.converted / current.count) * 100 : 0;
  ...
  async detectDemandSpikes(
    ...
    let query_tickets = supabase
      .from("bb_support_tickets")
      .select("title, description, category, created_at")
      .gte("created_at", periodStart.toISOString());
    if (organizationId)
      query_tickets = query_tickets.eq("organization_id", organizationId);
    ...
    const avgFrequency =
      Array.from(keywordFrequency.values()).reduce(
        (sum, v) => sum + v.count,
        0,
      ) / keywordFrequency.size || 1;

    keywordFrequency.forEach((data, keyword) => {
      const daysSinceFirst =
        (Date.now() - data.firstSeen.getTime()) / (1000 * 60 * 60 * 24);
      const isEmergingTrend =
        daysSinceFirst < 30 && data.count > avgFrequency * 1.5;
  ...
  async detectFeatureUsageTrends(
    organizationId?: string,
    daysBack: number = 90,
  ): Promise<FeatureUsageTrend[]> {
    const currentPeriodStart = new Date(
      Date.now() - daysBack * 24 * 60 * 60 * 1000,
    );
    const previousPeriodStart = new Date(
    ...
    let query_currentActivity = supabase
      .from("bb_activity_logs")
      .select("action, user_id")
      .gte("created_at", currentPeriodStart.toISOString());
    if (organizationId)
      query_currentActivity = query_currentActivity.eq(
        "organization_id",
        organizationId,
      );
    const { data: currentActivity } = await query_currentActivity;

    let query_previousActivity = supabase
      .from("bb_activity_logs")
      .select("action")
      .gte("created_at", previousPeriodStart.toISOString())
      .lt("created_at", currentPeriodStart.toISOString());
    if (organizationId)
      query_previousActivity = query_previousActivity.eq(
        "organization_id",
        organizationId,
      );
    ...
    currentActivity?.forEach((log) => {
      ...
      const existing = currentCounts.get(feature) || {
        count: 0,
    ...
    allFeatures.forEach((feature) => {
      const current = currentCounts.get(feature) || {
        count: 0,
        users: new Set(),
      };
  ...
  async detectClientRequestPatterns(
    organizationId?: string,
    daysBack: number = 90,
  ): Promise<ClientRequestPattern[]> {
    const periodStart = new Date(Date.now() - daysBack * 24 * 60 * 60 * 1000);
    ...
    const commonPatterns = [
      {
        ...
        keywords: ["report", "reporting", "analytics", "dashboard"],
      },
      {
      ...
      },
      {
        ...
        keywords: ["automate", "automation", "workflow", "trigger"],
      },
      {
        pattern: "Performance Optimization",
        keywords: ["slow", "performance", "optimize", "speed"],
      },
      {
        pattern: "Data Migration",
        keywords: ["migrate", "migration", "import", "transfer"],
      },
      {
      ...
      },
      {
        ...
        keywords: ["real-time", "live", "instant", "websocket"],
      },
    ];
      ...
      commonPatterns.forEach(({ pattern, keywords }) => {
        if (keywords.some((keyword) => text.includes(keyword))) {
          ...
          if (ticket.organization_id)
            existing.clients.add(ticket.organization_id);
          if (ticket.category) existing.services.add(ticket.category);
      ...
      commonPatterns.forEach(({ pattern, keywords }) => {
        if (keywords.some((keyword) => text.includes(keyword))) {
          ...
          if (project.organization_id)
            existing.clients.add(project.organization_id);
    ...
    patterns.forEach((data, pattern) => {
      ...
      results.push({
        ...
        averageValue:
          data.clients.size > 0 ? data.totalValue / data.clients.size : 0,
        relatedServices: Array.from(data.services),
        isGrowing,
      });
    });
  ...
  async generateTrendInsights(
    ...
    const [serviceTrends, industryTrends, demandSpikes, requestPatterns] =
      await Promise.all([
    ...
    if (
      topServiceTrend &&
      topServiceTrend.direction === "up" &&
      topServiceTrend.growthRate > 30
    ) {
      insights.push({
        type: "opportunity",
    ...
    if (
      topIndustryTrend &&
      topIndustryTrend.direction === "up" &&
      topIndustryTrend.growthRate > 40
    ) {
      insights.push({
        type: "opportunity",
        ...
        action:
          "Develop industry-specific case studies and marketing materials",
      });
    }
    ...
    const growingPattern = requestPatterns.find(
      (p) => p.isGrowing && p.frequency > 5,
    );
    if (growingPattern) {
      insights.push({
        type: "opportunity",
    ...
    const decliningServices = serviceTrends.filter(
      (s) => s.direction === "down" && s.growthRate < -20,
    );
  ...
  async getHotOpportunities(organizationId?: string) {
    const [serviceTrends, industryTrends, demandSpikes, insights] =
      await Promise.all([
        this.detectTrendingServices(organizationId),
        this.detectIndustryGrowth(organizationId),
        this.detectDemandSpikes(organizationId),
    ...
    return {
      ...
      summary: {
        ...
        opportunityScore:
          insights.filter((i) => i.type === "opportunity").length * 20,
      },
    };
  }
}
