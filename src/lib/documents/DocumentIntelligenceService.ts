...
export class DocumentIntelligenceService {
  ...
  async executeIntelligencePipeline(documentId: string, tenantId: string) {
    ...
    if (!text || text.length < 10) {
      console.warn(
        "Document has no extracted text, skipping intelligence pipeline.",
      );
      return null;
    }

    // 2. Ask AI to Classify & Extract
    const classification = await this.askAIForClassification(
      text,
      document.file_name,
    );
    ...
    const proposedMatches = await this.findDatabaseMatches(
      tenantId,
      classification.inferred_targets,
    );
    ...
    const isHighlyConfident =
      classification.confidence_score >= AUTO_ATTACH_THRESHOLD;
    const hasSingleStrongMatch =
      proposedMatches.organizations.length === 1 &&
      proposedMatches.projects.length === 0 &&
      proposedMatches.people.length === 0;
  ...
  private async askAIForClassification(
    text: string,
    filename: string,
  ): Promise<IntelligenceClassification | null> {
    const messages = [
      {
        role: "system",
    ...
    const response = await aiService.executeAITask<IntelligenceClassification>(
      messages,
      false,
    );
  ...
  private async findDatabaseMatches(
    tenantId: string,
    targets: IntelligenceClassification["inferred_targets"],
  ) {
    const results = {
