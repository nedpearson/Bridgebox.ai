...
export function calculatePricing(config: PricingConfig): PricingOutput {
  ...
  const totalMonthlyMsrp =
    baseMsrp + perUserMsrp + techMsrp + aiMsrp + mobileMsrp + locationMsrp;
  ...
  const integrationCogs = config.integrations.length * 2.5;
  ...
  const grossMarginPercentage = Math.round(
    (grossProfit / totalMonthlyMsrp) * 100,
  );
