...
export function scoreMarketOpportunity(signals: MarketSignal[]): number {
  ...
  const baseScore = (strengthScore * 0.4 + momentumScore * 0.3 + recencyBonus * 0.2 + diversityBonus * 0.1);
...
export function calculateMomentumScore(signals: MarketSignal[]): number {
  ...
  const risingCount = signals.filter(s => s.growth_direction === 'rising' || s.growth_direction === 'emerging').length;
...
export function generateScoreInsights(
  ...
  if (score.momentum_score >= 70 && score.growth_direction === 'rising') {
    recommendations.push({
      ...
      context: 'Market momentum is accelerating. Early entry could maximize advantage.',
    });
  }
  ...
  if (score.confidence_level < 40) {
    recommendations.push({
      ...
      context: 'Consider gathering additional market signals before taking action.',
    });
  }
  ...
  const serviceTypes = new Set(signals.map(s => s.service_type).filter(Boolean));
  if (serviceTypes.size > 3) {
    recommendations.push({
      type: 'insight',
      priority: 'low',
      message: 'Diverse service demand detected',
      context: 'Multiple service types showing interest. Consider bundled offerings.',
    });
  }
...
export function extractKeySignals(signals: MarketSignal[], limit: number = 5): KeySignal[] {
  return signals
    .sort((a, b) => {
...
export function computeSignalScore(
...
): Omit<MarketSignalScore, 'id' | 'organization_id' | 'created_at' | 'updated_at'> {
  ...
  const avgVelocity = signals.length > 0
    ? signals.reduce((sum, s) => sum + s.velocity, 0) / signals.length
    : 0;

  const growthDirection = inferGrowthDirection(avgVelocity, avgStrength, signals.length);
...
export function rankOpportunities(scores: MarketSignalScore[]): MarketSignalScore[] {
  return [...scores].sort((a, b) => {
    const scoreA = a.opportunity_score * 0.5 + a.momentum_score * 0.3 + a.confidence_level * 0.2;
    const scoreB = b.opportunity_score * 0.5 + b.momentum_score * 0.3 + b.confidence_level * 0.2;
    return scoreB - scoreA;
  });
}

export function getTopEmergingSignals(scores: MarketSignalScore[], limit: number = 10): MarketSignalScore[] {
  return scores
    .filter(s => s.growth_direction === 'rising' || s.growth_direction === 'emerging')
