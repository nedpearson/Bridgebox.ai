...
export function normalizeSignal(input: SignalIngestionInput): Omit<MarketSignal, 'id' | 'organization_id' | 'created_at' | 'updated_at'> {
  return {
    source: input.source,
...
export function calculateVelocity(dataPoints: { value: number; date: string }[]): number {
...
export function detectSignalAnomaly(
  ...
  const avgStrength =
    historical.reduce((sum, s) => sum + s.strength_score, 0) / historical.length;
  const stdDev = Math.sqrt(
    historical.reduce((sum, s) => sum + Math.pow(s.strength_score - avgStrength, 2), 0) /
      historical.length
  );
