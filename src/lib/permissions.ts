...
export const permissions = {
  ...
  canManageOrganization(
    ...
    return (
      context.role === "client_admin" && context.organizationId === targetOrgId
    );
  },
  ...
  canEditProject(context: PermissionContext, projectOrgId: string): boolean {
    ...
    return (
      context.role === "client_admin" && context.organizationId === projectOrgId
    );
  },
  ...
  canManageBilling(context: PermissionContext, billingOrgId?: string): boolean {
    ...
    if (!billingOrgId) return false;
    return (
      context.role === "client_admin" && context.organizationId === billingOrgId
    );
  },
  ...
  canManageIntegrations(
    ...
    return (
      ...
      context.organizationId === integrationOrgId
    );
  },
  ...
  canManageUsers(context: PermissionContext, targetOrgId?: string): boolean {
    ...
    if (
      context.role === "internal_staff" &&
      context.organizationType === "internal"
    ) {
      return true;
    }
    if (!targetOrgId) return false;
